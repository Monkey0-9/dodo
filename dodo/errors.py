...
class dodoError(Exception):
    """Base class for all dodo related errors."""
...
class PendingApprovalError(dodoError):
...
class NoActiveRunsToCancelError(dodoError):
...
class ConcurrentUpdateError(dodoError):
...
class ConversationBusyError(dodoError):
...
class MemoryRepoBusyError(dodoError):
...
class dodoToolCreateError(dodoError):
...
class dodoToolNameConflictError(dodoError):
...
class dodoToolNameSchemaMismatchError(dodoToolCreateError):
...
class dodoConfigurationError(dodoError):
...
class EmbeddingConfigRequiredError(dodoError):
...
class dodoAgentNotFoundError(dodoError):
    """Error raised when an agent is not found."""


class dodoUserNotFoundError(dodoError):
    """Error raised when a user is not found."""


class dodoUnsupportedFileUploadError(dodoError):
    """Error raised when an unsupported file upload is attempted."""


class dodoInvalidArgumentError(dodoError):
...
class dodoImageFetchError(dodoError):
...
class dodoMCPError(dodoError):
    """Base error for MCP-related issues."""


class dodoInvalidMCPSchemaError(dodoMCPError):
...
class dodoMCPConnectionError(dodoMCPError):
...
class dodoMCPTimeoutError(dodoMCPError):
...
class dodoServiceUnavailableError(dodoError):
...
class dodoUnexpectedStreamCancellationError(dodoError):
    """Error raised when a streaming request is terminated unexpectedly."""


class dodoExpiredError(dodoError):
...
class BedrockPermissionError(dodoError):
...
class LLMJSONParsingError(dodoError):
...
class LocalLLMConnectionError(dodoError):
...
class ContextWindowExceededError(dodoError):
...
class RateLimitExceededError(dodoError):
...
class dodoMessageError(dodoError):
    """Base error class for handling message-related errors."""

    messages: List[Union["Message", "dodoMessage"]]
    default_error_message: str = "An error occurred with the message."

    def __init__(self, *, messages: List[Union["Message", "dodoMessage"]], explanation: Optional[str] = None) -> None:
        error_msg = self.construct_error_message(messages, self.default_error_message, explanation)
        super().__init__(error_msg)
        self.messages = messages

    @staticmethod
    def construct_error_message(messages: List[Union["Message", "dodoMessage"]], error_msg: str, explanation: Optional[str] = None) -> str:
...
class HandleNotFoundError(dodoError):
...
class InsufficientCreditsError(dodoError):
...
class RunCancelError(dodoError):
