...
class LoadGate:
    ...
    # ------------------------------------------------------------------
    # In-flight tracking â€” called from streaming_service on start/end
    # ------------------------------------------------------------------
    ...
    def on_admission_wait(self, wait_ms: float) -> None:
        ...
        try:
            ...
            from dodo.monitoring.readiness_state import get_readiness_state
...
# Singleton â€” one gate per process.
