...
class EventLoopWatchdog:
    ...
    def _maybe_degrade_readiness(self, now: float, current_lag_ms: float) -> None:
        ...
        try:
            ...
            from dodo.monitoring.readiness_state import get_readiness_state, mark_degraded
    ...
    def _maybe_recover_readiness(self, now: float) -> None:
        ...
        try:
            ...
            from dodo.monitoring.readiness_state import clear_degraded, get_readiness_state
        ...
        try:
            ...
            for task in active_tasks:
                try:
                    ...
                    # Find top dodo frame for grouping
                    for frame in reversed(stack):
                        if "dodo" in frame.f_code.co_filename:
                            idx = frame.f_code.co_filename.find("dodo/")
            ...
            logger.warning(f"  dodo tasks: {total_tasks} total")
            ...
            for i, (location, tasks) in enumerate(sorted_patterns, 1):
                ...
                if i <= 3:
                    ...
                    for frame in sample_stack[-8:]:
                        ...
                        dodo_idx = filename.find("dodo/")
                        if dodo_idx != -1:
                            short_path = filename[dodo_idx + 6 :]
