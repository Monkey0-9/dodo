...
from dodo.constants import NON_USER_MSG_PREFIX
...
from dodo.helpers.json_helpers import json_dumps, json_loads, sanitize_unicode_surrogates
...
from dodo.local_llm.json_parser import clean_json_string_extra_backslash
...
from dodo.schemas.openai.chat_completion_response import ChatCompletionResponse, Choice, FunctionCall, Message, ToolCall, UsageStatistics
...
class GoogleVertexClient(LLMClientBase):
    ...
    def add_dummy_model_messages(messages: List[dict]) -> List[dict]:
        ...
        In dodo, the 'model' will often call a function (e.g. send_message) that itself yields to the user,
    ...
    def _resolve_json_schema_refs(self, schema: dict, defs: dict | None = None) -> dict:
        ...
        if "$ref" in schema:
            ...
            logger.warning(f"Could not resolve $ref '{ref}' in schema â€” will be stripped by schema cleaner")
    ...
    def convert_tools_to_google_ai_format(self, tools: List[Tool], llm_config: LLMConfig) -> List[dict]:
        ...
        for func in function_list:
            ...
            if llm_config.put_inner_thoughts_in_kwargs:
                from dodo.local_llm.constants import INNER_THOUGHTS_KWARG_DESCRIPTION, INNER_THOUGHTS_KWARG_VERTEX
    ...
    def build_request_data(
        ...
        contents = self.add_dummy_model_messages(
            PydanticMessage.to_google_dicts_from_list(
                ...
                put_inner_thoughts_in_kwargs=False if agent_type == AgentType.dodo_v1_agent else True,
                native_content=True if agent_type == AgentType.dodo_v1_agent else False,
            ),
        )
    ...
    def extract_usage_statistics(self, response_data: dict | None, llm_config: LLMConfig) -> dodoUsageStatistics:
        """Extract usage statistics from Gemini response and return as dodoUsageStatistics."""
                ...
                for response_message in parts:
                    ...
                    if response_message.function_call:
                        ...
                        if llm_config.put_inner_thoughts_in_kwargs:
                            ...
                            from dodo.local_llm.constants import INNER_THOUGHTS_KWARG_VERTEX
    ...
    async def count_tokens(
        ...
        """
        from dodo.llm_api.google_constants import GOOGLE_MODEL_FOR_API_KEY_CHECK
