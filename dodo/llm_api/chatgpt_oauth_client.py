...
from dodo.schemas.providers.chatgpt_oauth import ChatGPTOAuthCredentials, ChatGPTOAuthProvider
...
class ChatGPTOAuthClient(LLMClientBase):
    ...
    def extract_usage_statistics(self, response_data: dict | None, llm_config: LLMConfig) -> dodoUsageStatistics:
        """Extract usage statistics from ChatGPT OAuth response and return as dodoUsageStatistics."""
    ...
    async def stream_async(
        ...
        async def stream_generator():
            ...
            sequence_counter = 0
            # Track whether we've yielded any events â€” once we have, we can't
            ...
            for attempt in range(self.MAX_RETRIES):
                try:
                    ...
                    if has_yielded or attempt >= self.MAX_RETRIES - 1:
                        # Already yielded partial data or exhausted retries â€” must propagate
                        raise
                    wait = 2**attempt
                    logger.warning(
    ...
    def handle_llm_error(self, e: Exception, llm_config: Optional[LLMConfig] = None) -> Exception:
        ...
        # Already a typed LLM/dodo error (e.g. from SSE error handling) â€” pass through
        if isinstance(e, dodoError):
            return e
