...
from dodo.constants import FUNC_FAILED_HEARTBEAT_MESSAGE, REQ_HEARTBEAT_MESSAGE, REQUEST_HEARTBEAT_PARAM
...
from dodo.schemas.openai.chat_completion_request import Tool as OpenAITool
...
class AnthropicClient(LLMClientBase):
        ...
        if async_client:
            if api_key:
                if is_oauth_provider:
                    return client_manager.get_async_anthropic_client(
                ...
                return client_manager.get_async_anthropic_client(api_key=api_key, max_retries=model_settings.anthropic_max_retries)
            return client_manager.get_async_anthropic_client(max_retries=model_settings.anthropic_max_retries)

        if api_key:
            if is_oauth_provider:
                return client_manager.get_sync_anthropic_client(
                    max_retries=model_settings.anthropic_max_retries,
                    default_headers={
                        "Authorization": f"Bearer {api_key}",
                        "anthropic-version": "2023-06-01",
            ...
            return client_manager.get_sync_anthropic_client(api_key=api_key, max_retries=model_settings.anthropic_max_retries)
        return client_manager.get_sync_anthropic_client(max_retries=model_settings.anthropic_max_retries)
    ...
    def build_request_data(
        ...
        prefix_fill = True if agent_type != AgentType.dodo_v1_agent else False
        is_v1 = agent_type == AgentType.dodo_v1_agent
        ...
        if not tools:
            ...
            tool_choice = None
        elif (self.is_reasoning_model(llm_config) and llm_config.enable_reasoner) or agent_type == AgentType.dodo_v1_agent:
        ...
        if agent_type == AgentType.dodo_v1_agent:
    ...
    def extract_usage_statistics(self, response_data: dict | None, llm_config: LLMConfig) -> dodoUsageStatistics:
        """Extract usage statistics from Anthropic response and return as dodoUsageStatistics."""
