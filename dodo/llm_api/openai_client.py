...
from dodo.schemas.openai.responses_request import ResponsesRequest
from dodo.schemas.response_format import JsonSchemaResponseFormat
...
class OpenAIClient(LLMClientBase):
    ...
    def _is_true_openai_request(self, llm_config: LLMConfig) -> bool:
        ...
        # Keep dodo inference endpoint behavior unchanged.
        if llm_config.model_endpoint == dodo_MODEL_ENDPOINT:
            return False
    ...
    def _apply_prompt_cache_settings(
        ...
        good cache affinity for dodo agents, since each agent has a unique system
    ...
    def build_request_data_responses(
        ...
        if tools:
            ...
            # (temporal_dodo_v1_agent_workflow.py or dodo_agent_v3.py) before reaching here.
        ...
        # Handle "fast" model variants â†’ real model + priority service tier
        ...
        if llm_config.model_endpoint == dodo_MODEL_ENDPOINT:
            if not self.actor:
                # override user id for inference.dodo.com
                import uuid

                data.user = str(uuid.UUID(int=0))

            data.model = "dodo-openai"
    ...
    def build_request_data(
        ...
        """
        # Shortcut for GPT-5 to use Responses API, but only for dodo_v1_agent
        if use_responses_api(llm_config) and agent_type == AgentType.dodo_v1_agent:
        ...
        if tools:  # only set tool_choice if tools exist
            if force_tool_call is not None:
                ...
                tool_choice = "required"
            elif self.requires_auto_tool_choice(llm_config) or agent_type == AgentType.dodo_v1_agent:
                tool_choice = "auto"
            else:
    ...
    def extract_usage_statistics(self, response_data: dict | None, llm_config: LLMConfig) -> dodoUsageStatistics:
        """Extract usage statistics from OpenAI response and return as dodoUsageStatistics."""
    ...
    async def stream_async(
        ...
        Args:
            ...
            ws_session: An active ``OpenAIWSSessionManager``. When provided *and*
                ...
                on-the-fly (but won't persist across steps â€” prefer passing one).
        """
        # Sanitize Unicode surrogates to prevent encoding errors
        request_data = sanitize_unicode_surrogates(request_data)
    ...
    def handle_llm_error(self, e: Exception, llm_config: Optional[LLMConfig] = None) -> Exception:
        ...
        # trigger dodo's context window compaction / retry logic.
        #
        # Example message:
