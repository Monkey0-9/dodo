...
# Define constants here to avoid circular import with dodo.log
...
class SummarizerSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="dodo_summarizer_", extra="ignore")
    ...
    # The amount of tokens before a system warning about upcoming truncation is sent to dodo
...
class ModelSettings(BaseSettings):
    ...
    auto_mode_enabled: bool = Field(
        default=False,
        description="Enable auto mode model handles (i.e. dodo/auto, dodo/auto-fast) in model listing",
    )
...
if "--use-file-pg-uri" in sys.argv:
    try:
        with open(Path.home() / ".dodo/pg_uri", "r") as f:
            ...
            print(f"Read pg_uri from ~/.dodo/pg_uri: {default_pg_uri}")
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="dodo_", extra="ignore")
    ...
    # Example:
    #   dodo_OBJECT_STORE_URI="gs://my-bucket/repository?project=my-gcp-project"
    object_store_uri: str | None = Field(
        default=None,
        validation_alias=AliasChoices("dodo_OBJECT_STORE_URI"),
    ...
    object_store_project: str | None = Field(
        default=None,
        validation_alias=AliasChoices("dodo_OBJECT_STORE_PROJECT"),
    ...
    memfs_service_url: str | None = Field(
        default=None,
        validation_alias=AliasChoices("dodo_MEMFS_SERVICE_URL"),
    ...
    clickhouse_endpoint: str | None = Field(
        default=None,
        validation_alias=AliasChoices("CLICKHOUSE_ENDPOINT", "dodo_clickhouse_endpoint"),
        description="ClickHouse endpoint URL",
    )
    clickhouse_database: str | None = Field(
        default="otel",
        validation_alias=AliasChoices("CLICKHOUSE_DATABASE", "dodo_clickhouse_database"),
        description="ClickHouse database name",
    )
    clickhouse_username: str | None = Field(
        default="default",
        validation_alias=AliasChoices("CLICKHOUSE_USERNAME", "dodo_clickhouse_username"),
        description="ClickHouse username",
    )
    clickhouse_password: str | None = Field(
        default=None,
        validation_alias=AliasChoices("CLICKHOUSE_PASSWORD", "dodo_clickhouse_password"),
    ...
    # dodo client settings for tool execution
    default_base_url: str = Field(default="http://localhost:8283", description="Default base URL for dodo client in tool execution")
    default_token: Optional[str] = Field(default=None, description="Default token for dodo client in tool execution")

    # enabling dodo_agent_v1 architecture
    use_dodo_v1_agent: bool = False
    ...
    def dodo_pg_uri(self) -> str:
        if self.pg_uri:
            ...
            return "postgresql+pg8000://dodo:dodo@localhost:5432/dodo"

    # add this property to avoid being returned the default
    # reference: https://github.com/dodo-ai/dodo/issues/1362
    @property
    def dodo_pg_uri_no_default(self) -> str:
        if self.pg_uri:
            return self.pg_uri
        elif self.pg_db and self.pg_user and self.pg_password and self.pg_host and self.pg_port:
            return f"postgresql+pg8000://{self.pg_user}:{self.pg_password}@{self.pg_host}:{self.pg_port}/{self.pg_db}"
    ...
    def database_engine(self) -> DatabaseChoice:
        return DatabaseChoice.POSTGRES if self.dodo_pg_uri_no_default else DatabaseChoice.SQLITE
...
class TestSettings(Settings):
    model_config = SettingsConfigDict(env_prefix="dodo_test_", extra="ignore")

    dodo_dir: Path | None = Field(Path.home() / ".dodo/test", alias="dodo_TEST_DIR")


class LogSettings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="dodo_logging_", extra="ignore")
    ...
    dodo_log_path: Path | None = Field(Path.home() / ".dodo" / "logs" / "dodo.log")
...
class TelemetrySettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="dodo_telemetry_", extra="ignore")
    ...
    datadog_service_name: str = Field(default="dodo-server", description="Service name for Datadog profiling.")
    ...
    source: str | None = Field(
        default=None,
        description="Source identifier for telemetry (dodo-server, lettuce-py, etc.).",
    )
...
class ReadinessSettings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="dodo_readiness_", extra="ignore")

    # Master kill switch â€” enables health probe integration with readiness_state module.
