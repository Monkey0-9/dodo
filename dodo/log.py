...
class JSONFormatter(logging.Formatter):
    ...
    Usage:
        Enable JSON logging by setting the environment variable:
            dodo_LOGGING_JSON_LOGGING=true
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Add OpenTelemetry trace correlation (for OTEL â†’ Datadog integration)
        try:
...
class LogContextFilter(logging.Filter):
    ...
    Usage:
        from dodo.log_context import set_log_context, update_log_context
...
def _setup_logfile() -> "Path":
    ...
    """
    logfile = Path(settings.dodo_dir / "logs" / "dodo.log")
...
def get_logger(name: Optional[str] = None) -> "logging.Logger":
    ...
    """
    parent_logger = logging.getLogger("dodo")
    if name:
