...
from dodo.agents.helpers import generate_step_id
...
from dodo.functions.composio_helpers import execute_composio_action, generate_composio_action_from_func_name
from dodo.functions.functions import get_function_from_module
from dodo.helpers import ToolRulesSolver
from dodo.helpers.composio_helpers import get_composio_api_key
from dodo.helpers.datetime_helpers import get_utc_time
from dodo.helpers.json_helpers import json_dumps, json_loads
from dodo.helpers.message_helper import convert_message_creates_to_messages
from dodo.interface import AgentInterface
from dodo.llm_api.helpers import calculate_summarizer_cutoff, get_token_counts_for_messages, is_context_overflow_error
from dodo.llm_api.llm_api_tools import create
from dodo.llm_api.llm_client import LLMClient
from dodo.local_llm.constants import INNER_THOUGHTS_KWARG
from dodo.local_llm.utils import num_tokens_from_functions, num_tokens_from_messages
from dodo.log import get_logger
from dodo.memory import summarize_messages
...
from dodo.schemas.response_format import ResponseFormatType
from dodo.schemas.tool import Tool
from dodo.schemas.tool_execution_result import ToolExecutionResult
from dodo.schemas.tool_rule import TerminalToolRule
...
from dodo.services.helpers.agent_manager_helper import check_supports_structured_output
...
from dodo.services.telemetry_manager import NoopTelemetryManager, TelemetryManager
from dodo.services.tool_executor.tool_execution_sandbox import ToolExecutionSandbox
from dodo.services.tool_manager import ToolManager
from dodo.settings import model_settings, settings, summarizer_settings
from dodo.streaming_interface import StreamingRefreshCLIInterface
from dodo.system import get_heartbeat, get_token_limit_warning, package_function_response, package_summarize_message, package_user_message
from dodo.utils import count_tokens, get_friendly_error_msg, get_tool_call_id, log_telemetry, parse_json, validate_function_response
...
class Agent(BaseAgent):
    ...
    def _handle_ai_response(
        ...
        if response_message.function_call or (response_message.tool_calls is not None and len(response_message.tool_calls) > 0):
            ...
            target_dodo_tool = None
            for t in self.agent_state.tools:
                if t.name == function_name:
                    # This force refreshes the target_dodo_tool from the database
                    # We only do this on name match to confirm that the agent state contains a specific tool with the right name
                    target_dodo_tool = ToolManager().get_tool_by_name(tool_name=function_name, actor=self.user)
                    break

            if not target_dodo_tool:
            ...
            try:
                ...
                log_event(
                    ...
                    attributes={
                        ...
                        "target_dodo_tool": target_dodo_tool.model_dump(),
                        **{f"function_args.{k}": v for k, v in function_args.items()},
                    },
                )

                tool_execution_result = self.execute_tool_and_persist_state(function_name, function_args, target_dodo_tool)
                ...
                return_char_limit = target_dodo_tool.return_char_limit
    ...
    def step(
        ...
        **kwargs,
        ...
        return dodoUsageStatistics(**total_usage.model_dump(), step_count=step_count, steps_messages=steps_messages)
    ...
    def inner_step(
        ...
        try:
            ...
            if self.agent_state.llm_config.context_window is None:
                ...
                logger.info(f"{CLI_WARNING_PREFIX}could not find context_window in config, setting to default {LLM_MAX_TOKENS['DEFAULT']}")
                logger.info(f"{self.agent_state}")
    ...
    def execute_tool_and_persist_state(self, function_name: str, function_args: dict, target_dodo_tool: Tool) -> ToolExecutionResult:
        """
        ...
        try:
            if target_dodo_tool.tool_type == ToolType.dodo_CORE:
                # base tools are allowed to access the `Agent` object and run on the database
                callable_func = get_function_from_module(dodo_CORE_TOOL_MODULE_NAME, function_name)
                function_args["self"] = self  # need to attach self to arg since it's dynamically linked
                function_response = callable_func(**function_args)
            elif target_dodo_tool.tool_type == ToolType.dodo_MULTI_AGENT_CORE:
                callable_func = get_function_from_module(dodo_MULTI_AGENT_TOOL_MODULE_NAME, function_name)
                function_args["self"] = self  # need to attach self to arg since it's dynamically linked
                function_response = callable_func(**function_args)
            elif target_dodo_tool.tool_type == ToolType.dodo_MEMORY_CORE or target_dodo_tool.tool_type == ToolType.dodo_SLEEPTIME_CORE:
                callable_func = get_function_from_module(dodo_CORE_TOOL_MODULE_NAME, function_name)
            ...
            elif target_dodo_tool.tool_type == ToolType.EXTERNAL_COMPOSIO:
                action_name = generate_composio_action_from_func_name(target_dodo_tool.name)
            ...
            elif target_dodo_tool.tool_type == ToolType.EXTERNAL_MCP:
                # Get the server name from the tool tag
                # TODO make a property instead?
                server_name = target_dodo_tool.tags[0].split(":")[1]
                ...
                if function_name not in available_tool_names:
                    raise ValueError(
                        f"{function_name} is not available in MCP server {server_name}. Please check your `~/.dodo/mcp_config.json` file."
                    )
                ...
                try:
                    ...
                    annotations = get_function_annotations_from_source(target_dodo_tool.source_code, function_name)
                ...
                tool_execution_result = ToolExecutionSandbox(function_name, function_args, self.user, tool_object=target_dodo_tool).run(
...
def strip_name_field_from_user_message(user_message_text: str) -> Tuple[str, Optional[str]]:
    ...
    try:
        ...
        logger.info(f"{CLI_WARNING_PREFIX}handling of 'name' field failed with: {e}")
        raise e
...
def validate_json(user_message_text: str) -> str:
    ...
    try:
        ...
        logger.info(f"{CLI_WARNING_PREFIX}couldn't parse user input message as JSON: {e}")
        raise e
