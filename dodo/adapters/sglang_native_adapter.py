...
def _messages_to_input_ids(model_name: str, messages: list, tools: list) -> list[int]:
    ...
    Uses apply_chat_template(tokenize=True) â€” single step, correct template,
...
def _to_openai_messages(messages: list) -> list[dict]:
    """Convert dodo Message objects to OpenAI-style dicts."""
    result = []
    for msg in messages:
        if hasattr(msg, "role"):
            role = msg.role
...
class SGLangNativeAdapter(SimpleLLMRequestAdapter):
    ...
    Flow:
    1. Apply model's chat template via HF tokenizer â†’ input_ids (single tokenization step)
    2. POST input_ids to SGLang /generate â†’ output_ids + output_token_logprobs
    ...
    async def invoke_llm(
        ...
        actor: str | None = None,
    ) -> AsyncGenerator[dodoMessage | None, None]:
        ...
        # Tokenize via HF apply_chat_template â€” correct template, no double tokenization
