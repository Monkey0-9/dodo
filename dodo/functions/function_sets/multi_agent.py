...
def _get_sandbox_client() -> Any:
    ...
    api_key = getenv("dodo_API_KEY")
    base_url = getenv("dodo_SERVER_URL") or "http://localhost:8283"

    if not api_key:
        return dodo(base_url=base_url)

    try:
        return dodo(base_url=base_url, api_key=api_key)
    except TypeError:
        return dodo(base_url=base_url, token=api_key)


def _get_sender_agent_id() -> str:
    from os import getenv

    sender_agent_id = getenv("dodo_AGENT_ID")
    if not sender_agent_id:
        raise RuntimeError("dodo_AGENT_ID not set in tool execution environment")
...
def send_message_to_agent_and_wait_for_reply(message: str, other_agent_id: str) -> str:
    """
    Sends a message to a specific dodo agent within the same organization and waits for a response. The sender's identity is automatically included, so no explicit introduction is needed in the message. This function is designed for two-way communication where a reply is expected.

    Args:
        message (str): The content of the message to be sent to the target agent.
        other_agent_id (str): The unique identifier of the target dodo agent.
...
def send_message_to_agent_async(message: str, other_agent_id: str) -> str:
    """
    Sends a message to a specific dodo agent within the same organization. The sender's identity is automatically included, so no explicit introduction is required in the message. This function does not expect a response from the target agent, making it suitable for notifications or one-way communication.
    Args:
        message (str): The content of the message to be sent to the target agent.
        other_agent_id (str): The unique identifier of the target dodo agent.
    Returns:
    ...
    if settings.environment == "prod":
        raise RuntimeError("This tool is not allowed to be run on dodo Cloud.")
