...
class MemoryChunk(BaseModel):
    ...
    context: str = Field(
        ...,
        description="1-3 sentence paraphrase capturing key facts/details, user preferences, or goals that this chunk revealsâ€”written for future retrieval.",
    )


def store_memories(agent_state: "AgentState", chunks: List[MemoryChunk]) -> None:
    """
    Persist dialogue that is about to fall out of the agentâ€™s context window.
...
def search_memory(
    ...
    """
    Look in long-term or earlier-conversation memory only when the user asks about something missing from the visible context. The userâ€™s latest utterance is sent automatically as the main query.
