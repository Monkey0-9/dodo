...
def derive_typescript_json_schema(source_code: str, name: Optional[str] = None) -> dict:
    ...
    Raises:
        dodoToolCreateError: If parsing fails or no exported function is found
    """
    try:
        ...
        if not match:
            raise dodoToolCreateError("No exported function found in TypeScript source code")
        ...
        raise dodoToolCreateError(f"TypeScript schema generation failed: {str(e)}") from e
