...
from dodo.schemas.response_format import (
    ...
    for content in message_content:
        if isinstance(content, MessageContent):
            if content.type == MessageContentType.image:
                assert content.source.type == ImageSourceType.dodo, f"Invalid image source type: {content.source.type}"
...
def deserialize_message_content(data: Optional[List[Dict]]) -> List[MessageContent]:
    ...
    for item in data:
        ...
        if content_type == MessageContentType.text:
            ...
            assert item["source"]["type"] == ImageSourceType.dodo, f"Invalid image source type: {item['source']['type']}"
