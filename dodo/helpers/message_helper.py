...
async def _fetch_image_from_url(url: str, max_retries: int = 1, timeout_seconds: float = 5.0) -> tuple[bytes, str | None]:
    ...
    Raises:
        dodoImageFetchError: If image fetch fails after all retries
    """
    ...
    headers = {"User-Agent": f"dodo/{__version__}"}
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            raise dodoImageFetchError(url=url, reason=f"Timeout after {max_retries + 1} attempts: {e}")
        except (httpx.RemoteProtocolError, httpx.HTTPStatusError) as e:
            # Don't retry on protocol errors or HTTP errors (4xx, 5xx)
            raise dodoImageFetchError(url=url, reason=str(e))
        except Exception as e:
            raise dodoImageFetchError(url=url, reason=f"Unexpected error: {e}")

    # Should never reach here, but just in case
    raise dodoImageFetchError(url=url, reason=f"Failed after {max_retries + 1} attempts: {last_exception}")
...
def _parse_data_image_url(url: str) -> tuple[str, str]:
    ...
    try:
        ...
        raise dodoImageFetchError(url=url[:100] + "...", reason="Invalid data URL format") from exc
    ...
    for content in message_content:
        if isinstance(content, TextContent):
            ...
            if content.source.type == ImageSourceType.url:
                ...
                content.source = Base64Image(media_type=image_media_type, data=image_data)
            if content.source.type == ImageSourceType.dodo and not content.source.data:
                # TODO: hydrate dodo image with data from db
                pass
...
async def resolve_tool_return_images(func_response: str | list) -> str | list:
    """Resolve URL and dodoImage sources to base64 for tool returns."""
    ...
    for part in func_response:
        if isinstance(part, ImageContent):
            ...
            elif part.source.type == ImageSourceType.dodo and not part.source.data:
                pass
