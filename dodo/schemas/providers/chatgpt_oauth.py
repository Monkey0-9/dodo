...
from dodo.errors import ErrorCode, LLMAuthenticationError, LLMError
...
# This is the public client_id used by Codex CLI / dodo Code
...
class ChatGPTOAuthProvider(Provider):
    ...
    The client (e.g., dodo Code) performs the OAuth flow and sends the credentials
    ...
    async def _update_stored_credentials(self, creds: ChatGPTOAuthCredentials, actor: Optional["User"] = None) -> None:
        ...
        if actor and self.id:
            try:
                from dodo.schemas.providers.base import ProviderUpdate
