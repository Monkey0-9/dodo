...
class FileMetadataBase(dodoBase):
...
class FileAgentBase(dodoBase):
    """Base class for the FileMetadata-â‡„-Agent association schemas"""
...
class FileAgent(FileAgentBase):
    """
    A single FileMetadata â‡„ Agent association row.

    Captures:
    â€¢ whether the agent currently has the file â€œopenâ€
    â€¢ the excerpt (grepped section) in the context window
    â€¢ the last time the agent accessed the file
    """
...
class AgentFileAttachment(dodoBase):
...
class PaginatedAgentFiles(dodoBase):
