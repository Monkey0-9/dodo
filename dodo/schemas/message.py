...
from dodo.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, REQUEST_HEARTBEAT_PARAM, TOOL_CALL_ID_MAX_LEN
from dodo.helpers.datetime_helpers import get_utc_time, is_utc_datetime
...
class MessageCreate(MessageCreateBase):
    ...
    content: Union[str, List[dodoMessageContentUnion]] = Field(
        ...,
        description="The content of the message.",
        json_schema_extra=get_dodo_message_content_union_str_json_schema(),
    )
...
class ToolReturnCreate(MessageCreateBase):
    ...
    tool_returns: List[dodoToolReturn] = Field(
        ...,
...
class MessageUpdate(BaseModel):
    ...
    content: Optional[Union[str, List[dodoMessageContentUnion]]] = Field(
        default=None,
        description="The content of the message.",
        json_schema_extra=get_dodo_message_content_union_str_json_schema(),
    )
...
class Message(BaseMessage):
    """
        dodo's internal representation of a message. Includes methods to convert to/from LLM provider formats.
    ...
    content: Optional[List[dodoMessageContentUnion]] = Field(default=None, description="The content of the message.")
    ...
    @staticmethod
    @trace_method
    def to_dodo_messages_from_list(
        ...
        # Convert messages to dodoMessages
        dodo_messages = [
            msg
            for m in messages
            for msg in m.to_dodo_messages(
        ...
        if include_return_message_types is not None:
            ...
            dodo_messages = [msg for msg in dodo_messages if msg.message_type in include_return_message_types]

        return dodo_messages

    @staticmethod
    @trace_method
    def to_dodo_search_results_from_list(
        ...
        text_is_assistant_message: bool = False,
        convert_summary_to_user: bool = True,
    ) -> List[dodoMessageSearchResult]:
        """Convert MessageSearchResult objects into dodoMessageSearchResult objects.

        This mirrors the behavior of to_dodo_messages_from_list, but preserves the
        originating Message.agent_id on each search result variant.
        """

        dodo_search_results: List[dodoMessageSearchResult] = []

        for result in search_results:
            message = result.message

            # Convert the underlying Message into dodoMessage variants
            dodo_messages = message.to_dodo_messages(
                use_assistant_message=use_assistant_message,
                assistant_message_tool_name=assistant_message_tool_name,
                assistant_message_tool_kwarg=assistant_message_tool_kwarg,
                reverse=reverse,
            ...
            for lm in dodo_messages:
                if isinstance(lm, SystemMessage):
                    dodo_search_results.append(
                        SystemMessageListResult(
                ...
                # Other dodoMessage variants (tool, approval, etc.) are not part of
                # dodoMessageSearchResult and are intentionally skipped here.

        return dodo_search_results

    def to_dodo_messages(
        self,
        ...
        """Convert message object (in DB format) to the style used by the original dodo API
    ...
    def _convert_explicit_tool_returns(self) -> ToolReturnMessage:
        ...
        for tool_return in self.tool_returns:
            ...
            tool_return_obj = dodoToolReturn(
    ...
    def dict_to_message(
        ...
        content: List[dodoMessageContentUnion] = (
        ...
        for m in messages:
            ...
            if m.role == MessageRole.tool and m.tool_returns and len(m.tool_returns) > 0:
                for tr in m.tool_returns:
                    ...
                    # Convert multi-modal to text (images â†’ placeholders), then truncate
    ...
    def _image_source_to_data_url(image_content: ImageContent) -> Optional[str]:
        ...
        if source.type == ImageSourceType.dodo:
            data = getattr(source, "data", None)
            if not data:
                return None
            media_type = getattr(source, "media_type", None) or "image/png"
    ...
    def _get_base64_image_data(part: Union[ImageContent, dict]) -> Optional[tuple[str, str]]:
        ...
        if isinstance(part, ImageContent):
            ...
            elif source.type == ImageSourceType.dodo and getattr(source, "data", None):
    ...
    def to_anthropic_dict(
        ...
        if self.role == "system":
            ...
            if system_text is None:
                text_parts = [c.text for c in (self.content or []) if isinstance(c, TextContent)]
                if not text_parts:
                    from dodo.log import get_logger as _get_logger
...
class ToolReturn(BaseModel):
    ...
    func_response: Optional[Union[str, List[dodoToolReturnContentUnion]]] = Field(
