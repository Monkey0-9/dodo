...
from dodo.functions.functions import get_json_schema_from_module
...
class Tool(BaseTool):
    ...
    def refresh_source_code_and_json_schema(self):
        ...
        This method only handles built-in dodo tools.
        """
        ...
        elif self.tool_type in {ToolType.dodo_CORE, ToolType.dodo_MEMORY_CORE, ToolType.dodo_SLEEPTIME_CORE}:
            # If it's dodo core tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=dodo_CORE_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.dodo_MULTI_AGENT_CORE}:
            # If it's dodo multi-agent tool, we also generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=dodo_MULTI_AGENT_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.dodo_VOICE_SLEEPTIME_CORE}:
            # If it's dodo voice tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=dodo_VOICE_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.dodo_BUILTIN}:
            # If it's dodo voice tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=dodo_BUILTIN_TOOL_MODULE_NAME, function_name=self.name)
        elif self.tool_type in {ToolType.dodo_FILES_CORE}:
            # If it's dodo files tool, we generate the json_schema on the fly here
            self.json_schema = get_json_schema_from_module(module_name=dodo_FILES_TOOL_MODULE_NAME, function_name=self.name)

        return self


class ToolCreate(dodoBase):
    description: Optional[str] = Field(None, description="The description of the tool.")
    ...
    def from_mcp(cls, mcp_server_name: str, mcp_tool: MCPTool) -> "ToolCreate":
        from dodo.functions.helpers import generate_mcp_tool_wrapper
    ...
    def model_dump(self, to_orm: bool = False, **kwargs):
        """
        Override dodoBase.model_dump to explicitly handle 'tags' being None,
...
class ToolRunFromSource(dodoBase):
    source_code: str = Field(..., description="The source code of the function.")
...
class ToolSearchRequest(dodoBase):
    ...
    tool_types: Optional[List[str]] = Field(None, description="Filter by tool types (e.g., 'custom', 'dodo_core').")
...
class ToolSearchResult(dodoBase):
