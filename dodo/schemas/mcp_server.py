...
class CreateStdioMCPServer(dodoBase):
...
class CreateSSEMCPServer(dodoBase):
...
class CreateStreamableHTTPMCPServer(dodoBase):
...
class UpdateStdioMCPServer(dodoBase):
...
class UpdateSSEMCPServer(dodoBase):
...
class UpdateStreamableHTTPMCPServer(dodoBase):
...
class MCPServerResyncResult(dodoBase):
...
class ToolExecuteRequest(dodoBase):
...
class CreateMCPServerRequest(dodoBase):
...
class UpdateMCPServerRequest(dodoBase):
...
def convert_update_to_internal(request: UpdateMCPServerRequest):
    ...
    3. Maps auth_token â†’ token for SSE (internal uses 'token')
    4. Passes through auth_header + auth_token for StreamableHTTP
    5. Strips 'Bearer ' prefix from tokens if present
    """
    # Local import to avoid circulars
    from dodo.functions.mcp_client.types import MCPServerType as MCPType, StdioServerConfig as StdioCfg
    ...
    if isinstance(config, UpdateStdioMCPServer):
        ...
        # For SSE: map auth_token â†’ token, strip Bearer prefix if present
        token_value = None
