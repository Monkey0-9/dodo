...
class dodoResponse(BaseModel):
    """
    Response object from an agent interaction, consisting of the new messages generated by the agent and usage statistics.
    The type of the returned messages can be either `Message` or `dodoMessage`, depending on what was specified in the request.

    Attributes:
        messages (List[Union[Message, dodoMessage]]): The messages returned by the agent.
        usage (dodoUsageStatistics): The usage statistics
    """

    messages: List[dodoMessageUnion] = Field(
        ...,
        description="The messages returned by the agent.",
        json_schema_extra={
            "items": {
                "$ref": "#/components/schemas/dodoMessageUnion",
            }
        },
    )
    stop_reason: dodoStopReason = Field(
        ...,
        description="The stop reason from dodo indicating why agent loop stopped execution.",
    )
    usage: dodoUsageStatistics = Field(
        ...,
        ...
        return json_dumps(
            {
                ...
                # Assume `Message` and `dodoMessage` have a `dict()` method
                "usage": self.usage.model_dump(),  # Assume `dodoUsageStatistics` has a `dict()` method
            },
            indent=4,
        )
...
class dodoStreamingResponse(RootModel):
    """
...
class dodoBatchResponse(BaseModel):
    dodo_batch_id: str = Field(..., description="A unique identifier for the dodo batch request.")
...
class dodoBatchMessages(BaseModel):
