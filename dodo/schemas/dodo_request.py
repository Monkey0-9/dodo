...
from dodo.constants import DEFAULT_MAX_STEPS, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG
from dodo.schemas.dodo_message import MessageType
from dodo.schemas.dodo_message_content import dodoMessageContentUnion
from dodo.schemas.message import MessageCreate, MessageCreateUnion, MessageRole
from dodo.validators import AgentId
...
class dodoRequest(BaseModel):
    ...
    use_assistant_message: bool = Field(
        default=True,
        description="Whether the server should parse specific tool call arguments (default `send_message`) as `AssistantMessage` objects. Still supported for legacy agent types, but deprecated for dodo_v1_agent onward.",
        deprecated=True,
    )
    assistant_message_tool_name: str = Field(
        default=DEFAULT_MESSAGE_TOOL,
        description="The name of the designated message tool. Still supported for legacy agent types, but deprecated for dodo_v1_agent onward.",
        deprecated=True,
    )
    assistant_message_tool_kwarg: str = Field(
        default=DEFAULT_MESSAGE_TOOL_KWARG,
        description="The name of the message argument in the designated message tool. Still supported for legacy agent types, but deprecated for dodo_v1_agent onward.",
        deprecated=True,
    )
    ...
    def validate_input_or_messages(self):
        ...
        # input can be either a string or List[dodoMessageContentUnion]
        if self.input is not None:
            # Both str and List[dodoMessageContentUnion] are valid content types for MessageCreate
            self.messages = [MessageCreate(role=MessageRole.user, content=self.input)]

        return self


class dodoStreamingRequest(dodoRequest):
...
class ConversationMessageRequest(dodoRequest):
