...
from dodo.utils import calculate_file_defaults_based_on_context_window, create_random_username
from dodo.validators import BlockId, IdentityId, MessageId, SourceId, ToolId
...
class CreateAgent(BaseModel, validate_assignment=True):  #
    ...
    agent_type: AgentType = Field(default_factory=lambda: AgentType.dodo_v1_agent, description="The type of agent.")
    ...
    include_base_tools: bool = Field(True, description="If true, attaches the dodo core tools (e.g. core_memory related functions).")
    include_multi_agent_tools: bool = Field(
        False, description="If true, attaches the dodo multi-agent tools (e.g. sending a message to another agent)."
    )
    include_base_tool_rules: Optional[bool] = Field(
        None, description="If true, attaches the dodo base tool rules (e.g. deny all tools not explicitly allowed)."
    )
    ...
    def validate_model(cls, model: Optional[str]) -> Optional[str]:
        ...
        if "/" not in model:
            raise dodoInvalidArgumentError("The model handle should be in the format provider/model-name", argument_name="model")
