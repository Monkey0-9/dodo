...
from dodo.schemas.dodo_message_content import (
    dodoAssistantMessageContentUnion,
    dodoToolReturnContentUnion,
    dodoUserMessageContentUnion,
    get_dodo_assistant_message_content_union_str_json_schema,
    get_dodo_tool_return_content_union_str_json_schema,
    get_dodo_user_message_content_union_str_json_schema,
)

# ---------------------------
# dodo API Messaging Schemas
# ---------------------------
...
class ToolReturn(MessageReturn):
    ...
    tool_return: Union[str, List[dodoToolReturnContentUnion]] = Field(
        ...,
        description="The tool return value - either a string or list of content parts (text/image)",
        json_schema_extra=get_dodo_tool_return_content_union_str_json_schema(),
    )
...
dodoMessageReturnUnion = Annotated[Union[ApprovalReturn, ToolReturn], Field(discriminator="type")]
...
class dodoMessage(BaseModel):
    """
    Base class for simplified dodo message response type. This is intended to be used for developers
    ...
    Args:
        ...
        otid (Optional[str]): The offline threading id (OTID) associated with this message. Set by the client to deduplicate
            requests. This key is used for idempotency in background mode for streaming â€” it is critical that each message
    ...
    otid: str | None = Field(
        ...
        "Used for idempotency in background streaming mode â€” each message in a request must have a unique OTID. "
...
class SystemMessage(dodoMessage):
    """
...
class UserMessage(dodoMessage):
    ...
    Args:
        ...
        date (datetime): The date the message was created in ISO format
        name (Optional[str]): The name of the sender of the message
        content (Union[str, List[dodoUserMessageContentUnion]]): The message content sent by the user (can be a string or an array of multi-modal content parts)
    """

    message_type: Literal[MessageType.user_message] = Field(default=MessageType.user_message, description="The type of the message.")
    content: Union[str, List[dodoUserMessageContentUnion]] = Field(
        ...,
        description="The message content sent by the user (can be a string or an array of multi-modal content parts)",
        json_schema_extra=get_dodo_user_message_content_union_str_json_schema(),
    )


class ReasoningMessage(dodoMessage):
    """
...
class HiddenReasoningMessage(dodoMessage):
    """
...
class ToolCallMessage(dodoMessage):
    """
...
class ToolReturnMessage(dodoMessage):
    """
    A message representing the return value of a tool call (generated by dodo executing the requested tool).
...
class ApprovalRequestMessage(dodoMessage):
    """
...
class ApprovalResponseMessage(dodoMessage):
    """
    ...
    approvals: Optional[List[dodoMessageReturnUnion]] = Field(default=None, description="The list of approval responses")
...
class AssistantMessage(dodoMessage):
    """
    ...
    Args:
        ...
        date (datetime): The date the message was created in ISO format
        name (Optional[str]): The name of the sender of the message
        content (Union[str, List[dodoAssistantMessageContentUnion]]): The message content sent by the agent (can be a string or an array of content parts)
    """
    ...
    content: Union[str, List[dodoAssistantMessageContentUnion]] = Field(
        ...,
        description="The message content sent by the agent (can be a string or an array of content parts)",
        json_schema_extra=get_dodo_assistant_message_content_union_str_json_schema(),
    )


class dodoPing(dodoMessage):
    """
...
class dodoErrorMessage(BaseModel):
    """
...
class SummaryMessage(dodoMessage):
    """
...
class EventMessage(dodoMessage):
    """
...
def create_dodo_message_union_schema():
    return {
        "oneOf": [
...
def create_dodo_error_message_schema():
    return {
        "properties": {
            "message_type": {
                "type": "string",
...
dodoMessageUpdateUnion = Annotated[
...
dodoMessageSearchResult = Annotated[
    Union[
...
class LegacyFunctionCallMessage(dodoMessage):
    function_call: str


class LegacyFunctionReturn(dodoMessage):
    """
    A message representing the return value of a function call (generated by dodo executing the requested function).
...
class LegacyInternalMonologue(dodoMessage):
    """
...
LegacydodoMessage = Union[LegacyInternalMonologue, AssistantMessage, LegacyFunctionCallMessage, LegacyFunctionReturn]
