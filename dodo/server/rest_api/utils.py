...
from dodo.errors import ContextWindowExceededError, RateLimitExceededError
from dodo.helpers.datetime_helpers import get_utc_time, get_utc_timestamp_ns, ns_to_ms
from dodo.helpers.message_helper import convert_message_creates_to_messages, resolve_tool_return_images
...
from dodo.system import get_heartbeat, package_function_response
    ...
    try:
        ...
        if usage_task is not None:
            try:
                ...
                if not isinstance(usage, dodoUsageStatistics):
                    err_msg = f"Expected dodoUsageStatistics, got {type(usage)}"
...
async def create_approval_response_message_from_input(
    ...
    async def maybe_convert_tool_return_message(maybe_tool_return: dodoToolReturn):
        if isinstance(maybe_tool_return, dodoToolReturn):
...
def create_dodo_messages_from_llm_response(
    agent_id: str,
    model: str,
...
def create_assistant_messages_from_openai_response(
    ...
    return create_dodo_messages_from_llm_response(
        agent_id=agent_id,
        model=model,
...
def convert_in_context_dodo_messages_to_openai(in_context_messages: List[Message], exclude_system_messages: bool = False) -> List[dict]:
    """
    Flattens dodo's messages (with system, user, assistant, tool roles, etc.)
