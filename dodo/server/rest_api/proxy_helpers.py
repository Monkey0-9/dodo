...
from dodo.server.rest_api.utils import capture_and_persist_messages
...
def prepare_headers(request: Request, proxy_name: str, use_bearer_auth: bool = False) -> dict | None:
    ...
    # Extract API key from headers or fallback to dodo's key
    api_key = None
    if "x-api-key" in headers:
        ...
        # Fallback to dodo's anthropic api key if not provided
        api_key = model_settings.anthropic_api_key
        if api_key:
            logger.info(f"[{proxy_name}] Falling back to dodo's anthropic api key instead of user's key")
...
def format_memory_blocks(blocks, agent_id: str) -> str:
    ...
    memory_context = (
        "<dodo>\n"
        "You have persistent memory powered by dodo that is maintained across conversations. "
    ...
    memory_context += (
        ...
        f"https://app.dodo.com/agents/{agent_id}\n\n"
        ...
        "</memory_management>\n\n"
        "<documentation>\n"
        "- Memory blocks: https://docs.dodo.com/guides/agents/memory-blocks/index.md\n"
        "- Full dodo documentation: https://docs.dodo.com/llms.txt\n\n"
        "Reference these when users ask how dodo memory works or want to learn more about the platform.\n"
        "</documentation>\n"
        "</dodo>"
    )
...
async def get_or_create_claude_code_agent(
    ...
    Args:
        ...
        agent_id: Optional specific agent ID to use (from X-dodo-AGENT-ID header)
    ...
    if agent_id:
        ...
        try:
            ...
            logger.info(f"Found agent via X-dodo-AGENT-ID header: {agent.id} (name: {agent.name})")
            return agent
        except Exception as e:
