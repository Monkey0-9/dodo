    ...
    try:
        ...
        if not saw_done and not saw_error:
            ...
            if stop_reason:
                ...
                await writer.write_chunk(
                    run_id=run_id,
                    data=f"data: {dodoStopReason(stop_reason=StopReasonType.error).model_dump_json()}\n\n",
                    is_complete=False,
                )
                await writer.write_chunk(
        ...
        if run_was_explicitly_cancelled:
            ...
            try:
                await writer.write_chunk(
                    run_id=run_id,
                    data=f"data: {dodoStopReason(stop_reason=StopReasonType.cancelled).model_dump_json()}\n\n",
                    is_complete=False,
                )
                await writer.write_chunk(run_id=run_id, data="data: [DONE]\n\n", is_complete=True)
        ...
        error_message = dodoErrorMessage(
            ...
            message=str(e) if isinstance(e, dodoError) else "An unknown error occurred with the LLM streaming request.",
            detail=str(e),
        )
        await writer.write_chunk(
            run_id=run_id, data=f"data: {dodoStopReason(stop_reason=stop_reason).model_dump_json()}\n\n", is_complete=False
        )
        ...
        # the next request â€” they touch different rows/fields.
        if conversation_id:
            try:
