...
from dodo.__init__ import __version__ as dodo_version
from dodo.agents.exceptions import IncompatibleAgentType
from dodo.constants import ADMIN_PREFIX, API_PREFIX
...
from dodo.jobs.scheduler import start_scheduler_with_leader_election
from dodo.log import get_logger
from dodo.orm.errors import (
...
from dodo.otel.tracing import get_trace_id
from dodo.schemas.dodo_message import create_dodo_error_message_schema, create_dodo_message_union_schema
from dodo.schemas.dodo_message_content import (
    create_dodo_assistant_message_content_union_schema,
    create_dodo_message_content_union_schema,
    create_dodo_tool_return_content_union_schema,
    create_dodo_user_message_content_union_schema,
)
from dodo.server.constants import REST_DEFAULT_PORT
...
class SafeORJSONResponse(ORJSONResponse):
    ...
    (U+D800â€“U+DFFF) which are valid in Python str but illegal in UTF-8.
...
from dodo.settings import settings, telemetry_settings
from dodo.validators import PATH_VALIDATORS, PRIMITIVE_ID_PATTERNS
...
def generate_openapi_schema(app: FastAPI):
    ...
    dodo_docs = app.openapi_schema.copy()
    dodo_docs["paths"] = {k: v for k, v in dodo_docs["paths"].items() if not k.startswith("/openai")}
    dodo_docs["info"]["title"] = "dodo API"
    dodo_docs["components"]["schemas"]["dodoMessageUnion"] = create_dodo_message_union_schema()
    dodo_docs["components"]["schemas"]["dodoMessageContentUnion"] = create_dodo_message_content_union_schema()
    dodo_docs["components"]["schemas"]["dodoAssistantMessageContentUnion"] = create_dodo_assistant_message_content_union_schema()
    dodo_docs["components"]["schemas"]["dodoToolReturnContentUnion"] = create_dodo_tool_return_content_union_schema()
    dodo_docs["components"]["schemas"]["dodoUserMessageContentUnion"] = create_dodo_user_message_content_union_schema()
    dodo_docs["components"]["schemas"]["dodoErrorMessage"] = create_dodo_error_message_schema()

    # Update the app's schema with our modified version
    app.openapi_schema = dodo_docs
...
async def lifespan(app_: FastAPI):
    ...
    """
    worker_id = os.getpid()
    from dodo.monitoring.readiness_state import initialize_readiness_state, set_readiness_state
    ...
    try:
        ...
        from dodo.monitoring.event_loop_watchdog import start_watchdog
    ...
    try:
        ...
        from dodo.otel.db_pool_monitoring import setup_pool_monitoring
        from dodo.server.db import db_registry, engine as db_engine
    ...
    try:
        from dodo.server.rest_api.routers.v1.git_http import set_server_instance
    ...
    try:
        from dodo.monitoring.event_loop_watchdog import stop_watchdog
    ...
    try:
        from dodo.jobs.scheduler import shutdown_scheduler_and_release_lock
    ...
    if not settings.disable_tracing and settings.sqlalchemy_tracing:
        try:
            from dodo.otel.sqlalchemy_instrumentation_integration import teardown_dodo_db_instrumentation

            teardown_dodo_db_instrumentation()
...
def create_application() -> "FastAPI":
    ...
    if SENTRY_ENABLED:
        sentry_sdk.init(
            ...
            environment=os.getenv("dodo_ENVIRONMENT", "undefined"),
    ...
    if telemetry_settings.enable_datadog:
        try:
            ...
            os.environ.setdefault("DD_VERSION", dodo_version)
            ...
            try:
                ...
                if not getattr(OpenAIIntegration, "_dodo_provider_patch_done", False):
    ...
    app.add_exception_handler(dodoInvalidArgumentError, _error_handler_400)
    app.add_exception_handler(dodoToolCreateError, _error_handler_400)
    app.add_exception_handler(dodoToolNameConflictError, _error_handler_400)
    ...
    app.add_exception_handler(dodoImageFetchError, _error_handler_400)
    ...
    app.add_exception_handler(dodoAgentNotFoundError, _error_handler_404_agent)
    app.add_exception_handler(dodoUserNotFoundError, _error_handler_404_user)
    ...
    # 410 Expired errors
    app.add_exception_handler(dodoExpiredError, _error_handler_410)

    # 408 Timeout errors
    app.add_exception_handler(dodoMCPTimeoutError, _error_handler_408)
    app.add_exception_handler(dodoInvalidMCPSchemaError, _error_handler_400)
    ...
    app.add_exception_handler(dodoUnsupportedFileUploadError, _error_handler_415)
    ...
    app.add_exception_handler(dodoServiceUnavailableError, _error_handler_503)
    ...
    @app.exception_handler(dodoMCPConnectionError)
    async def mcp_connection_error_handler(request: Request, exc: dodoMCPConnectionError):
        return JSONResponse(
            status_code=502,
            content={
                "error": {
    ...
    if otlp_endpoint and not settings.disable_tracing:
        ...
        from dodo.otel.tracing import setup_tracing
        ...
        if settings.sqlalchemy_tracing:
            from dodo.otel.sqlalchemy_instrumentation_integration import setup_dodo_db_instrumentation

            try:
                setup_dodo_db_instrumentation(
...
def start_server(
    ...
    if debug:
        from dodo.server.server import logger as server_logger
    ...
    if (os.getenv("LOCAL_HTTPS") == "true") or "--localhttps" in sys.argv:
        ...
        if importlib.util.find_spec("granian") is not None and settings.use_granian:
            ...
            Granian(
                target="dodo.server.rest_api.app:app",
                # factory=True,
                interface="asgi",
