...
from dodo.streaming_utils import FunctionArgumentsStreamHandler, JSONInnerThoughtsExtractor
from dodo.utils import parse_json
...
class StreamingServerInterface(AgentChunkStreamingInterface):
    ...
    async def _create_generator(self) -> AsyncGenerator[Union[dodoMessage, LegacydodoMessage, MessageStreamStatus], None]:
    ...
    def _push_to_buffer(
        ...
        assert isinstance(item, dodoMessage) or isinstance(item, LegacydodoMessage) or isinstance(item, MessageStreamStatus), (
    ...
    def _process_chunk_to_dodo_style(
        self,
                    ...
                    # If we have main_json, we should output a ToolCallMessage
                        ...
                        # output the arguments chunk as a ToolCallMessage
                            ...
                            if self.use_assistant_message and (
                                ...
                                from dodo.streaming_utils import sanitize_streamed_message_content
                                ...
                                if self.function_id_buffer:
                                    self.prev_assistant_message_id = self.function_id_buffer
                                # Do not clear function_id_buffer here â€” we may still need it
                            else:
    ...
    def _process_chunk_to_openai_style(self, chunk: ChatCompletionChunkResponse) -> Optional[dict]:
        """Chunks should look like OpenAI, but be remapped from dodo-style concepts.
    ...
    def process_chunk(
        ...
        if self.streaming_chat_completion_mode:
            ...
            processed_chunk = self._process_chunk_to_dodo_style(
                chunk=chunk,
