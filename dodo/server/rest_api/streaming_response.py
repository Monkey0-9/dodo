        ...
        while True:
            try:
                ...
                if not stream_exhausted:
                    ...
                    yield f"data: {dodoPing(id=f'ping-{uuid4()}', date=datetime.now(timezone.utc), run_id=run_id, seq_id=last_seq_id).model_dump_json()}\n\n"
                else:
...
class StreamingResponseWithStatusCode(StreamingResponse):
    ...
    async def _protected_stream_response(self, send: Send) -> None:
        ...
        # Handle client timeouts (should throw error to inform user)
            ...
            raise dodoUnexpectedStreamCancellationError("Stream was terminated due to unexpected cancellation from server")
