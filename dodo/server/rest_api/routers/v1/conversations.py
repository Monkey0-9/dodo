...
from dodo.validators import ConversationId, ConversationIdOrDefault
...
async def list_conversation_messages(
    ...
    Returns dodoMessage objects (UserMessage, AssistantMessage, etc.) for all
...
async def _send_agent_direct_message(
    ...
    if request.streaming:
        streaming_request = dodoStreamingRequest(
    ...
    from dodo.services.streaming_service import derive_request_token, enrich_conversation_busy_error
...
async def send_conversation_message(
    ...
    **Deprecated**: Passing an agent ID as conversation_id still works but will be removed.
    ...
    # Streaming mode (default)
    if request.streaming:
        # Convert to dodoStreamingRequest for StreamingService compatibility
        streaming_request = dodoStreamingRequest(
            messages=request.messages,
            streaming=True,
            stream_tokens=request.stream_tokens,
                ...
                "text/event-stream": {
                    ...
                    "schema": {
                        "oneOf": [
                            ...
                            {"$ref": "#/components/schemas/dodoPing"},
                            {"$ref": "#/components/schemas/dodoErrorMessage"},
                            {"$ref": "#/components/schemas/dodoStopReason"},
                            {"$ref": "#/components/schemas/dodoUsageStatistics"},
                        ]
                    },
                },
            },
...
async def retrieve_conversation_stream(
    ...
    **Deprecated**: Passing an agent ID as conversation_id still works but will be removed.
    ...
    # Priority 3: Fall back to active run lookup
        ...
        if not active_runs:
            raise dodoInvalidArgumentError("No active runs found for this conversation.")
        ...
        if not run.background:
            raise dodoInvalidArgumentError("Run was not created in background mode, so it cannot be retrieved.")

    # Check expiration for all paths
    if run and run.created_at < get_utc_time() - timedelta(hours=3):
        raise dodoExpiredError("Run was created more than 3 hours ago, and is now expired.")
    ...
    if settings.enable_cancellation_aware_streaming:
        from dodo.server.rest_api.streaming_response import cancellation_aware_stream_wrapper, get_cancellation_event_for_run
...
async def recompile_conversation(
    ...
    if not dry_run:
        ...
        from dodo.schemas.message import MessageUpdate
...
async def compact_conversation(
    ...
    **Deprecated**: Passing an agent ID as conversation_id still works but will be removed.
    ...
    if resolved_agent_id:
        ...
        agent_loop = dodoAgentV3(agent_state=agent, actor=actor, conversation_id=conversation_id)
