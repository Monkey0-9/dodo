...
from dodo.constants import DEFAULT_MAX_STEPS, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, REDIS_RUN_ID_PREFIX
from dodo.data_sources.redis_client import get_redis_client
...
from dodo.schemas.message import Message, MessageCreate, MessageCreateType, MessageSearchRequest, MessageSearchResult
...
from dodo.utils import is_1_0_sdk_version, safe_create_shielded_task, safe_create_task, truncate_file_visible_content
from dodo.validators import AgentId, BlockId, FileId, MessageId, SourceId, ToolId
...
async def _import_agent(
    ...
    if not import_result.success:
        from dodo.errors import AgentFileImportError
...
async def import_agent(
    ...
    override_existing_tools: bool = Form(
        True,
        description="If set to True, existing tools can get their source code overwritten by the uploaded tool definitions. Note that dodo core tools can never be updated externally.",
    ),
...
async def open_file_for_agent(
    ...
    from dodo.services.file_processor.chunker.line_chunker import LineChunker
...
AgentMessagesResponse = Annotated[
    list[dodoMessageUnion], Field(json_schema_extra={"type": "array", "items": {"$ref": "#/components/schemas/dodoMessageUnion"}})
]
...
@router.patch("/{agent_id}/messages/{message_id}", response_model=dodoMessageUnion, operation_id="modify_message", deprecated=True)
async def modify_message(
    agent_id: AgentId,  # backwards compatible. Consider removing for v1
    message_id: MessageId,
    request: dodoMessageUpdateUnion = Body(...),
    server: "SyncServer" = Depends(get_dodo_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
...
@router.post(
    "/{agent_id}/messages",
    response_model=dodoResponse,
    operation_id="send_message",
    responses={
        200: {
            "description": "Successful response",
            "content": {
                "application/json": {"schema": {"$ref": "#/components/schemas/dodoResponse"}},
...
async def send_message(
    ...
    agent_id: AgentId,
    server: SyncServer = Depends(get_dodo_server),
    request: dodoStreamingRequest = Body(...),
    headers: HeaderParams = Depends(get_headers),
) -> StreamingResponse | dodoResponse:
    """
    ...
    **Note:** Sending multiple concurrent requests to the same agent can lead to undefined behavior.
    ...
    - If `streaming=false` (default): Returns a complete dodoResponse with all messages
...
@router.post(
    ...
    response_model=dodoStreamingResponse,
...
async def _cleanup_background_task_resources(agent_loop: BaseAgentV2 | dodoAgent, result: StreamingResponse | dodoResponse) -> None:
    """
    ...
    **Note:** Sending multiple concurrent requests to the same agent can lead to undefined behavior.
    ...
    if use_lettuce:
        ...
        is_v1_message_async_enabled = (
            agent_state.agent_type == AgentType.dodo_v1_agent and headers.experimental_params.dodo_v1_agent_message_async
        )
        if agent_state.multi_agent_group is None and (agent_state.agent_type != AgentType.dodo_v1_agent or is_v1_message_async_enabled):
            lettuce_client = await LettuceClient.create()
...
async def preview_model_request(
    agent_id: AgentId,
    request: Union[dodoRequest, dodoStreamingRequest] = Body(...),
    server: SyncServer = Depends(get_dodo_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
...
async def summarize_messages(
    ...
    agent_loop = dodoAgentV3(agent_state=agent, actor=actor)
    ...
    if agent.compaction_settings and request and request.compaction_settings:
        ...
        if (
            ...
            from dodo.services.summarizer.summarizer_config import get_default_prompt_for_mode
