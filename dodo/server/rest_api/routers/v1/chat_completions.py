...
async def _handle_chat_completion(
    ...
    # convert OpenAI messages to dodo MessageCreate format
    # NOTE: we only process the last user message
    if len(request.messages) > 1:
        logger.warning(
            f"Chat completions endpoint received {len(request.messages)} messages. "
            "dodo maintains conversation state internally, so only the last user message will be processed. "
    ...
    dodo_request = dodoStreamingRequest(
        messages=dodo_messages,
        stream_tokens=True,
    )
...
async def create_chat_completion(
    ...
    """
    Create a chat completion using a dodo agent (OpenAI-compatible).
