...
@router.post("/search", response_model=List[dodoMessageSearchResult], operation_id="search_all_messages")
async def search_all_messages(
    ...
    return Message.to_dodo_search_results_from_list(search_results=results, text_is_assistant_message=True)
...
async def create_batch(
    ...
    if not settings.enable_batch_job_polling:
        logger.warning("Batch job polling is disabled. Enable batch processing by setting dodo_ENABLE_BATCH_JOB_POLLING to True.")
    ...
    try:
        ...
        await batch_runner.step_until_request(batch_requests=payload.requests, dodo_batch_job_id=batch_job.id)
...
@router.get(
    ...
    response_model=dodoBatchMessages,
...
async def list_messages_for_batch(
    ...
    messages = await server.batch_manager.get_messages_for_dodo_batch_async(
        dodo_batch_job_id=batch_id, actor=actor, limit=limit, agent_id=agent_id, sort_descending=(order == "desc"), cursor=after
    )

    return dodoBatchMessages(messages=messages)
...
async def cancel_batch(
    ...
    llm_batch_jobs = await server.batch_manager.list_llm_batch_jobs_async(dodo_batch_id=job.id, actor=actor)
...
async def retrieve_message(
    ...
    return message.to_dodo_messages()
