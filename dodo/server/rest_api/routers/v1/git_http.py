...
def set_server_instance(server) -> None:
    """Set the dodo server instance for git operations. Called during app startup."""
...
async def _sync_after_push(actor_id: str, agent_id: str) -> None:
    ...
    from dodo.services.block_manager_git import GitEnabledBlockManager
    ...
    expected_labels = set()
    from dodo.services.memory_repo.block_markdown import parse_block_markdown
...
async def proxy_git_http(
    path: str,
    request: Request,
    server=Depends(get_dodo_server),
    headers: HeaderParams = Depends(get_headers),
):
    """Proxy `/v1/git/*` requests to the memfs service.

    Requires dodo_MEMFS_SERVICE_URL to be configured.
    """
    ...
    if not memfs_url:
        return JSONResponse(
            status_code=501,
            content={
                "detail": "git HTTP requires memfs service (dodo_MEMFS_SERVICE_URL not configured)",
            },
        )
