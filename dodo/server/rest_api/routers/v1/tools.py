...
from dodo.functions.mcp_client.types import MCPTool, SSEServerConfig, StdioServerConfig, StreamableHTTPServerConfig
...
from dodo.services.mcp.types import OauthStreamEvent
from dodo.settings import tool_settings
from dodo.validators import ToolId
...
async def count_tools(
    ...
    return_only_dodo_tools: Optional[bool] = Query(False, description="Count only tools with tool_type starting with 'dodo_'"),
    exclude_dodo_tools: Optional[bool] = Query(False, description="Exclude built-in dodo tools from the count"),
    server: SyncServer = Depends(get_dodo_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
    ...
    return await server.tool_manager.count_tools_async(
        ...
        exclude_dodo_tools=exclude_dodo_tools,
...
async def list_tools(
    ...
    return_only_dodo_tools: Optional[bool] = Query(False, description="Return only tools with tool_type starting with 'dodo_'"),
    server: SyncServer = Depends(get_dodo_server),
    headers: HeaderParams = Depends(get_headers),
):
    """
...
async def list_mcp_tools_by_server(
    ...
    try:
        ...
        raise dodoMCPConnectionError(str(e), server_name=mcp_server_name)
    except HTTPStatusError as e:
        # HTTPStatusError from the MCP server likely means auth issue
        if e.response.status_code == 401:
            raise dodoMCPConnectionError(f"Authentication failed: {e}", server_name=mcp_server_name)
        else:
            raise dodoMCPConnectionError(f"HTTP error from MCP server: {e}", server_name=mcp_server_name)
...
async def add_mcp_tool(
    ...
    """
    Register a new MCP tool as a dodo server by MCP server + tool name
    """
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    ...
    if tool_settings.mcp_read_from_config:
        try:
            ...
            raise dodoMCPTimeoutError(str(e), server_name=mcp_server_name)
...
async def add_mcp_server_to_config(
    ...
    """
    Add a new MCP server to the dodo MCP server config
    """
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    ...
    if tool_settings.mcp_read_from_config:
        ...
        if isinstance(request, StdioServerConfig) and tool_settings.mcp_disable_stdio:
            raise HTTPException(
                status_code=400,
                detail="stdio is not supported in the current environment, please use a self-hosted dodo server in order to add a stdio MCP server",
            )
...
async def test_mcp_server(
    ...
    except (ConnectionError, dodoMCPConnectionError) as e:
        if isinstance(e, dodoMCPConnectionError):
            raise
        raise dodoMCPConnectionError(str(e), server_name=request.server_name)
    except MCPTimeoutError as e:
        raise dodoMCPTimeoutError(f"MCP server connection timed out: {str(e)}", server_name=request.server_name)
    finally:
        if client:
            try:
...
async def generate_json_schema(
    ...
    if request.source_type == "typescript":
        from dodo.functions.typescript_parser import derive_typescript_json_schema
    ...
    try:
        ...
        try:
            ...
            if exception_to_check.__class__.__name__ == "ToolError":
                ...
                raise dodoMCPConnectionError(f"MCP tool execution failed: {str(exception_to_check)}", server_name=tool_name)
            raise
...
async def generate_tool_from_prompt(
    ...
    input_messages = [
        Message(role=MessageRole.system, content=[TextContent(text=get_system_text("dodo_generate_tool"))]),
