...
@router.patch("/{group_id}/messages/{message_id}", response_model=dodoMessageUnion, operation_id="modify_group_message", deprecated=True)
async def modify_group_message(
    ...
    actor = await server.user_manager.get_actor_or_default_async(actor_id=headers.actor_id)
    return await server.message_manager.update_message_by_dodo_message(message_id=message_id, dodo_message_update=request, actor=actor)
