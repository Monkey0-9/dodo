...
from dodo.constants import DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG, dodo_MODEL_ENDPOINT
...
async def create_chat_completions(
    ...
    dodo_agent = server.load_agent(agent_id=agent_id, actor=actor)
    llm_config = dodo_agent.agent_state.llm_config
    if llm_config.model_endpoint_type != "openai" or llm_config.model_endpoint == dodo_MODEL_ENDPOINT:
...
async def send_message_to_agent_chat_completions(
    ...
    try:
        ...
        dodo_agent.interface = ChatCompletionsStreamingInterface()
        streaming_interface = dodo_agent.interface
        ...
        safe_create_task(
            asyncio.to_thread(
                ...
                actor=actor,
                agent_id=dodo_agent.agent_state.id,
