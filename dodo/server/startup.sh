...
wait_for_postgres() {
    until pg_isready -U "${POSTGRES_USER:-dodo}" -h localhost; do
...
if [ -n "$dodo_REDIS_HOST" ]; then
    echo "External Redis configuration detected, using env var dodo_REDIS_HOST=$dodo_REDIS_HOST"
else
    ...
    export dodo_REDIS_HOST="localhost"
    echo "Using internal Redis at: $dodo_REDIS_HOST"
fi

# Check if we're configured for external Postgres
if [ -n "$dodo_PG_URI" ]; then
    echo "External Postgres configuration detected, using env var dodo_PG_URI"
else
    ...
    export dodo_PG_URI="postgresql://${POSTGRES_USER:-dodo}:${POSTGRES_PASSWORD:-dodo}@localhost:5432/${POSTGRES_DB:-dodo}"
    echo "Using internal PostgreSQL at: $dodo_PG_URI"
fi
...
if [ -n "$dodo_SANDBOX_MOUNT_PATH" ]; then
    if ! chmod 777 "$dodo_SANDBOX_MOUNT_PATH"; then
        echo "ERROR: Failed to set permissions for tool execution directory at: $dodo_SANDBOX_MOUNT_PATH"
...
CMD="dodo server --host $HOST --port $PORT"
...
echo "Starting dodo Server at http://$HOST:$PORT..."
echo "Executing: $CMD"
exec $CMD
