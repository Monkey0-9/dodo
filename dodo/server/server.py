...
import dodo.constants as constants
import dodo.server.utils as server_utils
...
from dodo.functions.mcp_client.types import MCPServerType, MCPTool, MCPToolHealth, SSEServerConfig, StdioServerConfig
from dodo.functions.schema_validator import validate_complete_json_schema
from dodo.helpers.datetime_helpers import get_utc_time

# TODO use custom interface
from dodo.interface import (
...
from dodo.schemas.providers import (
    AnthropicProvider,
    AzureProvider,
    BasetenProvider,
    BedrockProvider,
...
from dodo.services.archive_manager import ArchiveManager
from dodo.services.block_manager import BlockManager
from dodo.services.block_manager_git import GIT_MEMORY_ENABLED_TAG, GitEnabledBlockManager
from dodo.services.file_manager import FileManager
from dodo.services.files_agents_manager import FileAgentManager
from dodo.services.group_manager import GroupManager
from dodo.services.helpers.tool_execution_helper import prepare_local_sandbox
from dodo.services.identity_manager import IdentityManager
from dodo.services.job_manager import JobManager
from dodo.services.llm_batch_manager import LLMBatchManager
from dodo.services.mcp.base_client import AsyncBaseMCPClient
from dodo.services.mcp.fastmcp_client import AsyncFastMCPSSEClient
from dodo.services.mcp.sse_client import MCP_CONFIG_TOPLEVEL_KEY
from dodo.services.mcp.stdio_client import AsyncStdioMCPClient
from dodo.services.mcp_manager import MCPManager
from dodo.services.mcp_server_manager import MCPServerManager
from dodo.services.memory_repo import MemfsClient
...
from dodo.services.step_manager import StepManager
from dodo.services.telemetry_manager import TelemetryManager
from dodo.services.tool_executor.tool_execution_manager import ToolExecutionManager
from dodo.services.tool_manager import ToolManager
from dodo.services.user_manager import UserManager
from dodo.settings import DatabaseChoice, model_settings, settings, tool_settings
from dodo.streaming_interface import AgentChunkStreamingInterface
from dodo.utils import get_friendly_error_msg, get_persona_text
...
class SyncServer(object):
    ...
    def __init__(
        ...
        if settings.database_engine is DatabaseChoice.POSTGRES:
            ...
            config.recall_storage_uri = settings.dodo_pg_uri_no_default
            config.archival_storage_type = "postgres"
            config.archival_storage_uri = settings.dodo_pg_uri_no_default
        config.save()
        self.config = config
        ...
        from dodo.services.agent_generate_completion_manager import AgentGenerateCompletionManager
    ...
    async def init_async(self, init_with_default_org_and_user: bool = True):
        ...
        if init_with_default_org_and_user:
            ...
            tool_dir = tool_settings.tool_exec_dir or dodo_TOOL_EXECUTION_DIR

            venv_dir = Path(tool_dir) / venv_name
            tool_path = Path(tool_dir)

            if tool_path.exists() and not tool_path.is_dir():
                logger.error(f"dodo_TOOL_SANDBOX_DIR exists but is not a directory: {tool_dir}")
            else:
                if not tool_path.exists():
                    logger.warning(f"dodo_TOOL_SANDBOX_DIR does not exist, creating now: {tool_dir}")
                    tool_path.mkdir(parents=True, exist_ok=True)

                if tool_settings.tool_exec_venv_name and not venv_dir.is_dir():
                    logger.warning(
                        f"Provided dodo_TOOL_SANDBOX_VENV_NAME is not a valid venv ({venv_dir}), one will be created for you during tool execution."
                    )
    ...
    def _init_memory_repo_manager(self) -> Optional[MemfsClient]:
        ...
        Requires dodo_MEMFS_SERVICE_URL to be set to the external memfs service URL.
    ...
    async def init_mcp_clients(self):
        ...
        for server_name, server_config in mcp_server_configs.items():
            if server_config.type == MCPServerType.SSE:
                ...
                raise dodoInvalidArgumentError(f"Invalid MCP server config: {server_config}", argument_name="server_config")
        ...
        if request.llm_config is None:
            ...
            if request.model is None:
                if settings.default_llm_handle is None:
                    raise dodoInvalidArgumentError("Must specify either model or llm_config in request", argument_name="model")
                else:
                ...
                if isinstance(request.model, str):
                    ...
                    raise dodoInvalidArgumentError("Multiple models are not supported yet")
                else:
    ...
    async def insert_archival_memory_async(
        ...
        from dodo.services.context_window_calculator.token_counter import create_token_counter
        from dodo.settings import settings
    ...
    async def get_agent_recall(
    ...
    ) -> Union[List[Message], List[dodoMessage]]:
        ...
        if not return_message_object:
            records = Message.to_dodo_messages_from_list(
                messages=records,
    ...
    async def get_agent_recall_async(
        ...
        if not return_message_object:
            ...
            agent_state = await self.agent_manager.get_agent_by_id_async(agent_id=agent_id, actor=actor)
            text_is_assistant_message = agent_state.agent_type == AgentType.dodo_v1_agent
    ...
    async def get_all_messages_recall_async(
        ...
        if not return_message_object:
            # NOTE: We are assuming all messages are coming from dodo_v1_agent. This may lead to slightly incorrect assistant message handling.
            # text_is_assistant_message = agent_state.agent_type == AgentType.dodo_v1_agent
    ...
    def get_server_config(self, include_defaults: bool = False) -> dict:
        ...
        if include_defaults:
            default_config = vars(dodoConfig())
    ...
    async def load_file_to_source(self, source_id: str, file_path: str, job_id: str, actor: User) -> Job:
        ...
        # try:
        from dodo.data_sources.connectors import DirectoryConnector
    ...
    async def get_embedding_config_from_handle_async(
        ...
        try:
            ...
            # Convert to dodoInvalidArgumentError for backwards compatibility
            from dodo.orm.errors import NoResultFound
    ...
    async def run_tool_from_source(
        ...
        try:
            ...
            from dodo.schemas.dodo_message import ToolReturn as ToolReturnSchema
    ...
    async def get_mcp_servers(self) -> dict[str, Union[SSEServerConfig, StdioServerConfig]]:
        ...
        # Attempt to read from ~/.dodo/mcp_config.json
        mcp_config_path = os.path.join(constants.dodo_DIR, constants.MCP_CONFIG_NAME)
    ...
    async def get_tools_from_mcp_server(self, mcp_server_name: str) -> List[MCPTool]:
        ...
        if mcp_server_name not in self.mcp_clients:
            raise dodoInvalidArgumentError(f"No client was created for MCP server: {mcp_server_name}", argument_name="mcp_server_name")
    ...
    async def add_mcp_server_to_config(
        ...
        try:
            ...
            raise dodoInvalidArgumentError(f"Failed to parse MCP config file {mcp_config_path}")
    ...
    async def delete_mcp_server_from_config(self, server_name: str) -> dict[str, Union[SSEServerConfig, StdioServerConfig]]:
        ...
        if server_name not in current_mcp_servers:
            raise dodoInvalidArgumentError(f"Server name {server_name} not found in MCP config file", argument_name="server_name")
