...
# Base tools if the dodo agent has enable_sleeptime on
...
# Updated to match new arrow format: "1â†’ content"
# shared constant for both memory_insert and memory_replace
MEMORY_TOOLS_LINE_NUMBER_PREFIX_REGEX = re.compile(
    r"^[ \t]*\d+â†’[ \t]*",  # match number followed by arrow, with optional whitespace
    re.MULTILINE,
)
...
CORE_MEMORY_LINE_NUMBER_WARNING = "# NOTE: Line numbers shown below (with arrows like '1â†’') are to help during editing. Do NOT include line number prefixes in your memory edit tool calls."
...
# The error message that dodo will receive
