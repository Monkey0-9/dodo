...
from dodo.llm_api.error_utils import is_context_window_overflow_message
...
from dodo.server.rest_api.utils import decrement_message_uuid
from dodo.services.context_window_calculator.token_counter import create_token_counter
from dodo.streaming_utils import (
...
class OpenAIStreamingInterface:
    ...
    def __init__(
        ...
        self.dodo_message_id = Message.generate_id()
    ...
    def get_usage_statistics(self) -> "dodoUsageStatistics":
        """Extract usage statistics from accumulated streaming data.

        Returns:
            dodoUsageStatistics with token counts from the stream.
        """
        from dodo.schemas.usage import dodoUsageStatistics
    ...
    async def process(
    ...
    ) -> AsyncGenerator[dodoMessage | dodoStopReason, None]:
        """
        ...
        try:
            ...
            yield dodoStopReason(stop_reason=StopReasonType.error)
            raise e
        finally:
            ...
            if message_delta.tool_calls is not None and len(message_delta.tool_calls) > 0:
                ...
                if not self.emitted_hidden_reasoning and is_openai_reasoning_model(self.model) and not self.put_inner_thoughts_in_kwarg:
                    ...
                    hidden_message = HiddenReasoningMessage(
                        id=self.dodo_message_id,
                        date=datetime.now(timezone.utc),
                        state="omitted",
                        hidden_reasoning=None,
                        otid=Message.generate_otid_from_id(self.dodo_message_id, message_index),
                        run_id=self.run_id,
                            ...
                            if self.use_assistant_message and self._get_function_name_buffer() == self.assistant_message_tool_name:
                                ...
                                if self.tool_call_name in self.requires_approval_tools:
                                    tool_call_msg = ApprovalRequestMessage(
                                        id=decrement_message_uuid(self.dodo_message_id),
                                        date=datetime.now(timezone.utc),
                                        tool_call=tool_call_delta,
                                        tool_calls=tool_call_delta,
                                        otid=Message.generate_otid_from_id(decrement_message_uuid(self.dodo_message_id), -1),
                                        run_id=self.run_id,
                                        step_id=self.step_id,
                                    )
                                else:
    ...
    def get_content(self) -> list[TextContent | SummarizedReasoningContent]:
        ...
        for response in self.final_response.output:
            if isinstance(response, ResponseReasoningItem):
                ...
                dodo_summary = [SummarizedReasoningContentPart(index=i, text=part.text) for i, part in enumerate(response.summary)]
                content.append(
                    SummarizedReasoningContent(
                        id=response.id,
                        summary=dodo_summary,
