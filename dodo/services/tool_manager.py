...
from dodo.helpers.tool_helpers import compute_tool_hash, generate_modal_function_name
from dodo.log import get_logger

# TODO: Remove this once we translate all of these to the ORM
from dodo.orm.errors import NoResultFound
from dodo.orm.tool import Tool as ToolModel
from dodo.otel.tracing import trace_method, tracer
from dodo.schemas.enums import PrimitiveType, SandboxType, ToolType
...
from dodo.services.helpers.agent_manager_helper import calculate_multi_agent_tools
from dodo.services.mcp.types import SSEServerConfig, StdioServerConfig
from dodo.services.tool_schema_generator import generate_schema_for_tool_creation, generate_schema_for_tool_update
from dodo.settings import settings, tool_settings
from dodo.utils import enforce_types, fire_and_forget, printd
...
def modal_tool_wrapper(tool: PydanticTool, actor: PydanticUser, sandbox_env_vars: dict | None = None, project_id: str = "default"):
    ...
    packages.append("dodo_client>=1.1.1")
    packages.append("dodo")  # Base dodo without extras
    ...
    secrets_dict = {"dodo_API_KEY": None}
    if sandbox_env_vars:
    ...
    def modal_function(
        tool_name: str, agent_state: Optional[dict], agent_id: Optional[str], env_vars: dict, dodo_api_key: Optional[str] = None, **kwargs
    ):
        ...
        if agent_state:
            try:
                from dodo.schemas.agent import AgentState as AgentStateModel
        ...
        # TODO: directly instantiate the dodo client once we upgrade to 1.0.0+ in core
        # Initialize the dodo client
        # if dodo_api_key:
        #     dodo_client = dodo(token=dodo_api_key, base_url=os.environ.get("dodo_API_URL", "https://api.dodo.com"))
        # else:
        #     dodo_client = None

        tool_namespace = {
            "__builtins__": __builtins__,  # Include built-in functions
            # "_dodo_client": dodo_client,  # Make dodo_client available
            "os": os,  # Include os module for env vars access
            "agent_id": agent_id,
            "_dodo_API_KEY": dodo_api_key,
        ...
        return {
            ...
            "stdout": stdout,
            "stderr": stderr,
            "agent_state": agent_state,  # TODO: deprecate (use dodo_client instead)
...
class ToolManager:
    ...
    async def create_or_update_tool_async(
        ...
        if not pydantic_tool.name:
            ...
            if pydantic_tool.name != pydantic_tool.json_schema.get("name"):
                raise dodoToolNameSchemaMismatchError(
        ...
        if settings.dodo_pg_uri_no_default:
    ...
    async def _atomic_upsert_tool_postgresql(
        ...
        from dodo.helpers.tpuf_client import should_use_tpuf_for_tools
    ...
    async def list_tools_async(
        ...
        return_only_dodo_tools: bool = False,
        ...
        tools = await self._list_tools_async(
            ...
            tool_ids=tool_ids,
            search=search,
            return_only_dodo_tools=return_only_dodo_tools,
        ...
        if not after and upsert_base_tools:
            ...
            base_tool_names = dodo_TOOL_SET - set(LOCAL_ONLY_MULTI_AGENT_TOOLS) if settings.environment == "prod" else dodo_TOOL_SET
    ...
    async def _list_tools_async(
        ...
        async with db_registry.async_session() as session:
            ...
            # Apply return_only_dodo_tools filter at database level
            if return_only_dodo_tools:
                query = query.where(ToolModel.tool_type.like("dodo_%"))
    ...
    async def count_tools_async(
        ...
        async with db_registry.async_session() as session:
            ...
            # Handle exclude_dodo_tools logic (if True, exclude dodo tools)
            if exclude_dodo_tools:
                # Exclude tools that are in the dodo_TOOL_SET
                dodo_tool_names = list(dodo_TOOL_SET)
                query = query.where(~ToolModel.name.in_(dodo_tool_names))
    ...
    async def size_async(
        ...
        async with db_registry.async_session() as session:
            ...
            return await ToolModel.size_async(db_session=session, actor=actor, name=dodo_TOOL_SET)
    ...
    async def update_tool_by_id_async(
        ...
        async with db_registry.async_session() as session:
            ...
            if needs_name_conflict_check:
                ...
                if name_conflict:
                    raise dodoToolNameConflictError(tool_name=new_name)
    ...
    async def upsert_base_tools_async(
        ...
        functions_to_schema = {}
        for module_name in dodo_TOOL_MODULE_NAMES:
            try:
        ...
        for name, schema in functions_to_schema.items():
            if name not in dodo_TOOL_SET:
                continue

            if name in BASE_TOOLS:
                tool_type = ToolType.dodo_CORE
            elif name in BASE_MEMORY_TOOLS:
                tool_type = ToolType.dodo_MEMORY_CORE
            elif name in BASE_SLEEPTIME_TOOLS:
                tool_type = ToolType.dodo_SLEEPTIME_CORE
            elif name in calculate_multi_agent_tools():
                tool_type = ToolType.dodo_MULTI_AGENT_CORE
            elif name in BASE_VOICE_SLEEPTIME_TOOLS or name in BASE_VOICE_SLEEPTIME_CHAT_TOOLS:
                tool_type = ToolType.dodo_VOICE_SLEEPTIME_CORE
            elif name in BUILTIN_TOOLS:
                tool_type = ToolType.dodo_BUILTIN
            elif name in FILES_TOOLS:
                tool_type = ToolType.dodo_FILES_CORE
            else:
            ...
            parallel_safe = name in dodo_PARALLEL_SAFE_TOOLS
    ...
    async def create_or_update_modal_app(self, tool: PydanticTool, actor: PydanticUser):
        ...
        with modal.enable_output(show_progress=False):
            try:
                ...
                raise dodoInvalidArgumentError(f"Failed to deploy tool {tool.id} with name {tool.name} to Modal: {e}")
    ...
    async def search_tools_async(
        ...
        """
        from dodo.helpers.tpuf_client import TurbopufferClient, should_use_tpuf_for_tools
