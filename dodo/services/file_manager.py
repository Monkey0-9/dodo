...
from dodo.schemas.source_metadata import FileStats, OrganizationSourcesStats, SourceStats
...
from dodo.utils import bounded_gather, enforce_types
...
class FileManager:
    ...
    async def update_file_status(
        ...
        * 1st round-trip â†’ UPDATE with optional state validation
        * 2nd round-trip â†’ SELECT fresh row (same as read_async) if update succeeded
        """
    ...
    async def get_organization_sources_metadata(
        ...
        async with db_registry.async_session() as session:
            ...
            from dodo.orm.source import Source as SourceModel
    ...
    async def get_files_for_agents_async(
        ...
        async with db_registry.async_session() as session:
            ...
            from dodo.orm.files_agents import FileAgent as FileAgentModel
