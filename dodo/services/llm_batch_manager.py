...
from dodo.jobs.types import BatchPollingResult, ItemUpdateInfo, RequestStatusUpdateInfo, StepStatusUpdateInfo
from dodo.log import get_logger
from dodo.orm import Message as MessageModel
...
from dodo.schemas.enums import AgentStepStatus, JobStatus, ProviderType
from dodo.schemas.llm_batch_job import AgentStepState, LLMBatchItem as PydanticLLMBatchItem, LLMBatchJob as PydanticLLMBatchJob
...
class LLMBatchManager:
    ...
    async def bulk_update_llm_batch_statuses_async(
        ...
        `updates` = [(llm_batch_id, new_status, polling_response_or_None), â€¦]
        """
    ...
    async def list_llm_batch_jobs_async(
        ...
        """
        async with db_registry.async_session() as session:
            query = select(LLMBatchJob).where(LLMBatchJob.dodo_batch_job_id == dodo_batch_id)
    ...
    @enforce_types
    @trace_method
    async def get_messages_for_dodo_batch_async(
        self,
        dodo_batch_job_id: str,
        actor: PydanticUser,
        limit: int = 100,
        ...
        """
        Retrieve messages across all LLM batch jobs associated with a dodo batch job.
        ...
        async with db_registry.async_session() as session:
            ...
            query = (
                ...
                .where(LLMBatchJob.dodo_batch_job_id == dodo_batch_job_id)
