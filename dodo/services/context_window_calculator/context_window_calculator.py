...
from dodo.services.context_window_calculator.token_counter import TokenCounter
...
class ContextWindowCalculator:
    ...
    def _extract_system_prompt(system_message: str) -> Optional[str]:
        ...
        (e.g. custom system prompts from dodo Code agents), falls back to
        ...
        Note:
            ...
            - Custom prompt agents (e.g. dodo Code): raw preamble text without any tags
        """
    ...
    def _extract_top_level_tag(system_message: str, tag_name: str, container_tag: str = "memory_blocks") -> Optional[str]:
        ...
        and at top-level â€” scans all occurrences to find one outside the container.
        ...
        while True:
            ...
            # Found a top-level occurrence â€” extract it
    ...
    def extract_system_components(system_message: str) -> Dict[str, Optional[str]]:
        ...
        - Custom system prompts without <base_instructions> (e.g. dodo Code agents)
