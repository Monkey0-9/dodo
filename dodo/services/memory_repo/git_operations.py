...
from dodo.schemas.memory_repo import FileChange, MemoryCommit
...
class GitOperations:
    ...
    async def create_repo(
        ...
        author_name: str = "dodo System",
        author_email: str = "system@dodo.ai",
    ) -> str:
        ...
        def _create():
            temp_dir = tempfile.mkdtemp(prefix="dodo-memrepo-")
            try:
                ...
                if initial_files:
                    ...
                    # Create an empty .dodo directory to initialize
                    dodo_dir = os.path.join(repo_path, ".dodo")
                    os.makedirs(dodo_dir, exist_ok=True)
                    config_path = os.path.join(dodo_dir, "config.json")
                    with open(config_path, "w") as f:
                        f.write('{"version": 1}')
                    _run_git(["add", ".dodo/config.json"], cwd=repo_path)
    ...
    async def commit(
        ...
        message: str,
        author_name: str = "dodo Agent",
        author_email: str = "agent@dodo.ai",
        branch: str = "main",
    ) -> MemoryCommit:
