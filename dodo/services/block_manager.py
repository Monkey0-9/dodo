...
from dodo.orm.blocks_agents import BlocksAgents
from dodo.orm.blocks_tags import BlocksTags
...
from dodo.schemas.enums import ActorType, PrimitiveType
...
class BlockManager:
    ...
    async def checkpoint_block_async(
        ...
        async with db_registry.async_session() as session:
            ...
            history_entry = BlockHistory(
                ...
                actor_type=ActorType.dodo_AGENT if agent_id else ActorType.dodo_USER,
    ...
    async def undo_checkpoint_block(
        ...
        async with db_registry.async_session() as session:
            ...
            if not block.current_history_entry_id:
                raise dodoInvalidArgumentError(f"Block {block_id} has no history entry - cannot undo.", argument_name="block_id")
    ...
    async def redo_checkpoint_block(
        ...
        async with db_registry.async_session() as session:
            ...
            if not block.current_history_entry_id:
                raise dodoInvalidArgumentError(f"Block {block_id} has no history entry - cannot redo.", argument_name="block_id")
