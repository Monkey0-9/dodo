...
from dodo.services.helpers.tool_execution_helper import add_imports_and_pydantic_schemas_for_args
from dodo.services.helpers.tool_parser_helper import convert_param_to_str_value, parse_function_arguments
...
class AsyncToolSandboxBase(ABC):
    ...
    async def _init_async(self):
        ...
        if not self._initialized:
            ...
            if not self.tool.source_code and self.tool.tool_type == ToolType.dodo_MULTI_AGENT_CORE:
                module = importlib.import_module(dodo_MULTI_AGENT_TOOL_MODULE_NAME)
            ...
            # (these are Python-only features). Instead, agent_id is exposed via dodo_AGENT_ID env var.
            ...
            # Always inject dodo client (available as `client` variable in sandbox)
            # For Python: dodo_client package
            # For TypeScript: @dodo-ai/dodo-client package
            self.inject_dodo_client = True
    ...
    def _render_sandbox_code(
        ...
        *,
        ...
        if inject_agent_state:
            lines.extend(["import dodo", "from dodo import *"])

        # Import dodo client if available (wrapped in try/except for sandboxes without dodo_client installed)
        if inject_dodo_client:
            lines.extend(
                [
                    "try:",
        ...
        # Initialize dodo client if needed (client is always available as a variable, may be None)
        if inject_dodo_client:
            lines.extend(
                [
                    "# Initialize dodo client for tool execution",
                    "import os",
                    "client = None",
                    "if dodo is not None and os.getenv('dodo_API_KEY'):",
                    "    # Check dodo_client version to use correct parameter name",
                    "    from packaging import version as pkg_version",
                    "    import dodo_client as lc_module",
                    "    lc_version = pkg_version.parse(lc_module.__version__)",
                    "    if lc_version < pkg_version.parse('1.0.0'):",
                    "        client = dodo(",
                    "            token=os.getenv('dodo_API_KEY')",
                    "        )",
                    "    else:",
                    "        client = dodo(",
        ...
        if self.args:
            ...
            lines.extend(
                [
                    f"__dodo_raw_args = {{{raw_args}}}",
                    "try:",
                    "    from dodo.functions.ast_parsers import coerce_dict_args_by_annotations",
                    f"    __dodo_func = {self.tool.name}",
                    "    __dodo_annotations = getattr(__dodo_func, '__annotations__', {})",
            ...
            for name in self.args:
                lines.append(f"    {name} = __dodo_coerced_args.get({name!r}, {name})")
    ...
    async def _generate_typescript_execution_script(self, agent_state: Optional[AgentState]) -> str:
        ...
        - agent_id is available via process.env.dodo_AGENT_ID
        - Return JSON-serialized results instead of pickle
        - Require explicit json_schema (no docstring parsing)
        """
        from dodo.services.tool_sandbox.typescript_generator import generate_typescript_execution_script
    ...
    async def _gather_env_vars(
        ...
        if self.agent_id:
            env["dodo_AGENT_ID"] = self.agent_id
        if self.project_id:
            env["dodo_PROJECT_ID"] = self.project_id
        env["dodo_TOOL_ID"] = self.tool_id
