...
def generate_typescript_execution_script(
    ...
    1. Imports and initializes the dodo client (available as `client` variable)
    ...
    The agent_id is available via process.env.dodo_AGENT_ID environment variable.
    ...
    # Import and initialize dodo client (similar to Python's dodo_client injection)
    ...
    lines.extend(
        [
            "// Initialize dodo client for TypeScript tool execution",
            "let client: any = null;",
            "try {",
            "    const { dodoClient } = await import('@dodo-ai/dodo-client');",
            "    const apiKey = process.env.dodo_API_KEY;",
            "    if (apiKey) {",
            "        client = new dodoClient({ apiKey });",
            "    }",
            "} catch (e) {",
