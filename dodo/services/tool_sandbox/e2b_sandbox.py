...
from dodo.services.helpers.tool_parser_helper import parse_stdout_best_effort
from dodo.services.tool_sandbox.base import AsyncToolSandboxBase
...
class AsyncToolSandboxE2B(AsyncToolSandboxBase):
    ...
    async def create_e2b_sandbox_with_metadata_hash(self, sandbox_config: SandboxConfig) -> "AsyncSandbox":
        ...
        # Auto-install @dodo-ai/dodo-client for TypeScript tools (similar to dodo_client for Python)
        if self.is_typescript_tool():
            dodo_client_package = "@dodo-ai/dodo-client"
            log_event(
                "dodo_client_npm_install_started",
                {
                    "sandbox_id": sbx.sandbox_id,
                    "package": dodo_client_package,
                    "tool_name": self.tool.name if self.tool else None,
                },
            )
            try:
                await sbx.commands.run(f"npm install {dodo_client_package}")
                log_event(
                    "dodo_client_npm_install_finished",
                    {
                        "sandbox_id": sbx.sandbox_id,
                        "package": dodo_client_package,
                        "tool_name": self.tool.name if self.tool else None,
                ...
                logger.warning(f"Failed to install {dodo_client_package} in E2B sandbox: {e}")
                log_event(
                    "dodo_client_npm_install_failed",
                    {
                        "sandbox_id": sbx.sandbox_id,
                        "package": dodo_client_package,
                        "tool_name": self.tool.name if self.tool else None,
