...
from dodo.helpers.datetime_helpers import get_local_time
from dodo.llm_api.llm_client import LLMClient
from dodo.orm.agent import Agent as AgentModel
from dodo.orm.agents_tags import AgentsTags
from dodo.orm.archives_agents import ArchivesAgents
...
from dodo.orm.sources_agents import SourcesAgents
...
from dodo.system import get_initial_boot_messages, get_login_event, package_function_response
...
def derive_system_message(agent_type: AgentType, enable_sleeptime: Optional[bool] = None, system: Optional[str] = None) -> str:
    ...
    Args:
        agent_type: The type of agent (e.g., dodo_agent, sleeptime_agent, react_agent)
    ...
    if system is None:
        ...
        # Dodo v1, both w/ and w/o sleeptime
        elif agent_type == AgentType.dodo_agent and not enable_sleeptime:
            system = gpt_system.get_system_text("dodo_v2_chat")
        elif agent_type == AgentType.dodo_agent and enable_sleeptime:
            # NOTE: same as the chat one, since the chat one says that you "may" have the tools
            system = gpt_system.get_system_text("dodo_v2_chat")

        # Dodo v2, both w/ and w/o sleeptime
        elif agent_type == AgentType.dodo_v2_agent and not enable_sleeptime:
            system = gpt_system.get_system_text("dodo_v2_chat")
        elif agent_type == AgentType.dodo_v2_agent and enable_sleeptime:
            # NOTE: same as the chat one, since the chat one says that you "may" have the tools
            system = gpt_system.get_system_text("dodo_v2_chat")
        ...
        # dodo v1
        elif agent_type == AgentType.dodo_v1_agent:
            system = gpt_system.get_system_text("dodo_v1")
...
def initialize_message_sequence(
    ...
    first_user_message = get_login_event(agent_state.timezone)  # event letting dodo know the user just logged in
...
async def initialize_message_sequence_async(
    ...
    if agent_state.agent_type == AgentType.dodo_v1_agent:
    ...
    if include_initial_boot_message:
        ...
        if agent_state.agent_type == AgentType.sleeptime_agent or agent_state.agent_type == AgentType.dodo_v1_agent:
            initial_boot_messages = []
        elif llm_config.model is not None and "gpt-3.5" in llm_config.model:
            initial_boot_messages = get_initial_boot_messages("startup_with_send_message_gpt35", agent_state.timezone, tool_call_id)
        else:
...
async def build_passage_query(
    ...
    if embedded_text:
        if settings.database_engine is DatabaseChoice.POSTGRES:
            ...
            from dodo.orm.sqlite_functions import adapt_array
...
def calculate_base_tools(is_v2: bool) -> Set[str]:
    if is_v2:
        return (set(BASE_TOOLS) - set(DEPRECATED_dodo_TOOLS)) | set(BASE_MEMORY_TOOLS_V2)
    else:
        return (set(BASE_TOOLS) - set(DEPRECATED_dodo_TOOLS)) | set(BASE_MEMORY_TOOLS)
...
async def validate_agent_exists_async(session, agent_id: str, actor: User) -> None:
    ...
    if not result.scalar():
        raise dodoAgentNotFoundError(f"Agent with ID {agent_id} not found")
