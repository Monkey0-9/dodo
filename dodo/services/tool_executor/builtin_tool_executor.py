...
class dodoBuiltinToolExecutor(ToolExecutor):
    """Executor for built in dodo tools."""
    ...
    async def run_code_with_tools(self, agent_state: "AgentState", code: str) -> ToolExecutionResult:
        ...
        env = {"dodo_AGENT_ID": agent_state.id}
        ...
        if agent_state and agent_state.tools:
            for tool in agent_state.tools:
                if tool.tool_type == ToolType.CUSTOM and tool.source_code:
                    ...
                    # raises an error if dodo_API_KEY or other envs not set
                    tool_lines = [
                        f"def {tool.name}(**kwargs):",
                        "    if not os.getenv('dodo_API_KEY'):",
                        "        raise ValueError('dodo_API_KEY is not set')",
                        "    if not os.getenv('dodo_AGENT_ID'):",
                        "        raise ValueError('dodo_AGENT_ID is not set')",
                        f"    result = client.agents.tools.run(agent_id=os.getenv('dodo_AGENT_ID'), tool_name='{tool.name}', args=kwargs)",
    ...
    async def fetch_webpage(self, agent_state: "AgentState", url: str) -> str:
        ...
        try:
            ...
            def readability_pipeline():
                response = requests.get(url, timeout=30, headers={"User-Agent": "Mozilla/5.0 (compatible; dodoBot/1.0)"})
