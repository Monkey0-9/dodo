...
from dodo.services.helpers.tool_execution_helper import (
...
class ToolExecutionSandbox:
    ...
    def generate_execution_script(self, agent_state: AgentState, wrap_print_with_markers: bool = False) -> str:
        ...
        if self.args:
            ...
            code += f"__dodo_raw_args = {{{raw_args}}}\n"
            code += "try:\n"
            code += "    from dodo.functions.ast_parsers import coerce_dict_args_by_annotations\n"
            code += f"    __dodo_func = {self.tool.name}\n"
            code += "    __dodo_annotations = getattr(__dodo_func, '__annotations__', {})\n"
            code += "    __dodo_coerced_args = coerce_dict_args_by_annotations(\n"
            code += "        __dodo_raw_args,\n"
            code += "        __dodo_annotations,\n"
            code += "        allow_unsafe_eval=True,\n"
            code += "        extra_globals=__dodo_func.__globals__,\n"
            code += "    )\n"
            for name in self.args:
                code += f"    {name} = __dodo_coerced_args.get({name!r}, {name})\n"
