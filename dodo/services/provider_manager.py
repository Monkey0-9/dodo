...
from dodo.model_aliases import get_deprecated_google_handle_replacement
from dodo.orm.provider import Provider as ProviderModel
from dodo.orm.provider_model import ProviderModel as ProviderModelORM
...
from dodo.schemas.provider_model import ProviderModel as PydanticProviderModel
from dodo.schemas.providers import Provider as PydanticProvider, ProviderCheck, ProviderCreate, ProviderUpdate
...
AUTO_MODE_HANDLES = ["dodo/auto", "dodo/auto-fast", "dodo/auto-chat"]
...
class ProviderManager:
        ...
        async with db_registry.async_session() as session:
            ...
            for llm_config in llm_models:
                ...
                if not existing:
                    ...
                    if result:
                        logger.info(f"    âœ“ Successfully created LLM model {llm_config.handle}")
                    else:
            ...
            for embedding_config in embedding_models:
                ...
                if not existing:
                    ...
                    result = await model.create_async(session, ignore_conflicts=True)
                    if result:
                        logger.info(f"    âœ“ Successfully created embedding model {embedding_config.handle}")
                    else:
    ...
    async def get_llm_config_from_handle(
        ...
        if hasattr(typed_provider, "openai_compat_base_url"):
            ...
            from dodo.schemas.providers.chatgpt_oauth import CHATGPT_CODEX_ENDPOINT
