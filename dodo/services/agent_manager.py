...
from dodo.schemas.agent import (
...
from dodo.schemas.tool import Tool as PydanticTool
from dodo.schemas.tool_rule import ContinueToolRule, RequiresApprovalToolRule, TerminalToolRule
...
from dodo.services.context_window_calculator.context_window_calculator import ContextWindowCalculator
...
from dodo.services.helpers.agent_manager_helper import (
    _apply_filters,
...
class AgentManager:
    ...
    def _resolve_tools(session, names: Set[str], ids: Set[str], org_id: str) -> Tuple[Dict[str, str], Dict[str, str]]:
        """
        Bulkâ€‘fetch all ToolModel rows matching either name âˆˆ names or id âˆˆ ids
    ...
    async def create_agent_async(
        ...
        if agent_create.agent_type == AgentType.dodo_v1_agent:
        ...
        async with db_registry.async_session() as session:
            async with session.begin():
                ...
                from dodo.services.summarizer.summarizer_config import CompactionSettings, get_default_summarizer_model
    ...
    async def validate_agent_exists_async(self, agent_id: str, actor: PydanticUser) -> None:
        ...
        Raises:
            dodoAgentNotFoundError: If agent doesn't exist or user doesn't have access
        """
        async with db_registry.async_session() as session:
    ...
    async def attach_missing_files_tools_async(self, agent_state: PydanticAgentState, actor: PydanticUser) -> PydanticAgentState:
        ...
        attached_file_tool_names = {tool.name for tool in agent_state.tools if tool.tool_type == ToolType.dodo_FILES_CORE}
        ...
        async with db_registry.async_session() as session:
            query = select(ToolModel).where(
                ...
                ToolModel.tool_type == ToolType.dodo_FILES_CORE,
            )
            result = await session.execute(query)
    ...
    async def detach_all_files_tools_async(self, agent_state: PydanticAgentState, actor: PydanticUser) -> PydanticAgentState:
        ...
        file_tool_ids = [tool.id for tool in agent_state.tools if tool.tool_type == ToolType.dodo_FILES_CORE]
        ...
        # create a shallow copy with updated tools list to avoid modifying input
        agent_state_dict = agent_state.model_dump()
        agent_state_dict["tools"] = [tool for tool in agent_state.tools if tool.tool_type != ToolType.dodo_FILES_CORE]
