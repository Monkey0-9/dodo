...
from dodo.constants import CONVERSATION_SEARCH_TOOL_NAME, DEFAULT_MESSAGE_TOOL, DEFAULT_MESSAGE_TOOL_KWARG
from dodo.log import get_logger
from dodo.orm.conversation_messages import ConversationMessage
...
from dodo.services.helpers.agent_manager_helper import validate_agent_exists_async
from dodo.settings import DatabaseChoice, settings
from dodo.utils import enforce_types, fire_and_forget
...
class MessageManager:
    ...
    def _combine_assistant_tool_messages(self, messages: List[PydanticMessage]) -> List[PydanticMessage]:
        ...
        """
        from dodo.constants import DEFAULT_MESSAGE_TOOL
    ...
    async def check_run_exists_async(self, run_id: str, actor: PydanticUser) -> bool:
        ...
        from dodo.orm.run import Run as RunModel
    ...
    async def create_many_messages_async(
        ...
        from dodo.helpers.tpuf_client import should_use_tpuf_for_messages
    ...
    @enforce_types
    @trace_method
    async def update_message_by_dodo_message_async(
        self, message_id: str, dodo_message_update: dodoMessageUpdateUnion, actor: PydanticUser
    ) -> PydanticMessage:
        """
        Updated the underlying messages table giving an update specified to the user-facing dodoMessage
        """
        message = await self.get_message_by_id_async(message_id=message_id, actor=actor)
        if dodo_message_update.message_type == "assistant_message":
            ...
            original_args["message"] = dodo_message_update.content  # override the assistant message
        ...
        elif dodo_message_update.message_type == "reasoning_message":
            update_message = MessageUpdate(content=dodo_message_update.reasoning)
        elif dodo_message_update.message_type == "user_message" or dodo_message_update.message_type == "system_message":
            update_message = MessageUpdate(content=dodo_message_update.content)
        else:
            raise ValueError(f"Unsupported message type for modification: {dodo_message_update.message_type}")

        message = await self.update_message_by_id_async(message_id=message_id, message_update=update_message, actor=actor)

        # convert back to dodoMessage
        for dodo_msg in message.to_dodo_messages(use_assistant_message=True):
            if dodo_msg.message_type == dodo_message_update.message_type:
                return dodo_msg

        # raise error if message type got modified
        raise ValueError(f"Message type got modified: {dodo_message_update.message_type}")
