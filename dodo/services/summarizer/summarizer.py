...
from dodo.errors import ContextWindowExceededError, LLMProviderOverloaded, LLMRateLimitError
...
class Summarizer:
    ...
    async def summarize(
        ...
        Args:
            ...
            new_dodo_messages: The newly added dodo messages (just appended).
    ...
    async def _partial_evict_buffer_summarization(
        ...
        """Summarization as implemented in the original Dodo loop, but using message count instead of token count.
        ...
        """
        all_in_context_messages = in_context_messages + new_dodo_messages
        ...
        from dodo.settings import summarizer_settings
    ...
    def _static_buffer_summarization(
        ...
        Args:
            ...
            new_dodo_messages: Newly added messages to append
...
def build_summary_request_text(retain_count: int, evicted_messages: List[str], in_context_messages: List[str]) -> str:
    ...
    if retain_count == 0:
        parts.append(
            "Youâ€™re a memory-recall helper for an AI that is about to forget all prior messages. Scan the conversation history and write crisp notes that capture any important facts or insights about the conversation history."
        )
    else:
        parts.append(
            f"Youâ€™re a memory-recall helper for an AI that can only keep the last {retain_count} messages. Scan the conversation history, focusing on messages about to drop out of that window, and write crisp notes that capture any important facts or insights about the human so they arenâ€™t lost."
        )
...
async def simple_summary(
    ...
    """
    from dodo.prompts.summarizer_prompt import ALL_PROMPT
    from dodo.services.telemetry_manager import TelemetryManager
    ...
    request_data = llm_client.build_request_data(AgentType.dodo_v1_agent, input_messages_obj, summarizer_llm_config, tools=[])
    try:
...
async def _get_summarizer_fallback_config(llm_config: LLMConfig, actor: User) -> LLMConfig | None:
    ...
    if endpoint_type in ("zai", "zai_coding"):
        from dodo.services.llm_router.llm_router_client import _build_baseten_config
...
async def _run_summarizer_request(
    ...
    try:
        ...
        try:
            ...
            fallback_req = fallback_client.build_request_data(AgentType.dodo_v1_agent, req_messages_obj, fallback_llm_config, tools=[])
...
async def _execute_summarizer_request(req_data: dict, req_messages_obj: list[Message], llm_config: LLMConfig, llm_client: LLMClient) -> str:
    ...
    if llm_config.model_endpoint_type in [ProviderType.anthropic, ProviderType.bedrock]:
        ...
        from dodo.interfaces.anthropic_parallel_tool_call_streaming_interface import (
