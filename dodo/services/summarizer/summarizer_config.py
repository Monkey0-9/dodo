...
from dodo.prompts.summarizer_prompt import (
    ALL_PROMPT,
    SELF_ALL_PROMPT,
    SELF_SLIDING_PROMPT,
    SLIDING_PROMPT,
...
def get_default_summarizer_model(
...
def get_default_prompt_for_mode(
    mode: Literal[
        "all", "sliding_window", "self_compact_all", "self_compact_sliding_window"
    ],
) -> str:
...
class CompactionSettings(BaseModel):
    ...
    model: str | None = Field(
        default=None,
        description=(
            "Model handle to use for sliding_window/all summarization "
            "(format: provider/model-name). If None, uses lightweight provider-specific defaults."
        ),
    )
    ...
    sliding_window_percentage: float = Field(
        ...
        description=(
            "The percentage of the context window to keep post-summarization "
            "(only used in sliding window modes)."
        ),
    )
