...
from dodo.log_context import update_log_context
...
from dodo.schemas.usage import dodoUsageStatistics, normalize_cache_tokens, normalize_reasoning_tokens
...
from dodo.utils import enforce_types
...
class RunManager:
    ...
    async def list_runs(
        ...
        async with db_registry.async_session() as session:
            ...
            if order_by == "duration":
                ...
                from dodo.services.helpers.run_manager_helper import _apply_pagination_async
        ...
        if needs_callback:
            if refresh_result_messages:
                ...
                result = dodoResponse(
                    ...
                    stop_reason=dodoStopReason(stop_reason=stop_reason_value),
    ...
    @enforce_types
    @raise_on_invalid_id(param_name="run_id", expected_prefix=PrimitiveType.RUN)
    async def get_run_usage(self, run_id: str, actor: PydanticUser) -> dodoUsageStatistics:
        ...
        total_usage = dodoUsageStatistics()
        for step in steps:
    ...
    async def get_run_messages(
        ...
        text_is_assistant_message = agent.agent_type == AgentType.dodo_v1_agent
        ...
        dodo_messages = PydanticMessage.to_dodo_messages_from_list(
        ...
        if request_config and request_config.include_return_message_types:
            ...
            dodo_messages = [msg for msg in dodo_messages if msg.message_type in include_return_message_types_set]

        return dodo_messages

    @enforce_types
    @raise_on_invalid_id(param_name="run_id", expected_prefix=PrimitiveType.RUN)
    async def get_run_request_config(self, run_id: str, actor: PydanticUser) -> Optional[dodoRequestConfig]:
        """Get the dodo request config from a run."""
        async with db_registry.async_session() as session:
            run = await RunModel.read_async(db_session=session, identifier=run_id, actor=actor, access_type=AccessType.ORGANIZATION)
            if not run:
                raise NoResultFound(f"Run with id {run_id} not found")
    ...
    async def cancel_run(self, actor: PydanticUser, agent_id: Optional[str] = None, run_id: Optional[str] = None) -> None:
        ...
        if was_pending_approval:
            ...
            if all_pending_tool_calls:
                ...
                from dodo.schemas.message import ApprovalCreate
