...
from dodo.server.rest_api.redis_stream_manager import create_background_stream_processor, redis_sse_stream_generator
...
class StreamingService:
    ...
    async def create_agent_stream(
    ...
    ) -> tuple[Optional[PydanticRun], Union[StreamingResponse, dodoResponse]]:
        """
        ...
        Args:
            ...
            request: The dodoStreamingRequest containing all request parameters
        ...
        if lock_key and not isinstance(redis_client, NoopAsyncRedisClient):
            ...
            # Same-otid retries should never acquire the lock â€” just read from Redis.
            ...
            try:
                ...
                from dodo.monitoring.load_gate import get_load_gate
            ...
            if request.background and settings.track_agent_run:
                if isinstance(redis_client, NoopAsyncRedisClient):
                    raise dodoServiceUnavailableError(
                        ...
                        f"dodo_REDIS_HOST: {settings.redis_host}, dodo_REDIS_PORT: {settings.redis_port}",
                        service_name="redis",
                    )
    ...
    async def create_agent_stream_openai_chat_completions(
        ...
        Transforms dodo's internal streaming format to match OpenAI's
        ...
        # create standard dodo stream (returns SSE-formatted stream)
        run, dodo_stream_response = await self.create_agent_stream(
            agent_id=agent_id,
            actor=actor,
            request=request,
        ...
        if isinstance(dodo_stream_response, StreamingResponseWithStatusCode):
            dodo_stream = dodo_stream_response.body_iterator
        else:
        ...
        # transform dodo SSE stream to OpenAI format (parser handles SSE strings)
        openai_stream = transformer.transform_stream(dodo_stream)
        ...
        async def error_aware_stream():
            ...
            try:
                ...
                if not saw_done and not saw_error:
                    ...
                    stop_reason = dodoStopReason(stop_reason=StopReasonType.error)
                    ...
                    stop_reason = agent_loop.stop_reason if agent_loop.stop_reason else dodoStopReason(stop_reason=StopReasonType.end_turn)
                ...
                run_status = RunStatus.failed
                stop_reason = dodoStopReason(stop_reason=StopReasonType.llm_api_error)
                error_message = dodoErrorMessage(
                    run_id=run_id,
                ...
                run_status = RunStatus.failed
                stop_reason = dodoStopReason(stop_reason=StopReasonType.invalid_llm_response)
                error_message = dodoErrorMessage(
                    run_id=run_id,
                ...
                run_status = RunStatus.failed
                stop_reason = dodoStopReason(stop_reason=StopReasonType.context_window_overflow_in_system_prompt)
                ...
                error_message = dodoErrorMessage(
                    ...
                    message=error_detail if isinstance(e, dodoError) else "An unknown error occurred with the LLM streaming request.",
                    detail=error_detail,
                )
                error_data = {"error": error_message.model_dump()}
                ...
                if run_id and self.runs_manager and run_status is None and not saw_done:
                    if is_background:
                        logger.warning(
                            ...
                            f"Skipping run update â€” background stream processor will handle terminal state."
                        )
                    else:
                ...
                if run_id and self.runs_manager and run_status:
                    # Extract stop_reason enum value from dodoStopReason object
    ...
    def _is_token_streaming_compatible(self, agent: AgentState) -> bool:
        ...
        google_dodo_v1 = agent.agent_type == AgentType.dodo_v1_agent and agent.llm_config.model_endpoint_type in [
            "google_ai",
            "google_vertex",
        ]
        return base_compatible or google_dodo_v1
    ...
    def _extract_sse_error_type(chunk: str) -> Optional[str]:
        """Extract dodo stream error_type from an SSE error chunk."""
        if not ("\nevent: error" in chunk or chunk.startswith("event: error")):
            return None
    ...
    async def _create_run(
        self, agent_id: str, request: dodoStreamingRequest, run_type: str, actor: User, conversation_id: Optional[str] = None
    ) -> PydanticRun:
        ...
        run = await self.runs_manager.create_run(
            pydantic_run=PydanticRun(
                ...
                request_config=dodoRequestConfig.from_dodo_request(request),
            ),
            actor=actor,
        )
        return run
...
class OpenAIChatCompletionsStreamTransformer:
    """
    Transforms dodo streaming messages into OpenAI ChatCompletionChunk format.
        ...
        try:
            ...
            if sse_string.startswith("data: "):
                ...
                if message_type == "assistant_message":
                    ...
                    return dodoUsageStatistics(**data)
    ...
    async def transform_stream(self, dodo_stream: AsyncIterator) -> AsyncIterator[str]:
        """
        Transform dodo stream to OpenAI ChatCompletionChunk SSE format.

        Args:
            dodo_stream: Async iterator of dodo messages (may be SSE strings or objects)

        Yields:
            SSE-formatted strings: "data: {json}\n\n"
        """
        try:
            async for raw_chunk in dodo_stream:
    ...
    async def _process_assistant_message(self, message: AssistantMessage) -> AsyncIterator[str]:
        ...
        Args:
            message: dodo AssistantMessage with content
