...
from dodo.otel.tracing import log_attributes, trace_method
from dodo.schemas.openai.chat_completion_response import ChatCompletionResponse
from dodo.server.rest_api.dependencies import HeaderParams
...
class OpenAIBackcompatUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "openai.openai_object":
            from dodo.openai_backcompat.openai_object import OpenAIObject
...
def validate_function_response(
    ...
    """Check to make sure that a function used by dodo returned a valid response. Truncates to return_char_limit if necessary.
...
def list_agent_config_files(sort="last_modified"):
    ...
    agent_dir = os.path.join(dodo_DIR, "agents")
...
def list_human_files():
    ...
    defaults_dir = os.path.join(dodo.__path__[0], "humans", "examples")
    user_dir = os.path.join(dodo_DIR, "humans")

    dodo_defaults = os.listdir(defaults_dir)
    dodo_defaults = [os.path.join(defaults_dir, f) for f in dodo_defaults if f.endswith(".txt")]
    ...
    if os.path.exists(user_dir):
        ...
        user_added = []
    return dodo_defaults + user_added


def list_persona_files():
    """List all personas files"""
    defaults_dir = os.path.join(dodo.__path__[0], "personas", "examples")
    user_dir = os.path.join(dodo_DIR, "personas")
...
def get_friendly_error_msg(function_name: str, exception_name: str, exception_message: str):
    from dodo.constants import MAX_ERROR_MESSAGE_CHAR_LIMIT
...
def log_telemetry(logger: Logger, event: str, **kwargs):
    ...
    """
    from dodo.settings import log_settings
...
def safe_create_file_processing_task(coro, file_metadata, server, actor, logger: Logger, label: str = "file processing task"):
    ...
    """
    from dodo.schemas.enums import FileProcessingStatus
...
def is_1_0_sdk_version(headers: HeaderParams):
    ...
    2. If user_agent is provided and in the format
        @dodo-ai/dodo-client/version (node) or
        dodo-client/version (python)
    """
    from dodo.log import get_logger
    ...
    if len(parts) == 3:
        # Format: @dodo-ai/dodo-client/version
        if parts[0] != "@dodo-ai" or parts[1] != "dodo-client":
            return False
    elif len(parts) == 2:
        # Format: dodo-client/version
        if parts[0] != "dodo-client":
            return False
    else:
        return False
...
async def decrypt_agent_secrets(agents: list) -> list:
    ...
    async def decrypt_env_var(env_var):
        from dodo.orm.agent import ENCRYPTED_PLACEHOLDER
