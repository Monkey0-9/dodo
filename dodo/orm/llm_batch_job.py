...
from dodo.orm.custom_columns import CreateBatchResponseColumn, PollBatchResponseColumn
from dodo.orm.mixins import OrganizationMixin
from dodo.orm.sqlalchemy_base import SqlalchemyBase
from dodo.schemas.enums import JobStatus, ProviderType
from dodo.schemas.llm_batch_job import LLMBatchJob as PydanticLLMBatchJob
...
class LLMBatchJob(SqlalchemyBase, OrganizationMixin):
    ...
    dodo_batch_job_id: Mapped[str] = mapped_column(
        String, ForeignKey("jobs.id", ondelete="CASCADE"), nullable=False, doc="ID of the dodo batch job"
    )
