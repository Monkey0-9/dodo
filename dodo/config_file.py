...
This maps to environment variables:
    dodo_TELEMETRY_ENABLE_DATADOG=true
    dodo_PG_HOST=localhost
    OPENAI_API_KEY=sk-xxx
    ANTHROPIC_API_KEY=sk-yyy
    E2B_API_KEY=xxx
    MCP_DISABLE_STDIO=true
    DD_SITE=us5.datadoghq.com
    DD_SERVICE=dodo-server

Config file locations (in order of precedence):
    1. ~/.dodo/conf.yaml
    2. ./conf.yaml
    3. dodo_CONFIG_PATH environment variable
"""
...
DEFAULT_USER_CONFIG = Path.home() / ".dodo" / "conf.yaml"
...
def load_config_file(config_path: str | Path | None = None) -> dict[str, Any]:
    ...
    env_path = os.environ.get("dodo_CONFIG_PATH")
...
def config_to_env_vars(config: dict[str, Any]) -> dict[str, str]:
    ...
    # Handle 'dodo' section with dodo_ prefix
    if "dodo" in config:
        _flatten_with_prefix(config["dodo"], "dodo", env_vars)
