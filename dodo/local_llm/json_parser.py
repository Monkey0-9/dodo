...
def add_missing_heartbeat(llm_json):
    ...
    Basically, if dodo is calling a function (not send_message) immediately after the user sending a message,
...
def clean_and_interpret_send_message_json(json_string):
    from dodo.local_llm.constants import INNER_THOUGHTS_KWARG, VALID_INNER_THOUGHTS_KWARGS
...
def clean_json(raw_llm_output, messages=None, functions=None):
    ...
    raise LLMJSONParsingError(f"Failed to decode valid dodo JSON from LLM output:\n=====\n{raw_llm_output}\n=====")
