...
from dodo.adapters.dodo_llm_stream_adapter import dodoLLMStreamAdapter
...
from dodo.errors import ContextWindowExceededError, InsufficientCreditsError, LLMError
from dodo.helpers import ToolRulesSolver
from dodo.helpers.datetime_helpers import get_utc_time, get_utc_timestamp_ns, ns_to_ms
from dodo.helpers.reasoning_helper import scrub_inner_thoughts_from_messages
...
from dodo.schemas.step import Step, StepProgression
...
from dodo.services.credit_verification_service import CreditVerificationService
...
class dodoAgentV2(BaseAgentV2):
    """
    Abstract base class for the dodo agent loop, handling message management,
    ...
    async def step(
        ...
        result = dodoResponse(messages=response_dodo_messages, stop_reason=self.stop_reason, usage=self.usage)
        if run_id:
    ...
    async def stream(
        ...
        if stream_tokens:
            llm_adapter = dodoLLMStreamAdapter(
                llm_client=self.llm_client,
                llm_config=self.agent_state.llm_config,
                call_type=LLMCallType.agent_step,
                agent_id=self.agent_state.id,
        ...
        if run_id:
            ...
            if not self.stop_reason:
                self.stop_reason = dodoStopReason(stop_reason=StopReasonType.end_turn.value)
            result = dodoResponse(messages=dodo_messages, stop_reason=self.stop_reason, usage=self.usage)
            if self.job_update_metadata is None:
                self.job_update_metadata = {}
            self.job_update_metadata["result"] = result.model_dump(mode="json")
    ...
    async def _step(
        ...
        try:
            ...
            if tool_call is None and llm_adapter.tool_call is None:
                self.stop_reason = dodoStopReason(stop_reason=StopReasonType.no_tool_call.value)
            ...
            if llm_adapter.supports_token_streaming():
                if persisted_messages[-1].role != "approval":
                    tool_return = [msg for msg in persisted_messages if msg.role == "tool"][-1].to_dodo_messages()[0]
    ...
    def _initialize_state(self):
        ...
        self.usage = dodoUsageStatistics()
        self.last_step_usage: dodoUsageStatistics | None = None  # Per-step usage for Step token details
    ...
    async def _step_checkpoint_finish(
        ...
        if logged_step:
            ...
            # Build detailed token breakdowns from per-step dodoUsageStatistics
    ...
    def _update_global_usage_stats(self, step_usage_stats: dodoUsageStatistics):
    ...
    async def _handle_ai_response(
        ...
        if is_denial:
            ...
            stop_reason = None
            tool_call_messages = create_dodo_messages_from_llm_response(
                agent_id=agent_state.id,
        ...
        if not is_approval and tool_rules_solver.is_requires_approval_tool(tool_call_name):
            ...
            stop_reason = dodoStopReason(stop_reason=StopReasonType.requires_approval.value)
        else:
    ...
    async def summarize_conversation_history(
        self,
        in_context_messages: list[Message],
        new_dodo_messages: list[Message],
        ...
        latest_messages = in_context_messages + new_dodo_messages
            ...
            try:
                if force or (total_tokens and total_tokens > self.agent_state.llm_config.context_window):
                    ...
                    new_in_context_messages, _updated = await self.summarizer.summarize(
                        in_context_messages=in_context_messages,
                        new_dodo_messages=new_dodo_messages,
                        force=True,
                        clear=True,
                        run_id=run_id,
                        step_id=step_id,
                ...
                new_in_context_messages = in_context_messages + new_dodo_messages
        else:
            new_in_context_messages = in_context_messages + new_dodo_messages
    ...
    async def _log_request(
        ...
        if request_start_timestamp_ns:
            ...
            if request_span:
                request_span.add_event(name="dodo_request_ms", attributes={"duration_ms": ns_to_ms(duration_ns)})
