...
from dodo.schemas.step import StepProgression
...
from dodo.services.conversation_manager import ConversationManager
from dodo.services.helpers.tool_parser_helper import runtime_override_tool_json_schema
from dodo.services.llm_router import get_llm_routing_client
from dodo.services.provider_manager import AUTO_MODE_HANDLES
from dodo.services.summarizer.compact import compact_messages
from dodo.services.summarizer.summarizer_config import CompactionSettings
from dodo.services.summarizer.summarizer_sliding_window import count_tokens
from dodo.services.summarizer.thresholds import get_compaction_trigger_threshold
...
class dodoAgentV3(dodoAgentV2):
    ...
    * Support Gemini / OpenAI client
    ...
    def _compute_tool_return_truncation_chars(self) -> int:
        ...
        Heuristic: ~20% of context window Ã— 4 chars/token, minimum 5k chars.
    ...
    async def build_request(
        ...
        blocks, and client-side tools â€” matching the real execution path in step().
        ...
        response = self._step(
            ...
            llm_adapter=dodoLLMRequestAdapter(
    ...
    async def step(
        ...
        Returns:
            dodoResponse: Complete response with all messages and metadata
        """
        self._initialize_state()
        self.conversation_id = conversation_id
        self.client_tools = client_tools or []
        ...
        for i in range(max_steps):
            ...
            if credit_task is not None:
                if not await credit_task:
                    ...
                    self.stop_reason = dodoStopReason(stop_reason=StopReasonType.insufficient_credits)
                    break
                credit_task = None
            ...
            async for chunk in response:
                response_dodo_messages.append(chunk)
            ...
            #        new_dodo_messages=self.response_messages,
            ...
            if i == max_steps - 1 and self.stop_reason is None:
                self.stop_reason = dodoStopReason(stop_reason=StopReasonType.max_steps.value)
        ...
        if self.stop_reason is None:
            self.stop_reason = dodoStopReason(stop_reason=StopReasonType.end_turn.value)

        # construct the response
        response_dodo_messages = Message.to_dodo_messages_from_list(
            self.response_messages,
        ...
        if include_return_message_types:
            response_dodo_messages = [m for m in response_dodo_messages if m.message_type in include_return_message_types]
        ...
        result = dodoResponse(
            messages=response_dodo_messages,
    ...
    async def stream(
        ...
        try:
            ...
            for i in range(max_steps):
                ...
                async for chunk in response:
                    ...
                    if isinstance(chunk, dodoMessage) and (not chunk.id or not chunk.otid) and not is_compaction_event:
                        self.logger.warning(
            ...
            if self.stop_reason is None:
                ...
                if isinstance(e, SystemPromptTokenExceededError):
                    self.stop_reason = dodoStopReason(stop_reason=StopReasonType.context_window_overflow_in_system_prompt.value)
                elif isinstance(e, LLMError):
                    self.stop_reason = dodoStopReason(stop_reason=StopReasonType.llm_api_error.value)
                else:
                    self.stop_reason = dodoStopReason(stop_reason=StopReasonType.error.value)
            ...
            if first_chunk:
                ...
                error_message = dodoErrorMessage(
                    run_id=run_id,
        ...
        try:
            ...
            if run_id:
                # Filter out dodoStopReason from messages (only valid in dodoStreamingResponse, not dodoResponse)
                filtered_messages = [m for m in response_dodo_messages if not isinstance(m, dodoStopReason)]
                result = dodoResponse(
    ...
    def _create_summary_result_message(
        ...
        Returns:
            List of dodoMessage objects to yield to the client
        """
        if include_compaction_messages:
            ...
            messages = list(Message.to_dodo_messages(summary_message))
    ...
    async def _step(
    ...
    ) -> AsyncGenerator[dodoMessage | dict, None]:
        """
        ...
        Yields:
            dodoMessage or dict: Chunks for streaming mode, or request data for dry_run
        """
        ...
        try:
            ...
            if approval_request and approval_response:
                ...
                if not tool_calls and not tool_call_denials and not tool_returns:
                    ...
                    self.should_continue = False
                    self.stop_reason = dodoStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
                    return
                ...
                if run_id and await self._check_run_cancellation(run_id):
                    self.should_continue = False
                    self.stop_reason = dodoStopReason(stop_reason=StopReasonType.cancelled.value)
                    ...
                    try:
                        ...
                        if isinstance(e, ContextWindowExceededError) and llm_request_attempt < summarizer_settings.max_summarizer_retries:
                            ...
                            try:
                                ...
                                self.should_continue = False
                                self.stop_reason = dodoStopReason(
                ...
                if tool_calls:
                    ...
                    for tr in response_tool_returns:
                        ...
                        if tr.tool_call_id is None and tr.tool_returns:
                            continue
                        tool_return_dodo = tr.to_dodo_messages()[0]
                        if include_return_message_types is None or tool_return_dodo.message_type in include_return_message_types:
                            yield tool_return_dodo
            else:
                # TODO: modify this use step_response_messages
                filter_user_messages = [m for m in new_messages if m.role != "user"]
                dodo_messages = Message.to_dodo_messages_from_list(
    ...
    async def _handle_ai_response(
    ...
    ) -> tuple[list[Message], bool, dodoStopReason | None]:
        """
        ...
        if not tool_calls and not tool_call_denials and not tool_returns:
            ...
            if content is None or len(content) == 0:
                ...
                if uncalled:
                    ...
                    from dodo.server.rest_api.utils import create_heartbeat_system_message
            ...
            # Case 1b: No tool call but has content
                ...
                assistant_message = create_dodo_messages_from_llm_response(
                    agent_id=self.agent_state.id,
                    model=self.agent_state.llm_config.model,
                    function_name=None,
        ...
        if not is_approval_response:
            ...
            if requested_tool_calls:
                ...
                return messages_to_persist, False, dodoStopReason(stop_reason=StopReasonType.requires_approval.value)
        ...
        persisted_stop_reasons: list[dodoStopReason | None] = []
        ...
        for idx, spec in enumerate(exec_specs):
            ...
            if has_prefill_error:
                cont = False
                _hb_reason = None
                sr = dodoStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
            else:
    ...
    def _decide_continuation(
        ...
        finish_reason: str | None = None,
    ) -> tuple[bool, str | None, dodoStopReason | None]:
        """
        ...
        stop_reason: dodoStopReason | None = None

        if tool_call_name is None:
            # No tool call â€“ if there are required-before-exit tools uncalled, keep stepping
            ...
            # No required tools remaining â†’ end turn
            # Check if the LLM hit max_tokens (finish_reason == "length")
            if finish_reason == "length":
                return False, None, dodoStopReason(stop_reason=StopReasonType.max_tokens_exceeded.value)
            return False, None, dodoStopReason(stop_reason=StopReasonType.end_turn.value)
        else:
            if tool_rule_violated:
                ...
                if tool_rules_solver.is_terminal_tool(tool_call_name):
                    stop_reason = dodoStopReason(stop_reason=StopReasonType.tool_rule.value)
                    continue_stepping = False
                ...
                # â€“ hard stop overrides â€“
                if is_final_step:
                    continue_stepping = False
                    stop_reason = dodoStopReason(stop_reason=StopReasonType.max_steps.value)
                else:
                    uncalled = tool_rules_solver.get_uncalled_required_tools(available_tools=set([t.name for t in agent_state.tools]))
                    if uncalled:
                        continue_stepping = True
                        ...
                        stop_reason = None  # reset â€“ weâ€™re still going
