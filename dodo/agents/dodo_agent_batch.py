...
from dodo.agents.helpers import _prepare_in_context_messages_async
...
from dodo.jobs.types import RequestStatusUpdateInfo, StepStatusUpdateInfo
...
async def execute_tool_wrapper(params: ToolExecutionParams) -> tuple[str, ToolExecutionResult]:
    """
    Executes the tool in an outâ€‘ofâ€‘process worker and returns:
...
class dodoAgentBatch(BaseAgent):
    def __init__(
        self,
    ...
    async def step_until_request(
        self,
        batch_requests: List[dodoBatchRequest],
        dodo_batch_job_id: str,
        ...
        llm_batch_job = await self.batch_manager.create_llm_batch_job_async(
            ...
            dodo_batch_job_id=dodo_batch_job_id,
        )
        ...
        return dodoBatchResponse(
            dodo_batch_id=llm_batch_job.dodo_batch_job_id,
    ...
    @trace_method
    async def resume_step_after_request(self, dodo_batch_id: str, llm_batch_id: str) -> dodoBatchResponse:
        ...
        if len(next_reqs) == 0:
            await self.job_manager.update_job_by_id_async(
                job_id=dodo_batch_id, job_update=JobUpdate(status=JobStatus.completed), actor=self.actor
            )
            return dodoBatchResponse(
                dodo_batch_id=llm_batch_job.dodo_batch_job_id,
                last_llm_batch_id=llm_batch_job.id,
        ...
        return await self.step_until_request(
            ...
            dodo_batch_job_id=dodo_batch_id,
    ...
    async def _bulk_rethink_memory_async(self, params: List[ToolExecutionParams]) -> Sequence[tuple[str, ToolExecutionResult]]:
        ...
        for param in params:
            ...
            # TODO: Move 'rethink_memory' to a native dodo tool that we control
    ...
    async def _prepare_next_iteration_async(
        ...
        ctx: _ResumeContext,
        msg_map: Dict[str, List[Message]],
    ) -> Tuple[List[dodoBatchRequest], Dict[str, AgentStepState]]:
        # who continues?
        ...
        batch_reqs: List[dodoBatchRequest] = []
        ...
        # extend inâ€‘context ids when necessary
    ...
    def _prepare_tools_per_agent(agent_state: AgentState, tool_rules_solver: ToolRulesSolver) -> List[dict]:
        tools = [t for t in agent_state.tools if t.tool_type in {ToolType.CUSTOM, ToolType.dodo_CORE, ToolType.dodo_MEMORY_CORE}]
    ...
    ) -> AsyncGenerator[Union[dodoMessage, LegacydodoMessage, MessageStreamStatus], None]:
        raise NotImplementedError
