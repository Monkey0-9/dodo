...
from dodo.server.rest_api.utils import create_approval_response_message_from_input, create_input_messages
from dodo.services.message_manager import MessageManager

logger = get_logger(__name__)


def _create_dodo_response(
    new_in_context_messages: list[Message],
    use_assistant_message: bool,
    usage: dodoUsageStatistics,
    stop_reason: Optional[dodoStopReason] = None,
    include_return_message_types: Optional[List[MessageType]] = None,
) -> dodoResponse:
    """
    Converts the newly created/persisted messages into a dodoResponse.
    """
    ...
    # Convert to dodo messages first
    response_messages = Message.to_dodo_messages_from_list(
    ...
    return dodoResponse(messages=response_messages, stop_reason=stop_reason, usage=usage)
...
def _safe_load_tool_call_str(tool_call_args_str: str) -> dict:
    """Lenient JSON â†’ dict with fallback to eval on assertion failure."""
...
def merge_and_validate_prefilled_args(tool: "Tool", llm_args: Dict[str, Any], prefilled_args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    from dodo.schemas.tool import Tool  # local import to avoid circulars in type hints
