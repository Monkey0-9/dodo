...
from dodo.constants import DEFAULT_MAX_STEPS, NON_USER_MSG_PREFIX, PRE_EXECUTION_MESSAGE_ARG, REQUEST_HEARTBEAT_PARAM
from dodo.helpers.datetime_helpers import get_utc_time
from dodo.helpers.tool_execution_helper import add_pre_execution_message, enable_strict_mode, remove_request_heartbeat
from dodo.interfaces.openai_chat_completions_streaming_interface import OpenAIChatCompletionsStreamingInterface
...
class VoiceAgent(BaseAgent):
    ...
    async def step(self, input_messages: List[MessageCreate], max_steps: int = DEFAULT_MAX_STEPS) -> dodoResponse:
    ...
    async def step_stream(self, input_messages: List[MessageCreate], max_steps: int = DEFAULT_MAX_STEPS) -> AsyncGenerator[str, None]:
        ...
        if agent_state.llm_config.model_endpoint_type == "anthropic":
            ...
            raise ValueError("dodo voice agents are only compatible with OpenAI or Anthropic.")
        ...
        dodo_message_db_queue = await create_input_messages(
        ...
        for _ in range(max_steps):
            ...
            openai_messages = convert_in_context_dodo_messages_to_openai(in_context_messages, exclude_system_messages=True)
        ...
        await self._rebuild_context_window(summarizer, in_context_messages, dodo_message_db_queue)
    ...
    async def _handle_ai_response(
        ...
        dodo_message_db_queue: List[Any],
    ) -> bool:
        """
        ...
        if streaming_interface.content_buffer:
            ...
            dodo_message_db_queue.extend(assistant_msgs)
        ...
        if streaming_interface.tool_call_happened:
            ...
            dodo_message_db_queue.extend(tool_call_messages)
    ...
    async def _rebuild_context_window(
        self, summarizer: Summarizer, in_context_messages: List[Message], dodo_message_db_queue: List[Message]
    ) -> None:
        new_dodo_messages = await self.message_manager.create_many_messages_async(dodo_message_db_queue, actor=self.actor)

        # TODO: Make this more general and configurable, less brittle
        new_in_context_messages, _updated = await summarizer.summarize(
            in_context_messages=in_context_messages, new_dodo_messages=new_dodo_messages
        )
    ...
    def _build_tool_schemas(self, agent_state: AgentState, external_tools_only=True) -> List[Tool]:
        if external_tools_only:
            tools = [
                ...
                if t.tool_type in {ToolType.CUSTOM, ToolType.dodo_FILES_CORE, ToolType.dodo_BUILTIN, ToolType.EXTERNAL_MCP}
            ]
        else:
        ...
        search_memory_utterance_description = (
            ...
            "For example: 'Let me double-check my notesâ€”one moment, please.'"
        )
        ...
        search_memory_json = Tool(
            ...
            function=enable_strict_mode(  # strict mode based on config
                add_pre_execution_message(  # injects pre_exec_msg   âœ“
                    {
                        "name": "search_memory",
                        "description": (
                            ...
                            "â€¢ `convo_keyword_queries`   â€“ extra names/IDs if the request is vague.\n"
                            "â€¢ `start_minutes_ago` / `end_minutes_ago` â€“ limit results to a recent time window."
                        ),
                        "parameters": {
                            "type": "object",
                            "properties": {
