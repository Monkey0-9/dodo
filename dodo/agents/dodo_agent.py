...
from dodo.agents.ephemeral_summary_agent import EphemeralSummaryAgent
from dodo.agents.helpers import (
    _build_rule_violation_result,
    _create_dodo_response,
    _pop_heartbeat,
...
from dodo.errors import ContextWindowExceededError, LLMError
from dodo.helpers import ToolRulesSolver
from dodo.helpers.datetime_helpers import AsyncTimer, get_utc_time, get_utc_timestamp_ns, ns_to_ms
...
from dodo.interfaces.anthropic_streaming_interface import AnthropicStreamingInterface
...
from dodo.services.step_manager import NoopStepManager, StepManager
...
from dodo.utils import log_telemetry, validate_function_response
...
class dodoAgent(BaseAgent):
    ...
    async def step_stream_no_tokens(
            ...
            if in_context_messages[-1].role == "approval":
                ...
                try:
                    ...
                    try:
                        ...
                        stop_reason = dodoStopReason(stop_reason=StopReasonType.invalid_llm_response.value)
                        raise e
                    ...
                    if not response.choices[0].message.tool_calls:
                        stop_reason = dodoStopReason(stop_reason=StopReasonType.no_tool_call.value)
                    ...
                    dodo_messages = [m for m in dodo_messages if m.message_type != "approval_response_message"]
        ...
        if not agent_state.message_buffer_autoclear:
            await self._rebuild_context_window(
                in_context_messages=current_in_context_messages,
                new_dodo_messages=new_in_context_messages,
    ...
    async def _step(
        ...
        request_start_timestamp_ns: int | None = None,
        dry_run: bool = False,
    ) -> Union[tuple[list[Message], list[Message], dodoStopReason | None, dodoUsageStatistics], dict]:
        """
    ...
    async def step_stream(
        ...
        for i in range(max_steps):
            if in_context_messages[-1].role == "approval":
                ...
                # yields tool response as this is handled from dodo and not the response from the LLM provider
                ...
                try:
                    ...
                    try:
                        ...
                        stop_reason = dodoStopReason(stop_reason=StopReasonType.invalid_tool_call.value)
                        raise e
                    ...
                    persisted_messages, should_continue, stop_reason = await self._handle_ai_response(
                        ...
                        pre_computed_assistant_message_id=interface.dodo_message_id,
                        step_id=effective_step_id,
                        initial_messages=initial_messages,
                        agent_step_span=agent_step_span,
                        is_final_step=(i == max_steps - 1),
                    ...
                    # Update step with actual usage now that we have it (if step was created)
                    if logged_step:
                        # Build detailed token breakdowns from dodoUsageStatistics
    ...
    async def summarize_conversation_history(self) -> None:
        ...
        new_in_context_messages, _updated = await self.summarizer.summarize(
            in_context_messages=in_context_messages, new_dodo_messages=[], force=True
        )
    ...
    async def _create_llm_request_data_async(
        ...
        tools = [
            ...
            in {
                ToolType.CUSTOM,
                ToolType.dodo_CORE,
                ToolType.dodo_MEMORY_CORE,
                ToolType.dodo_MULTI_AGENT_CORE,
                ToolType.dodo_SLEEPTIME_CORE,
                ToolType.dodo_VOICE_SLEEPTIME_CORE,
                ToolType.dodo_BUILTIN,
                ToolType.dodo_FILES_CORE,
