...
class VoiceSleeptimeAgent(dodoAgent):
    """
    A special variant of the dodoAgent that helps with offline memory computations specifically for voice.
    """
    ...
    async def step(
        ...
        new_in_context_messages, _updated = await self.summarizer.summarize(
            in_context_messages=current_in_context_messages, new_dodo_messages=new_in_context_messages
        )
    ...
    async def _execute_tool(
        ...
        try:
            if target_tool.name == "rethink_user_memory" and target_tool.tool_type == ToolType.dodo_VOICE_SLEEPTIME_CORE:
            ...
            elif target_tool.name == "finish_rethinking_memory" and target_tool.tool_type == ToolType.dodo_VOICE_SLEEPTIME_CORE:
                return ToolExecutionResult(func_return="", status="success")
            elif target_tool.name == "store_memories" and target_tool.tool_type == ToolType.dodo_VOICE_SLEEPTIME_CORE:
