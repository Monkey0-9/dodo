...
@dataclass
class dodoConfig:
    config_path: str = os.getenv("DODO_CONFIG_PATH") or os.path.join(dodo_DIR, "config")
    ...
    recall_storage_path: str = dodo_DIR
    ...
    metadata_storage_path: str = dodo_DIR
    ...
    dodo_version: str = dodo.__version__
    ...
    @classmethod
    def load(cls, llm_config: Optional[LLMConfig] = None, embedding_config: Optional[EmbeddingConfig] = None) -> "dodoConfig":
        # avoid circular import
        from dodo.utils import printd

        # from dodo.migrate import VERSION_CUTOFF, config_is_compatible
        # if not config_is_compatible(allow_empty=True):
        #    error_message = " ".join(
        #        [
        #            f"\nYour current config file is incompatible with dodo versions later than {VERSION_CUTOFF}.",
        #            f"\nTo use dodo, you must either downgrade your dodo version (<= {VERSION_CUTOFF}) or regenerate your config using `dodo configure`, or `dodo migrate` if you would like to migrate old agents.",
        #        ]
        #    )
        ...
        if os.getenv("DODO_CONFIG_PATH"):
            config_path = os.getenv("DODO_CONFIG_PATH")
        else:
            config_path = dodoConfig.config_path
        ...
        if os.path.exists(config_path):
            ...
            config_dict = {
                ...
                "dodo_version": get_field(config, "version", "dodo_version"),
            }
    ...
    def save(self):
        ...
        # set version
        set_field(config, "version", "dodo_version", dodo.__version__)
    ...
    def create_config_dir():
        if not os.path.exists(dodo_DIR):
        ...
        for folder in folders:
            if not os.path.exists(os.path.join(dodo_DIR, folder)):
                os.makedirs(os.path.join(dodo_DIR, folder))
