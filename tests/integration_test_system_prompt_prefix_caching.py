...
class TestSystemPromptPrefixCaching:
    ...
    def test_system_prompt_stable_after_memory_tool_and_messages(self, client: dodo, agent):
        """
    ...
    def test_core_memory_recompiled_after_agent_block_edit(self, client: dodo, agent):
        """
        ...
        assert updated_context.system_prompt == initial_context.system_prompt, (
            "System prompt template should remain the same â€” only core memory content changes"
        )

    def test_system_prompt_updates_after_reset(self, client: dodo, agent):
        """
