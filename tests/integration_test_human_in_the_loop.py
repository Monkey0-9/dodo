...
from dodo_client.types import AgentState, MessageCreateParam, Tool
...
USER_MESSAGE_PARALLEL_TOOL_CALL_CONTENT = "This is an automated test message. Call the get_secret_code_tool 3 times in parallel for the following inputs: 'hello world', 'hello dodo', 'hello test', and also call the roll_dice_tool once with a 16-sided dice."
...
def approve_tool_call(client: dodo, agent_id: str, tool_call_id: str):
...
def approval_tool_fixture(client: dodo):
    """
...
@pytest.fixture(scope="function")
def dice_tool_fixture(client: dodo):
...
@pytest.fixture(scope="function")
def agent(client: dodo, approval_tool_fixture, dice_tool_fixture) -> AgentState:
    """
...
def test_parallel_tool_calling(
    ...
    assert "hello dodo" in messages[-1].tool_calls[1].arguments
