...
from dodo.errors import ErrorCode, LLMBadRequestError
...
class TestOpenAIEmbedder:
    ...
    def embedder(self, embedding_config):
        ...
        with patch("dodo.services.file_processor.embedder.openai_embedder.LLMClient.create") as mock_create:
            mock_client = Mock()
...
class TestFileProcessorWithPinecone:
    ...
    async def test_file_processor_sets_chunks_embedded_zero_with_pinecone(self):
        ...
        with patch("dodo.services.file_processor.embedder.pinecone_embedder.PINECONE_AVAILABLE", True):
            with patch("dodo.services.file_processor.embedder.pinecone_embedder.upsert_file_records_to_pinecone_index") as mock_upsert:
