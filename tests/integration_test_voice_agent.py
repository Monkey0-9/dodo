...
from dodo.agents.voice_sleeptime_agent import VoiceSleeptimeAgent
from dodo.config import dodoConfig
from dodo.constants import DEFAULT_MAX_MESSAGE_BUFFER_LENGTH, DEFAULT_MIN_MESSAGE_BUFFER_LENGTH
...
from dodo.utils import get_persona_text
from tests.utils import create_tool_from_func, wait_for_server

MESSAGE_TRANSCRIPTS = [
    "user: Hey, Iâ€™ve been thinking about planning a road trip up the California coast next month.",
    ...
    "user: Yes, please. Also, I prefer independent cafÃ©s over chains, and Iâ€™m vegan.",
    "assistant: Notedâ€”independent, vegan-friendly cafÃ©s. Anything else?",
    "user: Iâ€™d also like to listen to something upbeat, maybe a podcast or playlist suggestion.",
    "assistant: Sureâ€”perhaps an indie rock playlist or a travel podcast like â€œZero To Travel.â€",
    "user: Perfect. By the way, my birthday is June 12th, so Iâ€™ll be turning 30 on the trip.",
    ...
    "assistant: How about Vegan Treats in Santa Barbara? Theyâ€™re highly rated.",
    ...
    "user: Yes, letâ€™s do that.",
    "assistant: Iâ€™ll put together a day-by-day plan now.",
]
...
SUMMARY_REQ_TEXT = """
Youâ€™re a memory-recall helper for an AI that can only keep the last 4 messages. Scan the conversation history, focusing on messages about to drop out of that window, and write crisp notes that capture any important facts or insights about the human so they arenâ€™t lost.

(Older) Evicted Messages:

0. user: Hey, Iâ€™ve been thinking about planning a road trip up the California coast next month.
...
4. user: Yes, please. Also, I prefer independent cafÃ©s over chains, and Iâ€™m vegan.
5. assistant: Notedâ€”independent, vegan-friendly cafÃ©s. Anything else?
6. user: Iâ€™d also like to listen to something upbeat, maybe a podcast or playlist suggestion.
7. assistant: Sureâ€”perhaps an indie rock playlist or a travel podcast like â€œZero To Travel.â€
8. user: Perfect. By the way, my birthday is June 12th, so Iâ€™ll be turning 30 on the trip.
...
11. assistant: How about Vegan Treats in Santa Barbara? Theyâ€™re highly rated.
...
14. user: Yes, letâ€™s do that.
15. assistant: Iâ€™ll put together a day-by-day plan now."""
...
def server_url():
    ...
    if not os.getenv("dodo_SERVER_URL"):
        ...
        server_process = subprocess.Popen(["dodo", "server", "--port", "8283"], env={**os.environ, "PYTHONUNBUFFERED": "1"}, text=True)
...
def _assert_valid_chunk(chunk, idx, chunks):
    ...
    elif isinstance(chunk, dodoUsageStatistics):
...
async def test_summarization(disable_e2b_api_key, voice_agent, server_url):
    ...
    assert "15. assistant: Iâ€™ll put together a day-by-day plan now." in call_args[0].content[0].text
...
async def test_voice_sleeptime_agent(disable_e2b_api_key, voice_agent):
    ...
    for idx, msg in enumerate(messages):
        # 1) Print whatever â€œcontentâ€ this message carries
        if hasattr(msg, "content") and msg.content is not None:
            print(f"Message {idx} content:\n{msg.content}\n")
        # 2) If itâ€™s a ToolCallMessage, also grab its name and print the raw args
