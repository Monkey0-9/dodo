...
provider usage reporting â€” both single-chunk (OpenAI) and multi-chunk cumulative
(baseten/vLLM) patterns.

These tests collect REAL chunks from providers and feed them through the actual
`_process_chunk` â†’ `get_usage_statistics` code path.
...
from dodo.interfaces.openai_streaming_interface import (
...
def _collect_openai_chunks() -> list[ChatCompletionChunk]:
    ...
    # stream_options must match the dodo client setup:
    #   - apps/core/dodo/llm_api/openai_client.py (OpenAIClient.stream_async)
    client = OpenAI()
...
def _collect_baseten_chunks() -> list[ChatCompletionChunk]:
    ...
    # stream_options must match the dodo client setup:
    #   - apps/core/dodo/llm_api/baseten_client.py (BasetenClient.stream_async)
