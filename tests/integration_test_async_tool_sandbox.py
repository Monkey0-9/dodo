...
from dodo.schemas.environment_variables import AgentEnvironmentVariable, SandboxEnvironmentVariableCreate
from dodo.schemas.organization import Organization
from dodo.schemas.pip_requirement import PipRequirement
from dodo.schemas.sandbox_config import E2BSandboxConfig, LocalSandboxConfig, SandboxConfigCreate
...
from dodo.services.tool_sandbox.local_sandbox import AsyncToolSandboxLocal
...
os.environ["dodo_DISABLE_SQLALCHEMY_POOLING"] = "true"
...
def disable_db_pooling_for_tests():
    ...
    if "dodo_DISABLE_SQLALCHEMY_POOLING" in os.environ:
        del os.environ["dodo_DISABLE_SQLALCHEMY_POOLING"]
...
async def cowsay_tool(test_user):
    # This defines a tool for a package we definitely do NOT have in dodo
...
def list_tools_via_client() -> str:
    """
    List available tools using the dodo client available in sandbox scope.
...
async def test_local_sandbox_with_client_injection(disable_e2b_api_key, list_tools_with_client_tool, test_user, server_url):
    """Test that local sandbox can inject dodo client for tools that need it."""

    # Add dodo_API_KEY to sandbox environment
    api_key = os.getenv("dodo_API_KEY") or "test-key"
    base_url = server_url  # Use the server_url fixture

    # Pass environment variables directly to avoid encryption issues
    sandbox_env_vars = {
        "dodo_API_KEY": api_key,
        "dodo_BASE_URL": base_url,
    }
    ...
    assert sandbox.inject_dodo_client is True, "Client injection should always be enabled"
    ...
    assert "ERROR: client not available in scope" not in str(result.func_return), (
        "Client should be available in scope when dodo_API_KEY is set"
    )
...
async def test_e2b_sandbox_with_client_injection(check_e2b_key_is_set, list_tools_with_client_tool, test_user, server_url):
    """Test that E2B sandbox can inject dodo client for tools that need it."""
