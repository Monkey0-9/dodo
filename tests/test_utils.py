...
from dodo.constants import MAX_FILENAME_LENGTH
...
from dodo.server.rest_api.dependencies import HeaderParams, get_headers
from dodo.services.file_processor.chunker.line_chunker import LineChunker
from dodo.services.helpers.agent_manager_helper import safe_format
from dodo.utils import is_1_0_sdk_version, sanitize_filename, validate_function_response
...
def test_get_headers_user_id_allows_none():
    headers = get_headers(
        ...
        message_async=None,
        dodo_v1_agent=None,
        dodo_v1_agent_message_async=None,
        modal_sandbox=None,
...
def test_get_headers_user_id_rejects_invalid_format():
    with pytest.raises(dodoInvalidArgumentError, match="Invalid user ID format"):
        get_headers(
...
def test_filename_with_special_characters():
    filename = "invalid:/<>?*Æ’filename.txt"
    sanitized = sanitize_filename(filename, add_uuid_suffix=True)
    assert sanitized.startswith("Æ’filename_")
    assert sanitized.endswith(".txt")
...
async def test_get_latest_alembic_revision():
    ...
    from dodo.utils import get_latest_alembic_revision
...
def test_sdk_version_check():
    ...
    assert not is_1_0_sdk_version(HeaderParams(user_agent="dodo-client/0.0.200"))
    assert is_1_0_sdk_version(HeaderParams(user_agent="dodo-client/1.0.0a5"))
    assert not is_1_0_sdk_version(HeaderParams(user_agent="@dodo-ai/dodo-client/0.0.200"))
    assert is_1_0_sdk_version(HeaderParams(user_agent="@dodo-ai/dodo-client/1.0.0-alpha.5"))
...
def test_sanitize_null_bytes_string():
    ...
    from dodo.helpers.json_helpers import sanitize_null_bytes
