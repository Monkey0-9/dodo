...
from dodo.schemas.enums import MessageRole, ToolType
...
def other_organization(server: SyncServer):
    ...
    async def create_org():
        return await server.organization_manager.create_organization_async(pydantic_org=Organization(name="dodo"))
...
def test_deserialize_override_existing_tools(server, serialize_test_agent, default_user, weather_tool, print_tool, override_existing_tools):
    ...
    for tool_name, expected_tool_data in tool_names.items():
        ...
        if existing_tool.tool_type in {ToolType.dodo_CORE, ToolType.dodo_MULTI_AGENT_CORE, ToolType.dodo_MEMORY_CORE}:
