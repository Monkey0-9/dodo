...
1. Creating a dodoAgentV2 instance
...
from dodo.services.summarizer.summarizer import simple_summary
...
async def run_summarization(server: SyncServer, agent_state, in_context_messages, actor, force=True):
    ...
    """
    agent_loop = dodoAgentV3(agent_state=agent_state, actor=actor)
...
@pytest.mark.skipif(
    ...
    reason="Missing dodo_ANTHROPIC_API_KEY (or equivalent settings) for Anthropic integration test",
)
...
async def test_summarize_initialization_messages_only(server: SyncServer, actor, llm_config: LLMConfig):
    ...
    messages = [
        PydanticMessage(
            ...
            content=[TextContent(type="text", text="You are a helpful assistant. Your name is dodo.")],
        ),
        PydanticMessage(
            role=MessageRole.system,
...
# ======================================================================================================================
# CompactionSettings Mode Tests - Using dodoAgentV3
# ======================================================================================================================
...
async def test_summarize_with_mode(
    ...
    """
    Test summarization with different CompactionSettings modes using dodoAgentV3.
    ...
    new_dodo_messages = await server.message_manager.create_many_messages_async(new_dodo_messages, actor=actor)
...
async def test_v3_compact_uses_compaction_settings_model_and_model_settings(server: SyncServer, actor):
    """Integration test: dodoAgentV3.compact uses the LLMConfig implied by CompactionSettings.
    ...
    from dodo.services.summarizer import summarizer_all
...
async def test_v3_summarize_hard_eviction_when_still_over_threshold(
    ...
    with patch("dodo.services.summarizer.compact.count_tokens_with_tools") as mock_count_tokens:
...
async def test_sliding_window_cutoff_index_does_not_exceed_message_count(server: SyncServer, actor, llm_config: LLMConfig):
    ...
    """
    from dodo.services.summarizer.summarizer_config import CompactionSettings
    from dodo.services.summarizer.summarizer_sliding_window import summarize_via_sliding_window
...
async def test_self_sliding_window_cutoff_index_does_not_exceed_message_count(server: SyncServer, actor, llm_config: LLMConfig):
    ...
    from dodo.services.summarizer.self_summarizer import self_summarize_sliding_window
    from dodo.services.summarizer.summarizer_config import CompactionSettings
    ...
    try:
        summary, remaining_messages = await self_summarize_sliding_window(
            ...
            agent_type=AgentType.dodo_v1_agent,
            messages=messages,
...
#     # Create the agent loop using dodoAgentV3
#     agent_loop = dodoAgentV3(agent_state=agent_state, actor=actor)
...
#     from dodo.errors import SystemPromptTokenExceededError
...
#    )
#
#    # Convert to dodoMessage format for easier content inspection
#    dodo_messages = PydanticMessage.to_dodo_messages_from_list(updated_in_context_messages)
#
...
async def test_summarize_all(server: SyncServer, actor, llm_config: LLMConfig):
    ...
    """
    from dodo.services.summarizer.constants import SUMMARY_TRUNCATION_SUFFIX
    from dodo.services.summarizer.summarizer_all import summarize_all
    from dodo.services.summarizer.summarizer_config import CompactionSettings
...
async def test_summarize_self(server: SyncServer, actor, llm_config: LLMConfig):
    ...
    from dodo.llm_api.llm_client import LLMClient
    from dodo.schemas.agent import AgentType
    from dodo.services.summarizer.self_summarizer import self_summarize_all
    from dodo.services.summarizer.summarizer_config import CompactionSettings
    from dodo.services.telemetry_manager import TelemetryManager
...
async def test_self_mode_fallback(server: SyncServer, actor, llm_config: LLMConfig):
    ...
    with patch(
        "dodo.services.summarizer.compact.self_summarize_all",
...
def test_compaction_stats_embedding_in_packed_json():
    ...
    from dodo.system import package_summarize_message_no_counts
...
def test_extract_compaction_stats_from_packed_json():
    ...
    from dodo.schemas.dodo_message import CompactionStats, extract_compaction_stats_from_packed_json
...
def test_extract_compaction_stats_from_packed_json_without_stats():
    ...
    from dodo.schemas.dodo_message import extract_compaction_stats_from_packed_json
...
def test_extract_compaction_stats_from_message():
    ...
    from dodo.agents.dodo_agent_v3 import extract_compaction_stats_from_message
    from dodo.schemas.dodo_message import CompactionStats
