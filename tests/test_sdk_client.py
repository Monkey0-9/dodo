...
def agent(client: dodoSDKClient):
...
@pytest.fixture(scope="function")
def fibonacci_tool(client: dodoSDKClient):
...
@pytest.fixture(scope="function")
def preferences_tool(client: dodoSDKClient):
...
@pytest.fixture(scope="function")
def data_analysis_tool(client: dodoSDKClient):
...
@pytest.fixture(scope="function")
def persona_block(client: dodoSDKClient):
...
@pytest.fixture(scope="function")
def human_block(client: dodoSDKClient):
...
@pytest.fixture(scope="function")
def context_block(client: dodoSDKClient):
...
def test_shared_blocks(client: dodoSDKClient):
    # create a block
    block = client.blocks.create(
        label="human",
        value="username: sarah",
...
def test_read_only_block(client: dodoSDKClient):
...
def test_add_and_manage_tags_for_agent(client: dodoSDKClient):
    """
...
def test_reset_messages(client: dodoSDKClient):
...
def test_list_folders_for_agent(client: dodoSDKClient):
...
def test_list_files_for_agent(client: dodoSDKClient):
...
def test_modify_message(client: dodoSDKClient):
...
def test_list_groups_for_agent(client: dodoSDKClient):
...
def test_agent_tags(client: dodoSDKClient):
...
def test_update_agent_memory_label(client: dodoSDKClient, agent: AgentState):
...
def test_add_remove_agent_memory_block(client: dodoSDKClient, agent: AgentState):
...
def test_messages(client: dodoSDKClient, agent: AgentState):
...
def test_send_system_message(client: dodoSDKClient, agent: AgentState):
    """Important unit test since the dodo API exposes sending system messages, but some backends don't natively support it (eg Anthropic)"""
...
def test_function_return_limit(disable_e2b_api_key, client: dodoSDKClient, agent: AgentState):
...
def test_function_always_error(client: dodoSDKClient, agent: AgentState):
...
# @pytest.mark.asyncio
# async def test_send_message_parallel(client: dodoSDKClient, agent: AgentState):
#     """
...
def test_agent_creation(client: dodoSDKClient):
...
def test_many_blocks(client: dodoSDKClient):
    users = ["user1", "user2"]
...
def test_include_return_message_types(client: dodoSDKClient, agent: AgentState, message_create: str):
...
def test_base_tools_upsert_on_list(client: dodoSDKClient):
    ...
    missing_base_tools = dodo_TOOL_SET - initial_tool_names
    ...
    missing_after_upsert = dodo_TOOL_SET - updated_tool_names
    ...
    for base_tool_name in dodo_TOOL_SET:
...
def test_pydantic_inventory_management_tool(e2b_sandbox_mode, client: dodoSDKClient):
...
def test_pydantic_task_planning_tool(e2b_sandbox_mode, client: dodoSDKClient):
...
@pytest.mark.parametrize("e2b_sandbox_mode", [True, False], indirect=True)
def test_create_tool_from_function_with_docstring(e2b_sandbox_mode, client: dodoSDKClient):
...
def test_preview_payload(client: dodoSDKClient):
    temp_agent = client.agents.create(
        memory_blocks=[
            CreateBlockParam(
                label="human",
    ...
    try:
        ...
        assert "dodo" in system_msg["content"]
...
def test_agent_tools_list(client: dodoSDKClient):
...
def test_agent_tool_rules_deduplication(client: dodoSDKClient):
...
def test_add_tool_with_multiple_functions_in_source_code(client: dodoSDKClient):
...
# def test_tool_name_auto_update_with_multiple_functions(client: dodoSDKClient):
...
def test_tool_rename_with_json_schema_and_source_code(client: dodoSDKClient):
...
def test_export_import_agent_with_files(client: dodoSDKClient):
...
def test_upsert_tools(client: dodoSDKClient):
...
def test_run_list(client: dodoSDKClient):
    """Test listing runs."""
...
def test_create_agent(client: dodoSDKClient) -> None:
...
@pytest.mark.skip(reason="Not compatible with 1.0 SDK")
def test_list_all_messages(client: dodoSDKClient):
...
def test_create_agent_with_tools(client: dodoSDKClient) -> None:
...
def test_calling_tools(client: dodoSDKClient, agent: AgentState) -> None:
