...
async def test_tool_with_client_injection(disable_e2b_api_key, server: SyncServer, test_user):
    """Test that tools can access injected dodo_client and agent_id to modify agent blocks."""
...
def memory_clear(label: str, agent_id: str):
    ...
    Args:
        ...
        agent_id: The agent's ID (injected by dodo system)
    """
...
'''
    ...
    assert sandbox.inject_dodo_client == True  # Client is always injected
    ...
    assert "from dodo import dodo" in script or "import dodo" in script.lower()
