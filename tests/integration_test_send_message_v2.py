...
async def test_parallel_tool_calls(
    ...
    # CRITICAL: For TRUE parallel tool calling with dodo_v1_agent, there should be exactly ONE ToolCallMessage
    ...
    assert is_parallel, (
        ...
        f"When using dodo_v1_agent with parallel_tool_calls=True, all tool calls should be in a single message."
    )
...
async def test_conversation_non_streaming_raw_http(
    ...
    async with httpx.AsyncClient(base_url=server_url, timeout=60.0) as http_client:
        ...
        # Parse JSON response (dodoResponse)
