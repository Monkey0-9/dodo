...
from dodo.client.types import AgentState, MessageCreateParam, SystemMessage, ToolReturnMessage
...
def agent_obj(client: dodo) -> AgentState:
...
@pytest.fixture(scope="function")
def other_agent_obj(client: dodo) -> AgentState:
...
@pytest.fixture
def roll_dice_tool(client: dodo):
    def roll_dice():
        """
        Rolls a 6 sided die.
...
def test_send_message_to_agent(client: dodo, agent_obj: AgentState, other_agent_obj: AgentState):
    secret_word = "banana"
    ...
    print(f"\nâœ“ Tool call verified: {tool_return_message.tool_return[:200]}...")
...
@retry_until_success(max_attempts=5, sleep_time_seconds=2)
def test_send_message_to_agents_with_tags_simple(client: dodo):
    import uuid
...
@retry_until_success(max_attempts=5, sleep_time_seconds=2)
def test_send_message_to_agents_with_tags_complex_tool_use(client: dodo, roll_dice_tool):
    import uuid
