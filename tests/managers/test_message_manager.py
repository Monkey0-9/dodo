...
async def test_modify_dodo_message(server: SyncServer, sarah_agent, default_user):
    ...
    await server.message_manager.update_message_by_dodo_message_async(
...
async def test_convert_tool_call_messages_no_assistant_mode(server: SyncServer, sarah_agent, default_user):
    ...
    dodo_messages = message.to_dodo_messages(use_assistant_message=False)

    # should have 2 messages in reverse order: tool call message, then reasoning message
    assert len(dodo_messages) == 2
    assert dodo_messages[0].message_type == "tool_call_message"
    assert dodo_messages[1].message_type == "reasoning_message"

    # check the tool call message has all tool calls in the new field
    tool_call_msg = dodo_messages[0]
...
async def test_convert_tool_call_messages_with_assistant_mode(server: SyncServer, sarah_agent, default_user):
    ...
    dodo_messages = message.to_dodo_messages(use_assistant_message=True)
    ...
    assert len(dodo_messages) == 4
    assert dodo_messages[0].message_type == "tool_call_message"
    assert dodo_messages[1].message_type == "assistant_message"
    assert dodo_messages[2].message_type == "tool_call_message"
    assert dodo_messages[3].message_type == "reasoning_message"

    # check first tool call message (actually the last in forward order) has conversation_search
    first_tool_msg = dodo_messages[0]
...
async def test_convert_tool_call_messages_multiple_non_assistant_tools(server: SyncServer, sarah_agent, default_user):
    ...
    assert len(dodo_messages) == 3
    assert dodo_messages[0].message_type == "assistant_message"
    assert dodo_messages[1].message_type == "tool_call_message"
    assert dodo_messages[2].message_type == "reasoning_message"

    # check the tool call message has all 3 non-assistant tools
    tool_msg = dodo_messages[1]
    ...
    assert dodo_messages[0].content == "Results found!"
...
async def test_convert_single_tool_call_both_fields(server: SyncServer, sarah_agent, default_user):
    ...
    assert len(dodo_messages) == 2  # tool call + reasoning (reversed)
    tool_msg = dodo_messages[0]  # tool call is first due to reverse
    ...
    dodo_messages_assist = message.to_dodo_messages(use_assistant_message=True)

    assert len(dodo_messages_assist) == 2  # tool call + reasoning (reversed)
    tool_msg_assist = dodo_messages_assist[0]  # tool call is first due to reverse
...
async def test_convert_tool_calls_only_assistant_tools(server: SyncServer, sarah_agent, default_user):
    ...
    assert dodo_messages[0].content == "Second message"
    assert dodo_messages[1].content == "First message"
...
async def test_convert_assistant_message_with_dict_content(server: SyncServer, sarah_agent, default_user):
    ...
    dodo_messages_nested = message_nested.to_dodo_messages(use_assistant_message=True)
    assistant_msg_nested = dodo_messages_nested[0]
