...
from dodo.schemas.conversation import CreateConversation, UpdateConversation
...
async def test_list_conversation_messages(conversation_manager, server: SyncServer, sarah_agent, default_user):
    """Test listing messages from a conversation as dodoMessages."""
    from dodo.schemas.dodo_message_content import TextContent
    from dodo.schemas.message import Message as PydanticMessage
    ...
    # List conversation messages (returns dodoMessages)
    dodo_messages = await conversation_manager.list_conversation_messages(
        conversation_id=conversation.id,
        actor=default_user,
    )
    ...
    # Check message types
    message_types = [m.message_type for m in dodo_messages]
...
async def test_list_conversation_messages_pagination(conversation_manager, server: SyncServer, sarah_agent, default_user):
    ...
    dodo_messages_after = await conversation_manager.list_conversation_messages(
        conversation_id=conversation.id,
        actor=default_user,
        after=messages[0].id,
    )
    assert len(dodo_messages_after) == 4  # Should get messages 1-4
...
async def test_create_conversation_invalid_block_label(conversation_manager, server: SyncServer, charles_agent, default_user):
    ...
    with pytest.raises(dodoInvalidArgumentError) as exc_info:
        await conversation_manager.create_conversation(
            agent_id=charles_agent.id,
            conversation_create=CreateConversation(
...
async def test_list_conversation_messages_ascending_order(conversation_manager, server: SyncServer, sarah_agent, default_user):
    ...
    assert dodo_messages[0].message_type == "system_message"
    assert "Message 0" in dodo_messages[1].content
...
async def test_list_conversation_messages_descending_order(conversation_manager, server: SyncServer, sarah_agent, default_user):
    ...
    assert len(dodo_messages) == 4
    assert "Message 2" in dodo_messages[0].content
...
async def test_create_conversation_with_model_and_settings(conversation_manager, server: SyncServer, sarah_agent, default_user):
    ...
    from dodo.schemas.model import OpenAIModelSettings
...
async def test_create_conversation_schema_model_validation():
    ...
    with pytest.raises(dodoInvalidArgumentError):
