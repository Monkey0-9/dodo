...
from dodo.schemas.tool import Tool as PydanticTool, ToolUpdate
from dodo.server.db import db_registry
from dodo.server.server import SyncServer
from dodo.services.tool_schema_generator import generate_schema_for_tool_creation
...
async def test_bulk_detach_tools_nonexistent_agent(server: SyncServer, print_tool, other_tool, default_user):
    ...
    with pytest.raises(dodoAgentNotFoundError):
...
async def test_upsert_filtered_base_tools(server: SyncServer, default_user, tool_type, expected_names):
    ...
    if tool_type == ToolType.dodo_MULTI_AGENT_CORE and settings.environment == "prod":
...
async def test_list_tools_with_corrupted_tool(server: SyncServer, default_user, print_tool):
    ...
    from dodo.orm.tool import Tool as ToolModel
