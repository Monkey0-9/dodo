...
from dodo.schemas.job import Job as PydanticJob, JobUpdate
...
async def test_record_timing_invalid_job(server: SyncServer, default_user):
    """Test recording timing metrics for non-existent job raises dodoInvalidArgumentError."""
    # Try to record TTFT for non-existent job - should raise dodoInvalidArgumentError
    with pytest.raises(dodoInvalidArgumentError):
        await server.job_manager.record_ttft("nonexistent_job_id", 1_000_000_000, default_user)

    # Try to record response duration for non-existent job - should raise dodoInvalidArgumentError
    with pytest.raises(dodoInvalidArgumentError):
