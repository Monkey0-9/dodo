...
from dodo.schemas.sandbox_config import E2BSandboxConfig, SandboxConfigCreate
...
async def other_organization(server: SyncServer):
    ...
    org = await server.organization_manager.create_organization_async(pydantic_org=Organization(name="dodo"))
    yield org
...
@pytest.fixture
async def dodo_batch_job(server: SyncServer, default_user):
    """Create a batch job."""
