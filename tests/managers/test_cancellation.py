...
async def test_agent_with_tool(server: SyncServer, default_user, print_tool):
    """Create a test agent with dodo_v1_agent type (uses dodoAgentV3)."""
...
class TestMessageStateDesyncIssues:
    ...
    async def test_approval_request_message_ids_desync_with_background_token_streaming(
        ...
        print("\nâŒ CANCELLING RUN while in background + token streaming mode (MID-EXECUTION)...")
        await server.run_manager.cancel_run(
            actor=default_user,
            run_id=run.id,
        )
        ...
        if messages_in_db_not_in_agent:
            ...
            assert False, (
                ...
                f"Fix location: Check approval flow in dodo_agent_v3.py:442-486 and background\n"
...
class TestApprovalFlowCancellation:
    ...
    async def test_parallel_tool_calling_cancellation_with_denials(
        ...
        from dodo.errors import PendingApprovalError
...
class TestErrorDataPersistence:
    ...
    async def test_error_data_stored_in_run_metadata_on_background_streaming_llm_error(
        ...
        The test patches dodoAgentV3.stream to raise an LLMError, simulating
        ...
        with patch.object(dodoAgentV3, "stream", mock_stream_raises_llm_error):
        ...
        # The error is stored as a dict from dodoErrorMessage.model_dump()
    ...
    async def test_error_data_stored_on_streaming_timeout_error(
        ...
        from dodo.agents.dodo_agent_v3 import dodoAgentV3
        from dodo.errors import LLMTimeoutError
        from dodo.services.streaming_service import StreamingService
        ...
        with patch.object(dodoAgentV3, "stream", mock_stream_raises_timeout):
            run, stream_response = await streaming_service.create_agent_stream(
                agent_id=test_agent_with_tool.id,
                actor=default_user,
                request=request,
    ...
    async def test_error_data_stored_on_streaming_rate_limit_error(
        ...
        from dodo.agents.dodo_agent_v3 import dodoAgentV3
        from dodo.errors import LLMRateLimitError
        from dodo.services.streaming_service import StreamingService
        ...
        with patch.object(dodoAgentV3, "stream", mock_stream_raises_rate_limit):
            run, stream_response = await streaming_service.create_agent_stream(
                agent_id=test_agent_with_tool.id,
                actor=default_user,
                request=request,
    ...
    async def test_error_data_stored_on_streaming_auth_error(
        ...
        with patch.object(dodoAgentV3, "stream", mock_stream_raises_auth_error):
            run, stream_response = await streaming_service.create_agent_stream(
                agent_id=test_agent_with_tool.id,
                actor=default_user,
                request=request,
    ...
    async def test_error_data_stored_on_generic_exception(
        ...
        with patch.object(dodoAgentV3, "stream", mock_stream_raises_generic_error):
            run, stream_response = await streaming_service.create_agent_stream(
                agent_id=test_agent_with_tool.id,
                actor=default_user,
                request=request,
