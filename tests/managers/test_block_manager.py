...
from dodo.schemas.block import Block as PydanticBlock, BlockUpdate
...
async def test_undo_no_history(server: SyncServer, default_user):
    ...
    undo_checkpoint_block should raise a dodoInvalidArgumentError.
    """
    block_manager = BlockManager()
...
async def test_undo_first_checkpoint(server: SyncServer, default_user):
    ...
    # Attempt undo -> expect dodoInvalidArgumentError
    with pytest.raises(dodoInvalidArgumentError):
        await block_manager.undo_checkpoint_block(block_id=block.id, actor=default_user)
...
async def test_redo_no_history(server: SyncServer, default_user):
    ...
    then redo_checkpoint_block should raise dodoInvalidArgumentError.
    """
    block_manager = BlockManager()
    ...
    # Attempt to redo => expect dodoInvalidArgumentError
    with pytest.raises(dodoInvalidArgumentError):
...
async def test_redo_at_highest_checkpoint(server: SyncServer, default_user):
    ...
    redo_checkpoint_block should raise dodoInvalidArgumentError.
    """
    block_manager = BlockManager()
