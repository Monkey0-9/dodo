...
from dodo.agents.helpers import _prepare_in_context_messages_no_persist_async
...
async def agent_with_conversation(server, default_user):
    """Create a dodo_v1_agent (V3) with memory blocks, tools, and a conversation."""
...
class TestPreviewAccuracy:
    ...
    async def test_preview_matches_step_request_data(self, server, default_user, agent_with_conversation):
        ...
        # â”€â”€ 1. Preview path: build_request() â”€â”€
        ...
        # â”€â”€ 2. Step path: replicate step()'s setup, then dry_run â”€â”€
        # This mirrors what dodoAgentV3.step() does before entering _step().
        ...
        # Call _step with dry_run=True â€” identical to what step() would do
    ...
    async def test_preview_without_conversation_uses_agent_messages(self, server, default_user, agent_with_conversation):
        ...
        payload = await agent_loop.build_request(
            input_messages=input_messages,
            conversation_id=None,  # No conversation â€” uses agent-scoped messages
        )
