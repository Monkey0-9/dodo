...
class TestClientSideTools:
    ...
    def test_client_side_tool_full_flow(self, client: dodo, model: str) -> None:
        """
        ...
        try:
            ...
            print(f"  âœ“ Agent called get_secret_code tool (call_id: {tool_call_id})")
            ...
            print("  âœ“ Tool return message received with secret code")
            ...
            print(f"  âœ“ Agent continued execution (stop_reason: {response2.stop_reason})")
            ...
            print(f"  âœ“ Agent produced {len(assistant_messages_step2)} assistant message(s)")
            ...
            print(f"\nâœ“ Full flow validated successfully for {model}!")
    ...
    @pytest.mark.parametrize("model", TEST_MODELS)
    def test_client_tool_overrides_server_tool(self, client: dodo, model: str) -> None:
        """
...
'''
        ...
        try:
            ...
            print("  âœ“ Execution paused with requires_approval (client tool took precedence)")

            tool_call_id = response1.messages[-1].tool_call.tool_call_id
            assert response1.messages[-1].tool_call.name == "get_secret_code"
            print(f"  âœ“ Tool call is for 'get_secret_code' (call_id: {tool_call_id})")
            ...
            assert response2.messages[0].message_type == "tool_return_message"
            assert response2.messages[0].tool_return == SECRET_CODE
            print(f"  âœ“ Tool return contains client-provided value: {SECRET_CODE}")
            ...
            print(f"  âœ“ Agent used client-side value '{SECRET_CODE}' (not server value '{SERVER_TOOL_RETURN}')")
            ...
            print("  âœ“ Without client_tools, server tool executed directly (no approval required)")
            ...
            if tool_returns:
                ...
                print(f"  âœ“ Server tool returned: {server_return_value}")

            print(f"\nâœ“ Client tool override test passed for {model}!")
    ...
    @pytest.mark.parametrize("model", TEST_MODELS)
    def test_client_side_tool_error_return(self, client: dodo, model: str) -> None:
        """
        ...
        try:
            ...
            assert response1.stop_reason.stop_reason == "requires_approval"
            tool_call_id = response1.messages[-1].tool_call.tool_call_id
            print(f"  âœ“ Agent called tool (call_id: {tool_call_id})")
            ...
            print("  âœ“ Tool return shows error status")
            ...
            print(f"  âœ“ Agent continued execution after error (stop_reason: {response2.stop_reason})")
            ...
            print("  âœ“ Agent produced response after error")

            print(f"\nâœ“ Error handling validated successfully for {model}!")
