...
import dodo.server.rest_api.routers.v1.git_http as git_http_router
from dodo.log_context import get_log_context
from dodo.server.rest_api.middleware import LoggingMiddleware
...
class TestLogContextMiddleware:
    ...
    async def test_sync_after_push_syncs_nested_block_labels_to_postgres(self, monkeypatch):
        ...
        from dodo.settings import settings as core_settings

        monkeypatch.setattr(core_settings, "memfs_service_url", "http://memfs.test")

        with patch("dodo.services.block_manager_git.GitEnabledBlockManager", DummyGitEnabledBlockManager):
