...
- Anthropic: Requires cache_control breakpoints (â‰¥1024 tokens for Sonnet 4.5)
- Gemini: Implicit caching on 2.5 models (â‰¥1024 tokens for 2.5 Flash)

Test strategy:
1. Create agent with large memory block (>5000 tokens to exceed all thresholds)
2. Send message 1 â†’ assert cache WRITE occurred
3. Send message 2 â†’ assert cache HIT occurred
...
CACHING_TEST_CONFIGS = [
    ...
    # Anthropic: Requires cache_control, â‰¥1024 tokens for Sonnet 4.5
    pytest.param(
...
@pytest.fixture
def base_url() -> str:
    """Get the dodo server URL from environment or use default."""
    return os.getenv("dodo_SERVER_URL", "http://localhost:8283")
...
async def test_prompt_caching_cache_write_then_read(
    ...
    try:
        ...
        assert total_input_tokens >= min_tokens, f"Total input must be â‰¥{min_tokens} tokens for caching, got {total_input_tokens}"
...
async def test_anthropic_system_prompt_stability(async_client: Asyncdodo):
    ...
    try:
        ...
        if system1_str == system2_str:
            logger.info("âœ… ACTUAL SYSTEM PROMPTS SENT TO ANTHROPIC ARE IDENTICAL")
            logger.info("   â†’ Cache SHOULD work, but isn't. Issue is likely:")
            logger.info("   â†’ 1. Thinking blocks breaking cache")
            logger.info("   â†’ 2. Tool definitions changing")
            logger.info("   â†’ 3. Something else in the request changing")
        else:
            logger.error("âŒ ACTUAL SYSTEM PROMPTS SENT TO ANTHROPIC DIFFER!")
            logger.info("=" * 80)
            ...
            if "System prompt last recompiled" in diff_output:
                logger.error("âš ï¸  TIMESTAMP IS CHANGING IN ACTUAL REQUESTS!")
                logger.error("   â†’ This is the root cause of cache misses")
...
async def test_anthropic_cache_control_breakpoints(async_client: Asyncdodo):
    ...
    try:
        ...
        assert cache_creation is not None and cache_creation >= 1024, (
            f"Anthropic should create cache â‰¥1024 tokens on first message. Got {cache_creation}. This means cache_control breakpoints are NOT being added!"
        )
...
async def test_openai_automatic_caching(async_client: Asyncdodo):
    ...
    OpenAI caching is automatic, so this should just work if we have â‰¥1024 tokens.
    """
    model = "openai/gpt-4o"
    ...
    try:
        ...
        assert cached_tokens_2 is not None and cached_tokens_2 >= 1024, (
            f"OpenAI should cache â‰¥1024 tokens automatically on second message. Got {cached_tokens_2}. Automatic caching is NOT working!"
        )
...
async def test_gemini_2_5_flash_implicit_caching(async_client: Asyncdodo):
    ...
    Gemini 2.5 Flash has implicit caching (automatic) with â‰¥1024 token threshold.
    """
    ...
    try:
        ...
        assert cached_tokens is not None and cached_tokens >= 1024, (
            f"Gemini 2.5 Flash should implicitly cache â‰¥1024 tokens on second message. Got {cached_tokens}. Implicit caching is NOT working!"
        )
...
async def test_gemini_3_pro_preview_implicit_caching(async_client: Asyncdodo):
    ...
    Gemini 3 Pro Preview has implicit caching (automatic) with â‰¥2048 token threshold.
    ...
    try:
        ...
        assert max_cached >= 2048, (
            f"Gemini 3 Pro Preview should implicitly cache â‰¥2048 tokens in at least one of {len(follow_up_messages)} messages. Got max={max_cached}. Implicit caching is NOT working!"
        )
...
async def test_gemini_request_prefix_stability(async_client: Asyncdodo):
    ...
    try:
        ...
        if system_instruction_1 != system_instruction_2:
            logger.error("âŒ SYSTEM INSTRUCTIONS DIFFER!")
            ...
            if "System prompt last recompiled" in diff_output or "timestamp" in diff_output.lower():
                logger.error("âš ï¸  TIMESTAMP IN SYSTEM INSTRUCTION IS CHANGING!")
                logger.error("   â†’ This breaks Gemini implicit caching (prefix must match)")
        else:
            logger.info("âœ… System instructions are identical")
        ...
        if tool_names_1 != tool_names_2:
            logger.error("âŒ TOOL ORDER/NAMES DIFFER!")
            logger.info(f"Message 1 tools: {tool_names_1}")
            logger.info(f"Message 2 tools: {tool_names_2}")
            logger.error("   â†’ Tool order must be identical for Gemini implicit caching")
        else:
            logger.info(f"âœ… Tool order is identical ({len(tool_names_1)} tools)")
        ...
        if tools_1_str != tools_2_str:
            logger.warning("âš ï¸  Tool DEFINITIONS differ (not just order)")
            # Show a sample diff
            diff = difflib.unified_diff(
            ...
            logger.info("âœ… Tool definitions are identical")
        ...
        for i in range(min_len - 1):  # Exclude last item (user's new message)
            if contents_1[i] != contents_2[i]:
                ...
                logger.error(f"âŒ Content item {i} differs between requests!")
        ...
        if prefix_identical:
            logger.info("âœ… Content prefix matches between requests")
        ...
        if response2.usage and response2.usage.cached_input_tokens and response2.usage.cached_input_tokens > 0:
            logger.info("âœ… CACHE HIT DETECTED")
        else:
            logger.error("âŒ NO CACHE HIT - This is the issue we're debugging")
