...
def server_url() -> str:
    ...
    if not os.getenv("dodo_SERVER_URL"):
        if os.getenv("dodo_REDIS_HOST"):
            print(f"Redis is configured at {os.getenv('dodo_REDIS_HOST')}:{os.getenv('dodo_REDIS_PORT', '6379')}")
...
@pytest.mark.skipif(not os.getenv("dodo_REDIS_HOST"), reason="Redis is required for background streaming (set dodo_REDIS_HOST to enable)")
