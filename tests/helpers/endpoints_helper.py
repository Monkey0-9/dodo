...
from dodo.llm_api.helpers import unpack_inner_thoughts_from_kwargs
from dodo.schemas.block import CreateBlock
from dodo.schemas.tool_rule import BaseToolRule
...
from dodo.constants import DEFAULT_HUMAN, DEFAULT_PERSONA
from dodo.errors import InvalidInnerMonologueError, InvalidToolCallError, MissingInnerMonologueError, MissingToolCallError
...
# defaults
# NOTE: In tests we avoid using hosted dodo embeddings and instead default to OpenAI.
...
# ======================================================================================================================
# Section: dodo Message Assertions
# These functions are validating elements of parsed dodo Messsage
# ======================================================================================================================


def assert_sanity_checks(response: dodoResponse):
    assert response is not None, response
    assert response.messages is not None, response
    assert len(response.messages) > 0, response


def assert_invoked_send_message_with_keyword(messages: Sequence[dodoMessage], keyword: str, case_sensitive: bool = False) -> None:
...
def assert_invoked_function_call(messages: Sequence[dodoMessage], function_name: str) -> None:
    for message in messages:
...
def assert_inner_monologue_is_present_and_valid(messages: List[dodoMessage]) -> None:
    for message in messages:
