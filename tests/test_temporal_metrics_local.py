...
def setup_metrics():
    ...
    os.environ["DD_SERVICE"] = "dodo-temporal-test"
...
async def test_metrics_initialization():
    ...
    print(f"\nâœ“ Metrics initialized: enabled={TemporalMetrics.is_enabled()}")


@pytest.mark.asyncio
async def test_workflow_metrics():
    """Test workflow metrics recording."""
    with patch("dodo.agents.temporal.metrics.statsd") as mock_statsd:
        TemporalMetrics._initialized = False
        ...
        print("\nâœ“ Workflow metrics recorded successfully")
...
async def test_activity_metrics():
    ...
    with patch("dodo.agents.temporal.metrics.statsd") as mock_statsd:
        ...
        print("\nâœ“ Activity metrics recorded successfully")
        print(f"  - increment called {mock_statsd.increment.call_count} times")
...
async def test_metrics_with_real_dogstatsd():
    ...
    print("\nâœ“ Real metrics sent to DogStatsD at localhost:8125")
