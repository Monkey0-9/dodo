...
async def test_sync_base_providers_creates_new_providers(default_user, provider_manager):
    ...
    dodo_providers = await provider_manager.list_providers_async(name="dodo", actor=default_user)
    openai_providers = await provider_manager.list_providers_async(name="openai", actor=default_user)

    assert len(dodo_providers) == 1
    assert dodo_providers[0].name == "dodo"
    assert dodo_providers[0].provider_type == ProviderType.dodo
...
async def test_sync_base_providers_handles_none_api_key(default_user, provider_manager):
    ...
    # Mock base providers from environment (dodo doesn't need an API key)
    base_providers = [
        dodoProvider(name="dodo", api_key=None),
    ]
...
async def test_server_sync_provider_models_on_init(default_user):
    ...
    # Mock providers
    mock_dodo_provider = AsyncMock()
    mock_dodo_provider.name = "dodo"
    mock_dodo_provider.list_llm_models_async.return_value = [
        LLMConfig(
            model="dodo-model",
            model_endpoint_type="openai",  # Use valid endpoint type
            model_endpoint="https://api.dodo.com",
            context_window=8192,
            handle="dodo/dodo-model",
            provider_name="dodo",
            provider_category=ProviderCategory.base,
        )
    ]
    mock_dodo_provider.list_embedding_models_async.return_value = []
    ...
    server._enabled_providers = [mock_dodo_provider, mock_openai_provider]
    ...
    server.provider_manager.list_providers_async.return_value = [db_dodo, db_openai]
    ...
    # Verify dodo models were synced
    dodo_call = server.provider_manager.sync_provider_models_async.call_args_list[0]
    assert dodo_call.kwargs["provider"].id == "dodo_provider_id"
    assert len(dodo_call.kwargs["llm_models"]) == 1
    assert len(dodo_call.kwargs["embedding_models"]) == 0
    assert dodo_call.kwargs["organization_id"] is None
...
async def test_server_list_llm_models_byok_from_provider_api(default_user, provider_manager):
    ...
    """
    from dodo.schemas.providers import Provider
    from dodo.server.server import SyncServer
...
async def test_provider_ordering_matches_constants(default_user, provider_manager):
    ...
    from dodo.constants import PROVIDER_ORDER
    from dodo.server.server import SyncServer
...
async def test_refresh_base_provider_fails(default_user, provider_manager):
    ...
    from dodo.server.rest_api.routers.v1.providers import refresh_provider_models
    from dodo.server.server import SyncServer
...
async def test_get_model_by_handle_prioritizes_byok_over_base(default_user, provider_manager):
    ...
    from dodo.orm.provider import Provider as ProviderORM
    from dodo.schemas.providers import Provider as PydanticProvider
