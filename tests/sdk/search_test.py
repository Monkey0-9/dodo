...
def cleanup_agent_with_messages(client: dodo, agent_id: str):
    """
...
def cleanup_tool(client: dodo, tool_id: str):
    """
...
def test_passage_search_basic(client: dodo, enable_turbopuffer):
...
@pytest.mark.skipif(not settings.tpuf_api_key, reason="Turbopuffer API key not configured")
def test_passage_search_with_tags(client: dodo, enable_turbopuffer):
...
@pytest.mark.skipif(not settings.tpuf_api_key, reason="Turbopuffer API key not configured")
def test_passage_search_with_date_filters(client: dodo, enable_turbopuffer):
...
def test_message_search_basic(client: dodo, enable_message_embedding):
    ...
    try:
        ...
        # Note: The endpoint returns dodoMessageSearchResult (API schema)
...
@pytest.mark.skipif(not settings.tpuf_api_key, reason="Turbopuffer API key not configured")
def test_passage_search_pagination(client: dodo, enable_turbopuffer):
...
@pytest.mark.skipif(not settings.tpuf_api_key, reason="Turbopuffer API key not configured")
def test_passage_search_org_wide(client: dodo, enable_turbopuffer):
...
def test_tool_search_basic(client: dodo, enable_tool_embedding):
