...
def _poll_block_value(client: dodo, agent_id: str, block_label: str, expected_substring: str, timeout_seconds: float = 20.0) -> str:
...
@pytest.mark.skipif(not bool(os.getenv("dodo_MEMFS_SERVICE_URL")), reason="requires memfs service configured")
...
def test_git_http_post_push_sync_retries_transient_head_read(server_url: str, monkeypatch):
    ...
    try:
        from dodo.server.rest_api.app import server as app_server
