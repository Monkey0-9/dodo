...
def is_reasoner_model(model_handle: str, model_settings: dict) -> bool:
    ...
    - dodo/llm_api/openai_client.py:is_openai_reasoning_model
    - dodo/llm_api/anthropic_client.py:is_reasoning_model
    - dodo/llm_api/google_vertex_client.py:is_reasoning_model
    """
...
def accumulate_chunks(chunks: List[Any], verify_token_streaming: bool = False) -> List[Any]:
    ...
    if chunks and isinstance(chunks[0], str):
        ...
        for chunk in chunks:
            current_message_type = chunk.message_type
            # Skip keepalive/initial pings â€” not content messages
...
def test_agent_loop_error(
    ...
    with patch("dodo.agents.dodo_agent_v2.dodoAgentV2.step") as mock_step:
...
def test_step_stream_agent_loop_error(
    ...
    with patch("dodo.agents.dodo_agent_v2.dodoAgentV2.stream") as mock_step:
...
def wait_for_run_completion(client: dodo, run_id: str, timeout: float = 30.0, interval: float = 0.5) -> Run:
    start = time.time()
    while True:
...
def test_async_greeting_with_assistant_message(
    ...
    # NOTE: deprecated in preparation of dodo_v1_agent
    # @pytest.mark.parametrize(
    #    "llm_config",
    #    TESTED_LLM_CONFIGS,
...
def test_auto_summarize(disable_e2b_api_key: Any, client: dodo, model_config: Tuple[str, dict]):
...
def wait_for_run_status(client: dodo, run_id: str, target_status: str, timeout: float = 30.0, interval: float = 0.1) -> Run:
...
def test_inner_thoughts_toggle_interleaved(
    ...
    #     agent_id=agent_state.id,
    #     request=dodoRequest(messages=USER_MESSAGE_FORCE_REPLY),
    # )
