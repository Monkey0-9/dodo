...
This tests the actual production code path:
  SimpleLLMRequestAdapter.invoke_llm()
    â†’ llm_client.request_async_with_telemetry()
    â†’ llm_client.convert_response_to_chat_completion()
    â†’ adapter extracts from chat_completions_response.usage
    â†’ normalize_cache_tokens() / normalize_reasoning_tokens()
"""
...
from dodo.schemas.enums import AgentType, LLMCallType, MessageRole
...
async def test_openai_usage_via_adapter():
    ...
    This tests the actual production code path used by dodo_agent_v3.
    """
    ...
    request_data = client.build_request_data(AgentType.dodo_v1_agent, messages, llm_config)
...
async def test_anthropic_usage_via_adapter():
    ...
    request_data = client.build_request_data(AgentType.dodo_v1_agent, messages, llm_config, tools=[])
...
async def test_openai_prefix_caching_via_adapter():
    ...
    request_data1 = client.build_request_data(AgentType.dodo_v1_agent, messages1, llm_config)
    ...
    request_data2 = client.build_request_data(AgentType.dodo_v1_agent, messages2, llm_config)
...
async def test_anthropic_prefix_caching_via_adapter():
    ...
    request_data1 = client.build_request_data(AgentType.dodo_v1_agent, messages1, llm_config, tools=[])
    ...
    request_data2 = client.build_request_data(AgentType.dodo_v1_agent, messages2, llm_config, tools=[])
