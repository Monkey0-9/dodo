...
from dodo.client import MessageCreate, dodo
...
def client():
    """Create a dodo client for testing."""
    import threading

    # Get URL from environment or start server
    api_url = os.getenv("dodo_API_URL")
    server_url = os.getenv("dodo_SERVER_URL", "http://localhost:8283")
    ...
    # Override the base_url if the dodo_API_URL is set
    base_url = api_url if api_url else server_url
    # create the dodo client
    yield dodo(base_url=base_url, token=None)


@pytest.fixture
def agent_factory(client: dodo):
...
def test_streaming_buffering_behavior(client: dodo, agent_factory, model: str, expected_buffering: bool):
    """
    ...
    if expected_buffering:
        ...
        print(f"\nâœ“ Test PASSED: {model} shows expected buffering behavior")
    else:
        ...
        print(f"\nâœ“ Test PASSED: {model} shows expected progressive streaming")
