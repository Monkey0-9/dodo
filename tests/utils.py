...
from dodo.settings import TestSettings
...
def wipe_config():
    ...
    config_path = os.path.join(test_settings.dodo_dir, "config")
...
def wipe_dodo_home():
    """Wipes ~/.dodo (moves to a backup), and initializes a new ~/.dodo dir"""
    ...
    backup_dir = f"~/.dodo_test_backup_{timestamp}"

    # Use os.system to execute the 'mv' command
    os.system(f"mv ~/.dodo {backup_dir}")

    # Setup the initial directory
    test_settings = TestSettings()
    config_path = os.path.join(test_settings.dodo_dir, "config")
    config = dodoConfig(config_path=config_path)
    config.create_config_dir()


def configure_dodo_localllm():
    import pexpect

    wipe_config()
    child = pexpect.spawn("dodo configure")
...
def configure_dodo(enable_openai=False, enable_azure=False):
    if enable_openai:
        raise NotImplementedError
    elif enable_azure:
        raise NotImplementedError
    else:
        configure_dodo_localllm()
