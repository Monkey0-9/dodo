...
from dodo.constants import CORE_MEMORY_LINE_NUMBER_WARNING
from dodo.schemas.block import Block, FileBlock
from dodo.schemas.enums import AgentType
from dodo.schemas.memory import ChatMemory, Memory
...
def test_compile_standard_blocks_metadata_and_values():
    m = Memory(
        agent_type=AgentType.dodo_agent,
        blocks=[
...
def test_compile_line_numbered_blocks_sleeptime():
    ...
    assert "1â†’ line1" not in out and "2â†’ line2" not in out
    assert "line1" in out and "line2" in out  # Content should still be there


def test_compile_line_numbered_blocks_dodo_v2():
    m = Memory(agent_type=AgentType.dodo_v2_agent, blocks=[Block(label="notes", value="a\nb", limit=100)])
    out = m.compile()
    # Without llm_config, should NOT show line numbers (backward compatibility)
    assert "1â†’ a" not in out and "2â†’ b" not in out
...
def test_compile_line_numbered_blocks_with_anthropic():
    ...
    m = Memory(agent_type=AgentType.dodo_v1_agent, blocks=[Block(label="notes", value="line1\nline2", limit=100)])
    ...
    assert "1â†’ line1" in out and "2â†’ line2" in out
...
def test_compile_empty_returns_empty_string():
    m = Memory(agent_type=AgentType.dodo_agent, blocks=[])
    assert m.compile() == ""


def test_tool_usage_rules_inclusion_and_order():
    m = Memory(agent_type=AgentType.dodo_agent, blocks=[Block(label="a", value="b", limit=100)])
...
def test_directories_common_includes_files_and_metadata():
    ...
    m = Memory(agent_type=AgentType.dodo_agent, blocks=[Block(label="x", value="y", limit=10)], file_blocks=[fb])
...
def test_directories_common_omits_empty_value():
    ...
    m = Memory(agent_type=AgentType.dodo_agent, blocks=[], file_blocks=[fb])
...
def test_file_blocks_duplicates_pruned_and_warning(caplog):
    ...
    with caplog.at_level("WARNING", logger="dodo.schemas.memory"):
        m = Memory(agent_type=AgentType.dodo_agent, blocks=[], file_blocks=[fb1, fb2])
    out = m.compile(sources=[src])
...
async def test_compile_async_matches_sync():
    m = Memory(agent_type=AgentType.dodo_agent, blocks=[Block(label="a", value="b", limit=10)])
...
def test_read_only_metadata_in_file_and_block():
    src = make_source("src1", "project")
    fb = FileBlock(label="fileA", value="data", limit=100, file_id="f1", source_id="src1", is_open=True, read_only=True)
    m = Memory(agent_type=AgentType.dodo_agent, blocks=[Block(label="x", value="y", limit=10, read_only=True)], file_blocks=[fb])
    out = m.compile(sources=[src])
...
def test_compile_git_structured_dedup_client_skills():
    ...
    # Agent skill wins â€” description comes from agent block, not client
        ...
        blocks=[
            ...
            Block(
                ...
                value=(
                    ...
                    "`path` -> `location`; `ClientSkillSchema` on `dodoRequest`"
                ),
                limit=1000,
