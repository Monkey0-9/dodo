...
from dodo.server.rest_api.dependencies import ExperimentalParams
...
class TestOpenAIWSSessionManager:
    ...
    async def test_stream_responses_yields_events_and_stops_on_terminal(self):
        ...
        with patch("dodo.llm_api.openai_ws_session.AsyncOpenAI") as MockClient:
...
class TestAdapterWSFlag:
    ...
    def test_dodo_llm_stream_adapter_stores_flag(self):
        from dodo.adapters.dodo_llm_stream_adapter import dodoLLMStreamAdapter

        mock_client = MagicMock()
        mock_config = MagicMock()

        adapter = dodoLLMStreamAdapter.__new__(dodoLLMStreamAdapter)
        adapter.__init__(
    ...
    async def test_aclose_default_is_noop(self):
        ...
        class ConcreteAdapter(dodoLLMAdapter):
