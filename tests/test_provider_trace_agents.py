...
- dodoAgent (v1), dodoAgentV2, dodoAgentV3
...
class TestToolGenerationTelemetry:
    ...
    async def test_generate_tool_sets_call_type(self, mock_llm_config):
        ...
        with patch.object(LLMClient, "create") as mock_create:
            ...
            from dodo.server.rest_api.routers.v1.tools import GenerateToolInput, generate_tool_from_prompt
            ...
            with patch("dodo.server.rest_api.routers.v1.tools.derive_openai_json_schema") as mock_schema:
...
class TestAdapterTelemetryAttributes:
    ...
    def test_base_adapter_has_telemetry_attributes(self, mock_llm_config):
        """Verify base dodoLLMAdapter has telemetry attributes."""
        ...
        class TestAdapter(dodoLLMAdapter):
    ...
    def test_request_adapter_inherits_telemetry_attributes(self, mock_llm_config):
        """Verify dodoLLMRequestAdapter inherits telemetry attributes."""
    ...
    def test_stream_adapter_inherits_telemetry_attributes(self, mock_llm_config):
        """Verify dodoLLMStreamAdapter inherits telemetry attributes."""
    ...
    def test_request_and_stream_adapters_have_consistent_interface(self, mock_llm_config):
        ...
        request_adapter = dodoLLMRequestAdapter(llm_client=mock_client, llm_config=mock_llm_config, call_type=LLMCallType.agent_step)
        stream_adapter = dodoLLMStreamAdapter(llm_client=mock_client, llm_config=mock_llm_config, call_type=LLMCallType.agent_step)

        for attr in ["agent_id", "agent_tags", "run_id", "call_type"]:
            assert hasattr(request_adapter, attr), f"dodoLLMRequestAdapter missing {attr}"
            assert hasattr(stream_adapter, attr), f"dodoLLMStreamAdapter missing {attr}"
...
class TestAgentAdapterInstantiation:
    ...
    def test_agent_v2_creates_summarizer_with_agent_id(self, mock_llm_config):
        """Verify dodoAgentV2 creates Summarizer with correct agent_id."""
        ...
        agent = dodoAgentV2(agent_state=agent_state, actor=mock_actor)
