...
from dodo.client.types import AgentState, CreateBlockParam
from dodo.constants import DEFAULT_ORG_ID, FILES_TOOLS
from dodo.helpers.pinecone_utils import should_use_pinecone
from dodo.helpers.tpuf_client import TurbopufferClient
from dodo.schemas.enums import FileProcessingStatus, ToolType
...
def recompile_agent_system_prompt(client: dodoSDKClient, agent_id: str) -> None:
...
def get_raw_system_message(client: dodoSDKClient, agent_id: str, recompile: bool = False) -> str:
...
def clear_sources(client: dodoSDKClient):
...
def client() -> dodoSDKClient:
    # Get URL from environment or start server
    server_url = os.getenv("dodo_SERVER_URL", f"http://localhost:{SERVER_PORT}")
    ...
    client = dodoSDKClient(base_url=server_url)
    yield client


@pytest.fixture
def agent_state(disable_pinecone, client: dodoSDKClient):
...
def test_auto_attach_detach_files_tools(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
    ...
    def get_file_tools(agent_state):
        return {tool.name for tool in agent_state.tools if tool.tool_type == ToolType.dodo_FILES_CORE}
    ...
    def assert_no_file_tools(agent_state):
        has_file_tools = any(tool.tool_type == ToolType.dodo_FILES_CORE for tool in agent_state.tools)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tests/data/dodo_paper.pdf", "Dodo", r"test_source/dodo_paper\.pdf"),
...
def test_attach_existing_files_creates_source_blocks_correctly(
    disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState
):
    # Create a new source
    source = client.folders.create(name="test_source", embedding="openai/text-embedding-3-small")
    assert len(list(client.folders.list())) == 1
...
def test_agent_uses_open_close_file_correctly(disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState):
    # Create a new source
    source = client.folders.create(name="test_source", embedding="openai/text-embedding-3-small")
    ...
    assert "1: Enrico dodo" in old_value, f"Expected line 1 to start with '1: Enrico dodo', got: {old_value[:200]}..."
    ...
    assert "1: Enrico dodo" not in new_value, f"Second range (6-10) should not contain line 1, got: {new_value}"

    print("âœ“ File successfully opened with different range - content differs as expected")


def test_agent_uses_search_files_correctly(disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState):
    # Create a new source
    source = client.folders.create(name="test_source", embedding="openai/text-embedding-3-small")
...
def test_agent_uses_grep_correctly_basic(disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState):
    # Create a new source
    source = client.folders.create(name="test_source", embedding="openai/text-embedding-3-small")
...
def test_agent_uses_grep_correctly_advanced(disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState):
    # Create a new source
    source = client.folders.create(name="test_source", embedding="openai/text-embedding-3-small")
...
def test_create_agent_with_source_ids_creates_source_blocks_correctly(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
    ...
    file_tools = {tool.name for tool in temp_agent_state.tools if tool.tool_type == ToolType.dodo_FILES_CORE}
    assert file_tools == set(FILES_TOOLS)


def test_view_ranges_have_metadata(disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState):
    # Create a new source
    source = client.folders.create(name="test_source", embedding="openai/text-embedding-3-small")
...
def test_duplicate_file_renaming(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
    ...
    print("âœ“ Successfully tested duplicate file renaming:")
    for i, file in enumerate(files):
        print(f"  File {i + 1}: original='{file.original_file_name}' â†’ renamed='{file.file_name}'")


def test_duplicate_file_handling_replace(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
    ...
    try:
        ...
        print("âœ“ Successfully tested DuplicateFileHandling.REPLACE functionality")
...
def test_upload_file_with_custom_name(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
...
def test_open_files_schema_descriptions(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
...
def test_grep_files_schema_descriptions(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
...
def test_agent_open_file(disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState):
...
def test_agent_close_file(disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState):
...
def test_agent_close_all_open_files(disable_pinecone, disable_turbopuffer, client: dodoSDKClient, agent_state: AgentState):
...
def test_file_processing_timeout(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
...
def test_openai_embedding(disable_pinecone, disable_turbopuffer, client: dodoSDKClient):
...
# --- Pinecone Tests ---
def test_pinecone_search_files_tool(disable_turbopuffer, client: dodoSDKClient):
...
def test_pinecone_list_files_status(disable_turbopuffer, client: dodoSDKClient):
...
def test_pinecone_lifecycle_file_and_source_deletion(disable_turbopuffer, client: dodoSDKClient):
    """Test that file and source deletion removes records from Pinecone"""
    from dodo.helpers.pinecone_utils import list_pinecone_index_for_files, should_use_pinecone
...
# --- Turbopuffer Tests ---
def test_turbopuffer_search_files_tool(disable_pinecone, client: dodoSDKClient):
...
def test_turbopuffer_file_processing_status(disable_pinecone, client: dodoSDKClient):
...
def test_turbopuffer_lifecycle_file_and_source_deletion(disable_pinecone, client: dodoSDKClient):
...
def test_turbopuffer_multiple_sources(disable_pinecone, client: dodoSDKClient):
