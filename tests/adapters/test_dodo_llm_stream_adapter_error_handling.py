...
@pytest.mark.asyncio
async def test_dodo_llm_stream_adapter_converts_anthropic_streaming_api_status_error(monkeypatch):
    ...
    adapter = dodoLLMStreamAdapter(llm_client=llm_client, llm_config=llm_config, call_type=LLMCallType.agent_step)
...
@pytest.mark.asyncio
async def test_dodo_llm_stream_adapter_converts_anthropic_413_request_too_large(monkeypatch):
...
@pytest.mark.asyncio
async def test_dodo_llm_stream_adapter_converts_httpx_read_error(monkeypatch):
...
@pytest.mark.asyncio
async def test_dodo_llm_stream_adapter_converts_httpx_write_error(monkeypatch):
...
@pytest.mark.asyncio
async def test_dodo_llm_stream_adapter_raises_empty_response_error_for_anthropic(monkeypatch):
