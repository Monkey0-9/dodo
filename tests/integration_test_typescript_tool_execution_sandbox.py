...
from dodo.schemas.tool import Tool as PydanticTool, ToolCreate
...
from dodo.services.tool_sandbox.e2b_sandbox import AsyncToolSandboxE2B
...
@pytest.mark.e2b_sandbox
class TestTypescriptToolWithdodoClient:
    """Tests for TypeScript tools with dodo client integration."""

    @pytest.mark.asyncio
    async def test_e2b_typescript_dodo_client_available(self, check_e2b_key_is_set, test_user):
        """Test that the dodo client is available in TypeScript sandbox (as null when no API key)."""
        # Create a tool that checks if the client variable exists
        tool = PydanticTool(
            name="check_client",
            description="Check if dodo client is available",
            source_code="""
...
""",
        ...
        assert result.status == "success"
        # Without dodo_API_KEY, client should be null
...
class TestTypescriptGeneratorUnit:
    ...
    def test_convert_param_to_ts_value_string(self):
        ...
        from dodo.services.tool_sandbox.typescript_generator import convert_param_to_ts_value
    ...
    def test_extract_typescript_function_name(self):
        ...
        from dodo.services.tool_sandbox.typescript_generator import extract_typescript_function_name
    ...
    def test_is_async_typescript_function(self):
        ...
        from dodo.services.tool_sandbox.typescript_generator import is_async_typescript_function
    ...
    def test_generate_typescript_execution_script(self):
        ...
        # Verify dodo client initialization is included (using dynamic import with try/catch)
        assert "let client: any = null;" in script
        assert "await import('@dodo-ai/dodo-client')" in script
        assert "process.env.dodo_API_KEY" in script
    ...
    def test_parse_typescript_result(self):
        ...
        from dodo.services.tool_sandbox.typescript_generator import parse_typescript_result
