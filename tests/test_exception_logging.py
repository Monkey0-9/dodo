...
from dodo.exceptions.logging import add_exception_context, log_and_raise, log_exception
...
def test_exception_middleware_logs_basic_exception(app_with_exception_middleware):
    ...
    with patch("dodo.server.rest_api.middleware.logging.logger") as mock_logger:
...
def test_add_exception_context():
    ...
    assert hasattr(exc, "__dodo_context__")
    assert exc.__dodo_context__["user_id"] == "user-123"
    assert exc.__dodo_context__["agent_id"] == "agent-456"
    assert exc.__dodo_context__["operation"] == "test_op"
...
def test_log_and_raise():
    ...
    with patch("dodo.exceptions.logging.logger") as mock_logger:
...
async def test_asyncio_exception_handler():
    ...
    from dodo.server.global_exception_handler import setup_asyncio_exception_handler
