...
def batch_requests(agents):
    ...
    Returns:
        List[dodoBatchRequest]: Batch requests for each agent
    """
    return [
        dodoBatchRequest(agent_id=agent.id, messages=[MessageCreate(role="user", content=[TextContent(text=f"Hi {agent.name}")])])
        for agent in agents
    ]
...
async def test_rethink_tool_modify_agent_state(disable_e2b_api_key, server, default_user, batch_job, rethink_tool):
    ...
    agents = [agent]
    batch_requests = [
        dodoBatchRequest(agent_id=agent.id, messages=[MessageCreate(role="user", content=[TextContent(text="Rethink memory.")])])
        for agent in agents
    ]
    ...
    with patch("dodo.llm_api.anthropic_client.AnthropicClient.send_llm_batch_request_async", return_value=dummy_batch_response):
        # Create batch runner
        batch_runner = dodoAgentBatch(
        ...
        pre_resume_response = await batch_runner.step_until_request(
            ...
            dodo_batch_job_id=batch_job.id,
        )

    # 2. Invoke the polling job and mock responses from Anthropic
    mock_retrieve = AsyncMock(return_value=create_batch_response(batch_id=pre_resume_response.dodo_batch_id, processing_status="ended"))
...
async def test_partial_error_from_anthropic_batch(
    ...
    with patch("dodo.llm_api.anthropic_client.AnthropicClient.send_llm_batch_request_async", return_value=dummy_batch_response):
        ...
        llm_batch_jobs = await server.batch_manager.list_llm_batch_jobs_async(
            dodo_batch_id=pre_resume_response.dodo_batch_id, actor=default_user
        )
    ...
    with patch.object(server.anthropic_async_client.beta.messages.batches, "retrieve", mock_retrieve):
        ...
        with patch.object(server.anthropic_async_client.beta.messages.batches, "results", mock_results):
            with patch("dodo.llm_api.anthropic_client.AnthropicClient.send_llm_batch_request_async", return_value=dummy_batch_response):
                ...
                assert post_resume_response.dodo_batch_id == pre_resume_response.dodo_batch_id, (
                    "resume_step_after_request is expected to have the same dodo_batch_id"
                )
                ...
                # New batchâ€‘items should exist, initialised in (created, paused) state
                ...
                #     (sanity â€“ we already asserted this above, but we keep it close for clarity)
                ...
                # Toolâ€‘call sideâ€‘effects â€“ each agent gets at least 2 extra messages
                for agent in agents:
                    ...
                    if agent.id == agents_failed[0].id:
                        ...
                        assert after - before >= 2, (
                            f"Agent {agent.id} should have an assistant toolâ€‘call and toolâ€‘response message persisted."
                        )
                ...
                messages = await server.batch_manager.get_messages_for_dodo_batch_async(
                    dodo_batch_job_id=pre_resume_response.dodo_batch_id, limit=200, actor=default_user
                )
        ...
        with patch.object(server.anthropic_async_client.beta.messages.batches, "results", mock_results):
            with patch("dodo.llm_api.anthropic_client.AnthropicClient.send_llm_batch_request_async", return_value=dummy_batch_response):
                ...
                for agent in agents:
                    before = msg_counts_before[agent.id]  # captured just before resume
                    after = await server.message_manager.size_async(actor=default_user, agent_id=agent.id)
                    assert after - before >= 2, f"Agent {agent.id} should have an assistant toolâ€‘call and toolâ€‘response message persisted."
...
async def test_step_until_request_prepares_and_submits_batch_correctly(
    ...
    with patch("dodo.llm_api.anthropic_client.AnthropicClient.send_llm_batch_request_async") as mock_send:
        ...
        llm_batch_jobs = await server.batch_manager.list_llm_batch_jobs_async(dodo_batch_id=response.dodo_batch_id, actor=default_user)
        assert len(llm_batch_jobs) == 1, f"Expected 1 llm_batch_jobs, got {len(llm_batch_jobs)}"
