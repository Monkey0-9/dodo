...
from dodo.jobs.types import ItemUpdateInfo, RequestStatusUpdateInfo, StepStatusUpdateInfo
from dodo.orm import Base, Block
from dodo.orm.block_history import BlockHistory
from dodo.orm.errors import NoResultFound, UniqueConstraintViolationError
...
from dodo.schemas.environment_variables import SandboxEnvironmentVariableCreate, SandboxEnvironmentVariableUpdate
...
from dodo.schemas.organization import Organization, Organization as PydanticOrganization, OrganizationUpdate
...
from dodo.schemas.sandbox_config import E2BSandboxConfig, LocalSandboxConfig, SandboxConfigCreate, SandboxConfigUpdate
from dodo.schemas.source import Source as PydanticSource, SourceUpdate
from dodo.schemas.tool import Tool as PydanticTool, ToolCreate, ToolUpdate
from dodo.schemas.tool_rule import InitToolRule
from dodo.schemas.user import User as PydanticUser, UserUpdate
...
from dodo.services.helpers.agent_manager_helper import calculate_base_tools, calculate_multi_agent_tools, validate_agent_exists_async
from dodo.services.step_manager import FeedbackType
from dodo.settings import settings, tool_settings
from dodo.utils import calculate_file_defaults_based_on_context_window
from tests.helpers.utils import comprehensive_agent_checks, validate_context_window_overview
from tests.utils import random_string

DEFAULT_EMBEDDING_CONFIG = EmbeddingConfig.default_config(provider="openai")
CREATE_DELAY_SQLITE = 1
USING_SQLITE = not bool(os.getenv("dodo_PG_URI"))
...
async def other_organization(server: SyncServer):
    """Fixture to create and return the default organization."""
    org = server.organization_manager.create_organization(pydantic_org=Organization(name="dodo"))
    yield org
...
@pytest.fixture
def dodo_batch_job(server: SyncServer, default_user) -> Job:
...
def test_calculate_multi_agent_tools(set_dodo_environment):
...
async def test_upsert_base_tools_excludes_local_only_in_production(server: SyncServer, default_user, set_dodo_environment):
...
async def test_upsert_multi_agent_tools_only(server: SyncServer, default_user, set_dodo_environment):
    ...
    tools = await server.tool_manager.upsert_base_tools_async(actor=default_user, allowed_types={ToolType.dodo_MULTI_AGENT_CORE})
    tool_names = {tool.name for tool in tools}
...
def set_dodo_environment(request, monkeypatch):
...
async def test_update_agent(server: SyncServer, comprehensive_test_agent_fixture, other_tool, other_source, other_block, default_user):
    ...
    update_agent_request = UpdateAgent(
        ...
        system="train system",
        llm_config=LLMConfig.default_config("gpt-4o-mini"),
        embedding_config=EmbeddingConfig.default_config(model_name="dodo"),
        message_ids=["10", "20"],
...
async def test_attach_missing_files_tools_async(server: SyncServer, sarah_agent, default_user):
    ...
    await server.tool_manager.upsert_base_tools_async(actor=default_user, allowed_types={ToolType.dodo_FILES_CORE})
    ...
    file_tool_names = {tool.name for tool in updated_agent_state.tools if tool.tool_type == ToolType.dodo_FILES_CORE}
...
async def test_attach_missing_files_tools_async_partial(server: SyncServer, sarah_agent, default_user):
    ...
    file_tools = [tool for tool in all_tools if tool.tool_type == ToolType.dodo_FILES_CORE and tool.name in FILES_TOOLS]
    ...
    agent_state = await server.agent_manager.get_agent_by_id_async(agent_id=sarah_agent.id, actor=default_user)
    assert len([t for t in agent_state.tools if t.tool_type == ToolType.dodo_FILES_CORE]) == 1
...
async def test_attach_missing_files_tools_async_idempotent(server: SyncServer, sarah_agent, default_user):
    ...
    file_tool_names = {tool.name for tool in final_agent_state.tools if tool.tool_type == ToolType.dodo_FILES_CORE}
    assert file_tool_names == set(FILES_TOOLS)
...
async def test_detach_all_files_tools_async(server: SyncServer, sarah_agent, default_user):
    ...
    file_tool_count_before = len([t for t in agent_state.tools if t.tool_type == ToolType.dodo_FILES_CORE])
    ...
    file_tool_count_after = len([t for t in updated_agent_state.tools if t.tool_type == ToolType.dodo_FILES_CORE])
...
async def test_detach_all_files_tools_async_empty(server: SyncServer, sarah_agent, default_user):
    ...
    file_tool_count = len([t for t in agent_state.tools if t.tool_type == ToolType.dodo_FILES_CORE])
...
async def test_detach_all_files_tools_async_with_other_tools(server: SyncServer, sarah_agent, print_tool, default_user):
    ...
    file_tools = [t for t in agent_state.tools if t.tool_type == ToolType.dodo_FILES_CORE]
    ...
    remaining_file_tools = [t for t in updated_agent_state.tools if t.tool_type == ToolType.dodo_FILES_CORE]
...
async def test_detach_all_files_tools_async_idempotent(server: SyncServer, sarah_agent, default_user):
    ...
    assert len([t for t in agent_state.tools if t.tool_type == ToolType.dodo_FILES_CORE]) == 0
    ...
    assert len([t for t in final_agent_state.tools if t.tool_type == ToolType.dodo_FILES_CORE]) == 0
...
@pytest.mark.asyncio
async def test_modify_dodo_message(server: SyncServer, sarah_agent, default_user):
    """
    ...
    server.message_manager.update_message_by_dodo_message(
        message_id=user_message.id, dodo_message_update=update_user_message, actor=default_user
    )
    ...
    server.message_manager.update_message_by_dodo_message(
        message_id=system_message.id, dodo_message_update=update_system_message, actor=default_user
    )
    ...
    server.message_manager.update_message_by_dodo_message(
        message_id=reasoning_message.id, dodo_message_update=update_reasoning_message, actor=default_user
    )
    ...
    server.message_manager.update_message_by_dodo_message(
        message_id=assistant_message.id, dodo_message_update=update_assistant_message, actor=default_user
    )
...
async def test_list_tools_with_tool_types(server: SyncServer, default_user):
    ...
    tools = await server.tool_manager.list_tools_async(
        actor=default_user, tool_types=[ToolType.CUSTOM.value, ToolType.dodo_CORE.value], upsert_base_tools=False
    )
    assert len(tools) == 2
...
@pytest.mark.asyncio
async def test_list_tools_return_only_dodo_tools(server: SyncServer, default_user):
    """Test filtering for only dodo tools."""
    # first, upsert base tools to ensure we have dodo tools
    ...
    # test without filter (should get custom tool + all dodo tools)
    tools = await server.tool_manager.list_tools_async(actor=default_user, return_only_dodo_tools=False, upsert_base_tools=False)
    # should have at least the custom tool and some dodo tools
    assert len(tools) > 1
    assert any(t.name == "custom_tool" for t in tools)

    # test with filter (should only get dodo tools)
    tools = await server.tool_manager.list_tools_async(actor=default_user, return_only_dodo_tools=True, upsert_base_tools=False)
    assert len(tools) > 0
    # all tools should have tool_type starting with "dodo_"
    assert all(t.tool_type.value.startswith("dodo_") for t in tools)
...
async def test_count_tools_async(server: SyncServer, default_user):
    ...
    # upsert base tools to ensure we have dodo tools for counting
    await server.tool_manager.upsert_base_tools_async(actor=default_user)

    # count all tools (should have 2 custom tools + dodo tools)
    count = await server.tool_manager.count_tools_async(actor=default_user)
    assert count > 2  # at least our 2 custom tools + dodo tools
    ...
    # count with return_only_dodo_tools
    count = await server.tool_manager.count_tools_async(actor=default_user, return_only_dodo_tools=True)
    assert count > 0  # should have dodo tools

    # count with exclude_tool_types (exclude all dodo tool types)
    count = await server.tool_manager.count_tools_async(
        actor=default_user,
        exclude_tool_types=[
            ToolType.dodo_CORE.value,
            ToolType.dodo_MEMORY_CORE.value,
            ToolType.dodo_MULTI_AGENT_CORE.value,
            ToolType.dodo_SLEEPTIME_CORE.value,
            ToolType.dodo_VOICE_SLEEPTIME_CORE.value,
            ToolType.dodo_BUILTIN.value,
            ToolType.dodo_FILES_CORE.value,
        ],
    )
    assert count == 2  # only our custom tools
...
async def test_upsert_base_tools(server: SyncServer, default_user):
    ...
    if settings.environment == "PRODUCTION":
        expected_tool_names = sorted(dodo_TOOL_SET - set(LOCAL_ONLY_MULTI_AGENT_TOOLS))
    else:
        expected_tool_names = sorted(dodo_TOOL_SET)
    ...
    for t in tools:
        if t.name in BASE_TOOLS:
            assert t.tool_type == ToolType.dodo_CORE
        elif t.name in BASE_MEMORY_TOOLS:
            assert t.tool_type == ToolType.dodo_MEMORY_CORE
        elif t.name in MULTI_AGENT_TOOLS:
            assert t.tool_type == ToolType.dodo_MULTI_AGENT_CORE
        elif t.name in BASE_SLEEPTIME_TOOLS:
            assert t.tool_type == ToolType.dodo_SLEEPTIME_CORE
        elif t.name in BASE_VOICE_SLEEPTIME_TOOLS:
            assert t.tool_type == ToolType.dodo_VOICE_SLEEPTIME_CORE
        elif t.name in BASE_VOICE_SLEEPTIME_CHAT_TOOLS:
            assert t.tool_type == ToolType.dodo_VOICE_SLEEPTIME_CORE
        elif t.name in BUILTIN_TOOLS:
            assert t.tool_type == ToolType.dodo_BUILTIN
        elif t.name in FILES_TOOLS:
            assert t.tool_type == ToolType.dodo_FILES_CORE
        else:
...
@pytest.mark.parametrize(
    ...
    [
        (ToolType.dodo_CORE, BASE_TOOLS),
        (ToolType.dodo_MEMORY_CORE, BASE_MEMORY_TOOLS),
        (ToolType.dodo_MULTI_AGENT_CORE, MULTI_AGENT_TOOLS),
        (ToolType.dodo_SLEEPTIME_CORE, BASE_SLEEPTIME_TOOLS),
        (ToolType.dodo_VOICE_SLEEPTIME_CORE, sorted(set(BASE_VOICE_SLEEPTIME_TOOLS + BASE_VOICE_SLEEPTIME_CHAT_TOOLS) - {"send_message"})),
        (ToolType.dodo_BUILTIN, BUILTIN_TOOLS),
        (ToolType.dodo_FILES_CORE, FILES_TOOLS),
    ],
)
async def test_upsert_filtered_base_tools(server: SyncServer, default_user, tool_type, expected_names):
    ...
    if tool_type == ToolType.dodo_MULTI_AGENT_CORE and settings.environment == "PRODUCTION":
...
async def test_upsert_multiple_tool_types(server: SyncServer, default_user):
    allowed = {ToolType.dodo_CORE, ToolType.dodo_BUILTIN, ToolType.dodo_FILES_CORE}
...
def test_message_listing_text_search(server: SyncServer, hello_world_message_fixture, default_user, sarah_agent):
    ...
    # Test no results
    search_results = server.message_manager.list_user_messages_for_agent(
        agent_id=sarah_agent.id, actor=default_user, query_text="dodo", limit=10
    )
...
async def test_bulk_update_skips_missing_and_truncates_then_returns_none(server: SyncServer, default_user: PydanticUser, caplog):
    ...
    # prepare updates: one real id with an overâ€limit value, plus one missing id
    ...
    # default return_hydrated=False â†’ should be None
...
def test_checkpoint_creates_history(server: SyncServer, default_user):
    ...
    with db_registry.session() as session:
        ...
        assert hist.actor_type == ActorType.dodo_USER
...
def test_checkpoint_with_agent_id(server: SyncServer, default_user, sarah_agent):
    ...
    actor_type=dodo_AGENT, actor_id=<agent.id> in BlockHistory.
    """
    block_manager = BlockManager()
    ...
    with db_registry.session() as session:
        ...
        assert hist_entry.actor_type == ActorType.dodo_AGENT
...
async def test_create_and_upsert_identity(server: SyncServer, default_user):
    identity_create = IdentityCreate(
        ...
        properties=[
            IdentityProperty(key="email", value="caren@dodo.com", type=IdentityPropertyType.string),
...
async def test_get_identities(server, default_user):
    ...
    org = await server.identity_manager.create_identity_async(
        IdentityCreate(name="dodo", identifier_key="0001", identity_type=IdentityType.org), actor=default_user
    )
...
async def test_update_identity(server: SyncServer, sarah_agent, charles_agent, default_user):
    ...
    update_data = IdentityUpdate(
        ...
        properties=[IdentityProperty(key="email", value="caren@dodo.com", type=IdentityPropertyType.string)],
    )
...
async def test_source_vector_db_provider_with_tpuf(server: SyncServer, default_user):
    ...
    try:
        ...
        with patch("dodo.services.source_manager.should_use_tpuf", return_value=True):
            source_pydantic = PydanticSource(
        ...
        with patch("dodo.services.source_manager.should_use_tpuf", return_value=False):
            source_pydantic = PydanticSource(
...
async def test_create_local_sandbox_config_defaults(server: SyncServer, default_user):
    ...
    assert created_config.get_local_config().sandbox_dir in {dodo_TOOL_EXECUTION_DIR, tool_settings.tool_exec_dir}
    assert created_config.organization_id == default_user.organization_id
...
async def test_e2e_job_callback(monkeypatch, server: SyncServer, default_user):
    ...
    import dodo.services.job_manager as job_manager_module
...
def test_get_run_messages(server: SyncServer, default_user: PydanticUser, sarah_agent):
    ...
    run = server.job_manager.create_job(
        pydantic_job=PydanticRun(
            user_id=default_user.id,
            status=JobStatus.created,
            request_config=dodoRequestConfig(
...
@pytest.mark.asyncio
async def test_create_and_get_batch_request(server, default_user, dummy_beta_message_batch, dodo_batch_job):
    batch = await server.batch_manager.create_llm_batch_job_async(
        ...
        actor=default_user,
        dodo_batch_job_id=dodo_batch_job.id,
    )
...
@pytest.mark.asyncio
async def test_update_batch_status(server, default_user, dummy_beta_message_batch, dodo_batch_job):
    batch = await server.batch_manager.create_llm_batch_job_async(
        llm_provider=ProviderType.anthropic,
        status=JobStatus.created,
        create_batch_response=dummy_beta_message_batch,
...
async def test_create_and_get_batch_item(
    server, default_user, sarah_agent, dummy_beta_message_batch, dummy_llm_config, dummy_step_state, dodo_batch_job
):
    batch = await server.batch_manager.create_llm_batch_job_async(
        llm_provider=ProviderType.anthropic,
        status=JobStatus.created,
...
@pytest.mark.asyncio
async def test_list_running_batches(server, default_user, dummy_beta_message_batch, dodo_batch_job):
...
@pytest.mark.asyncio
async def test_bulk_update_batch_statuses(server, default_user, dummy_beta_message_batch, dodo_batch_job):
    batch = await server.batch_manager.create_llm_batch_job_async(
        llm_provider=ProviderType.anthropic,
        status=JobStatus.created,
        create_batch_response=dummy_beta_message_batch,
...
async def test_bulk_update_nonexistent_items(server, default_user, dummy_beta_message_batch, dummy_successful_response, dodo_batch_job):
    # Create a batch job
    batch = await server.batch_manager.create_llm_batch_job_async(
        llm_provider=ProviderType.anthropic,
        create_batch_response=dummy_beta_message_batch,
...
@pytest.mark.asyncio
@patch("dodo.services.mcp_manager.MCPManager.get_mcp_client")
async def test_create_mcp_server(mock_get_client, server, default_user):
    from dodo.schemas.mcp import MCPServer, MCPServerType, SSEServerConfig, StdioServerConfig
...
async def test_mcp_server_deletion_cascades_oauth_sessions(server, default_organization, default_user):
    ...
    from dodo.schemas.mcp import MCPOAuthSessionCreate, MCPServer as PydanticMCPServer, MCPServerType
...
async def test_mcp_server_resync_tools(server, default_user, default_organization):
    ...
    from dodo.schemas.tool import ToolCreate
...
def test_create_internal_template_objects(server: SyncServer, default_user):
    ...
    from dodo.schemas.agent import InternalTemplateAgentCreate
    from dodo.schemas.block import Block, InternalTemplateBlockCreate
    from dodo.schemas.group import InternalTemplateGroupCreate, RoundRobinManager
