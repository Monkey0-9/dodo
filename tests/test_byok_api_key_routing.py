...
def test_anthropic_client_uses_byok_override_key():
    ...
    with patch("dodo.llm_api.anthropic_client.anthropic") as mock_anthropic:
...
def test_minimax_client_uses_byok_override_key():
    ...
    with patch("dodo.llm_api.minimax_client.anthropic") as mock_anthropic:
        mock_anthropic.Anthropic.return_value = MagicMock()
...
def test_together_client_uses_byok_override_key():
    ...
    with patch("dodo.llm_api.together_client.OpenAI") as mock_openai:
...
def test_zai_client_uses_byok_override_key_sync_request():
    ...
    with patch("dodo.llm_api.zai_client.OpenAI") as mock_openai:
        mock_client = MagicMock()
        mock_response = MagicMock()
...
async def test_zai_client_uses_byok_override_key_async_request():
    ...
    with patch("dodo.llm_api.zai_client.AsyncOpenAI") as mock_async_openai:
...
def test_google_ai_client_uses_byok_override_key():
    ...
    with patch("dodo.llm_api.google_ai_client.genai.Client") as mock_gemini_client:
...
def test_google_vertex_client_uses_byok_override_key():
    ...
    with patch("dodo.llm_api.google_vertex_client.Client") as mock_vertex_client:
        client._get_client(llm_config)
...
def test_azure_client_uses_byok_override_key():
    ...
    with patch("dodo.llm_api.azure_client.AzureOpenAI") as mock_azure_openai:
        mock_client = MagicMock()
        mock_response = MagicMock()
