...
from dodo.functions.schema_validator import SchemaHealth, validate_complete_json_schema
...
async def test_add_mcp_tool_accepts_non_strict_schemas():
    ...
    from dodo.server.rest_api.routers.v1.tools import add_mcp_tool
    ...
    with patch("dodo.server.rest_api.routers.v1.tools.get_dodo_server") as mock_get_server:
...
async def test_add_mcp_tool_rejects_invalid_schemas():
    ...
    with patch("dodo.server.rest_api.routers.v1.tools.get_dodo_server") as mock_get_server:
        with patch.object(tool_settings, "mcp_read_from_config", True):  # Ensure we're using config path
            ...
            headers = HeaderParams(actor_id="test_user")
            from dodo.errors import dodoInvalidMCPSchemaError

            with pytest.raises(dodoInvalidMCPSchemaError) as exc_info:
