...
from dodo_client.types import MessageCreateParam, Tool, ToolReturnMessage
...
def client(server_url):
    ...
    client = dodo(base_url=server_url)
    return client
...
async def test_streamable_http_mcp_server_update_schema_no_docstring_required(client, agent_state, server_url):
    ...
    try:
        ...
        dodo_tool_1 = client.mcp_servers.tools.retrieve(mcp_server_id=server.id, tool_id=ask_question_tool.id)
        assert dodo_tool_1 is not None

        # Retrieve again - should work without issues
        dodo_tool_2 = client.mcp_servers.tools.retrieve(mcp_server_id=server.id, tool_id=ask_question_tool.id)
        assert dodo_tool_2 is not None
    finally:
        client.mcp_servers.delete(mcp_server_id=server.id)
        servers = client.mcp_servers.list()
        server_names = [s.server_name for s in servers]
