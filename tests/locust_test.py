...
from dodo.constants import BASE_TOOLS, DEFAULT_HUMAN, DEFAULT_PERSONA
from dodo.schemas.agent import AgentState, CreateAgent
from dodo.schemas.dodo_request import dodoRequest
from dodo.schemas.dodo_response import dodoResponse
from dodo.schemas.memory import ChatMemory
from dodo.schemas.message import MessageCreate, MessageRole
from dodo.utils import get_human_text, get_persona_text


class dodoUser(HttpUser):
    ...
    def send_message(self):
        ...
        with self.client.post(
            ...
            response = dodoResponse(**response.json())
    ...
    #    messages = [MessageCreate(role=MessageRole("user"), content="hello")]
    #    request = dodoRequest(messages=messages, stream_steps=True, stream_tokens=True, return_message_object=True)
    #    if stream_tokens or stream_steps:
    #        from dodo.client.streaming import _sse_post
    ...
    #        return dodoResponse(**response.json())
    #    try:
    #        response = self.dodo_client.send_message(message="Hello, world!", agent_id=self.agent_id, role="user")
    ...
    #    try:
    #        agent_state = self.dodo_client.get_agent(agent_id=self.agent_id)
    #    except Exception as e:
    #        with self.client.get("/", catch_response=True) as response:
    #            response.failure(str(e))
    ...
    #    try:
    #        memory = self.dodo_client.get_in_context_memory(agent_id=self.agent_id)
    #    except Exception as e:
    #        with self.client.get("/", catch_response=True) as response:
    #            response.failure(str(e))
