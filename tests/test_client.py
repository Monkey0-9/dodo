...
def mock_openai_server():
    ...
    # Ensure the dodo server uses this mock OpenAI endpoint.
...
def client(request, mock_openai_server):
    ...
    # Overide the base_url if the dodo_API_URL is set
    base_url = api_url if api_url else server_url
    # create the dodo client
    yield dodo(base_url=base_url)
...
# Fixture for test agent
@pytest.fixture
def search_agent_one(client: dodo):
    agent_state = client.agents.create(
...
# Fixture for test agent
@pytest.fixture
def search_agent_two(client: dodo):
    agent_state = client.agents.create(
...
def test_add_and_manage_tags_for_agent(client: dodo):
    """
...
def test_agent_tags(client: dodo, clear_tables):
...
# Agent memory blocks
# --------------------------------------------------------------------------------------------------------------------
def test_shared_blocks(disable_e2b_api_key, client: dodo):
    # create a block
...
def test_update_agent_memory_label(client: dodo):
...
def test_attach_detach_agent_memory_block(client: dodo, agent: AgentState):
...
def test_function_always_error(client: dodo):
...
def test_attach_detach_agent_tool(client: dodo, agent: AgentState):
...
# AgentMessages
# --------------------------------------------------------------------------------------------------------------------
def test_messages(client: dodo, agent: AgentState):
    # _reset_config()
...
# @pytest.mark.asyncio
# async def test_send_message_parallel(client: dodo, agent: AgentState, request):
#     """
...
def test_agent_listing(client: dodo, agent, search_agent_one, search_agent_two):
...
def test_agent_creation(client: dodo):
...
# --------------------------------------------------------------------------------------------------------------------
def test_initial_sequence(client: dodo):
    # create an agent
    agent = client.agents.create(
        memory_blocks=[{"label": "human", "value": ""}, {"label": "persona", "value": ""}],
        model="anthropic/claude-haiku-4-5",
...
def test_attach_sleeptime_block(client: dodo):
    agent = client.agents.create(
        memory_blocks=[{"label": "human", "value": ""}, {"label": "persona", "value": ""}],
        model="anthropic/claude-haiku-4-5",
        embedding="openai/text-embedding-3-small",
...
def test_agent_generate_with_model_override(client: dodo, agent: AgentState):
...
def test_agent_generate_invalid_agent_id(client: dodo):
...
def test_agent_generate_long_prompt(client: dodo, agent: AgentState):
...
def test_agent_generate_no_persistence(client: dodo, agent: AgentState):
