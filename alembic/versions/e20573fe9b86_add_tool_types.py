...
from dodo.constants import BASE_MEMORY_TOOLS, BASE_TOOLS
...
def upgrade() -> None:
    ...
    if not settings.dodo_pg_uri_no_default:
        return
    ...
    # Define the list of dodo core tools
    dodo_core_value = ToolType.dodo_CORE.value
    dodo_memory_core_value = ToolType.dodo_MEMORY_CORE.value
    custom_value = ToolType.CUSTOM.value

    # Update tool_type for dodo core tools
    op.execute(
        f"""
        UPDATE tools
        SET tool_type = '{dodo_core_value}'
    ...
    op.execute(
        f"""
        UPDATE tools
        SET tool_type = '{dodo_memory_core_value}'
